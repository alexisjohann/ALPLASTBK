...
**Version:** 3.0
**Datum:** 2026-02-25
**Änderungen v3.0:** Weg C — Gleichgewichtung innerhalb W und T, moderierte Hierarchie I (0.40/0.30/0.30), keine Items gestrichen. Alle Worked Examples neu berechnet.
...
### Gewichtungsphilosophie innerhalb der Dimensionen (Weg C, v3.0)

Ab v3.0 gilt der **pragmatisch-defensive Ansatz «Weg C»** für die Gewichtung *innerhalb* der vier Dimensionen:

| Dimension | Gewichtung innerhalb | Begründung |
|-----------|---------------------|------------|
| **A** | Gleichgewichtet (4 Items × 0.25) | Keine empirische Basis für Differenzierung |
| **W** | Gleichgewichtet (8 Items × 0.125) | Keine empirische Basis; Reifegrad 1 |
| **I** | Hierarchisch (0.40 / 0.30 / 0.30) | **Definitorisch** — act > plan > engage ist Teil der Konstrukt-Definition |
| **T** | Gleichgewichtet (6 Items × 1/6) | Keine empirische Basis; Reifegrad 1 |

**Warum Weg C (und nicht differenzierte Gewichtung)?**

1. **Doppelgewichtungs-Risiko:** Die Zwischen-Dimensionen-Gewichte (0.20/0.30/0.30/0.20) setzen bereits strategische Prioritäten. Innerhalb-Gewichte kompoundieren diesen Effekt unkontrolliert.
2. **Reifegrad 1:** Ohne Welle-1-Daten fehlt die empirische Basis für Differenzierung. Gleichgewichtung ist der Null-Hypothesen-konforme Default.
3. **Transparenz:** Gleichgewichtung ist sofort nachvollziehbar — kein Erklärungsbedarf gegenüber BFE.
4. **Stabilität:** Wenn einzelne Items in Folgewellen verändert werden, bleibt die Formel robust.

**Ausnahme I:** Die Hierarchie act > plan > engage ist **definitorisch** (nicht normativ). «Gemacht» (act) ist per Definition wertvoller als «geplant» (plan), was wiederum wertvoller ist als «sich informiert» (engage). Diese Ordnung ist Teil der Konstrukt-Definition und erfordert keine empirische Validierung.

**Reifegrad-Logik:** Ab Welle 2 können auf Basis empirischer Daten differenzierte Gewichte innerhalb W und T eingeführt werden (z.B. via Regressions-Gewichte auf G oder externe Kriterien).
...
```
W = (W1 + W2 + W3 + W4 + (100 − W5) + W6 + A5 + T5) / 8

Wobei:
  W1–W4, W6, A5, T5:  (Likert − 1) / 4 × 100  [direkt: höher = besser]
  W5:                   INVERTIERT (höherer Likert = höhere Barriere = schlechterer Score)
  Alle 8 Items gleichgewichtet (je 0.125 = 1/8)
```

### Gewichtungs-Rationale W (Weg C: Gleichgewichtung)

| Item | Gewicht | Konzept |
|------|---------|---------|
| W1 (u_F) | 1/8 | Finanzielle Motivation |
| W2 (u_S) | 1/8 | Prosoziale Motivation |
| W3 (u_X) | 1/8 | Identitäts-Kongruenz |
| W4 (θ_inv) | 1/8 | Self-Efficacy |
| W5 (τ, inv.) | 1/8 | Wahrgenommene Komplexität |
| W6 (σ_d) | 1/8 | Deskriptive Sozialnorm |
| A5 (u_E) | 1/8 | Environmental Utility (aus A-Block reklassifiziert) |
| T5 (σ_i) | 1/8 | Injunktive Sozialnorm (aus T-Block reklassifiziert) |

**Begründung Gleichgewichtung (Weg C):** Ohne Welle-1-Daten fehlt die empirische Basis für differenzierte Gewichtung. Die Zwischen-Dimensionen-Gewichte (W=0.30) setzen bereits die strategische Priorität. Innerhalb-Differenzierung wird ab Welle 2 empirisch geprüft.

**v2.0-Gewichte (zur Referenz):** W4=0.20, W5=0.20, W1=0.15, W2/W3/W6/A5=0.10, T5=0.05 — basierend auf Bandura (1977), Enke et al. (2024). Können ab Welle 2 als Alternativ-Szenario berechnet werden.
...
```
I = 0.40 × I_act + 0.30 × I_plan + 0.30 × I_engage
```
...
I = 0.40 × 20.0 + 0.30 × 20.0 + 0.30 × 68.0
  = 8.0 + 6.0 + 20.4
  = 34.4
```
...
```
T = (T1 + T2 + T3 + T4 + T6 + (100 − T7)) / 6

Alle 6 Items gleichgewichtet (je 1/6 ≈ 0.167)
```
...
T = (75.0 + 75.0 + 50.0 + 75.0 + 50.0 + 75.0) / 6
  = 400 / 6
  = 66.7
```
...
| | T1–T4, T6, T7 | Siehe T-Beispiel | 66.7 |
...
Schritt 1: Dimensions-Scores berechnen
  ...
  T = (75 + 75 + 50 + 75 + 50 + 75) / 6
    = 400 / 6
    = 66.7

Schritt 2: Gesamt-Score berechnen (Model B)

  G = 0.20 × 68.75 + 0.30 × 53.1 + 0.30 × 34.4 + 0.20 × 66.7
    = 13.75 + 15.93 + 10.32 + 13.34
    = 53.3

Schritt 3: Interpretation
  → G = 53.3 → «Zufriedenstellend» (gelb)
  → Stärkste Dimension: A = 68.75 (Awareness gut)
  → Schwächste Dimension: I = 34.4 (Impact — tatsächliche Handlung tief)
  → W-Diagnose: Komplexität (W5 inv.=25) und Sozialnorm (W6=25) sind die Hebel
...
→ W2 wird ausgeschlossen, Mittelwert über 7 Items:
→ W = (75 + 50 + 50 + 25 + 25 + 75 + 50) / 7 = 350 / 7 = 50.0
```
...
```
Dashboard-Output:
  A = 68.75  |  W = 53.1  |  I = 34.4  |  T = 66.7  |  G = 53.3
```
...
Level 2: Dimensions-Scores (Weg C: Gleichgewichtung W/T, Hierarchie I)
──────────────────────────────────────────────────────────────────────
  A = (A1 + A2 + A3 + A4) / 4                                       ← 4 Items, gleich

  W = (W1 + W2 + W3 + W4 + (100−W5) + W6 + A5 + T5) / 8           ← 8 Items, gleich

  I = 0.40×I_act + 0.30×I_plan + 0.30×I_engage                     ← hierarchisch

  T = (T1 + T2 + T3 + T4 + T6 + (100−T7)) / 6                     ← 6 Items, gleich
      (nur wenn T0 = Ja)
...
| 3.0 | 2026-02-25 | **Weg C — Innerhalb-Dimensionen-Gewichtung überarbeitet.** W: Gleichgewichtung aller 8 Items (je 1/8) statt differenzierter Gewichtung (W4=0.20, W5=0.20 etc.). T: Gleichgewichtung aller 6 Items (je 1/6) statt differenzierter Gewichtung (T1=0.25, T2=0.20 etc.). I: Moderierte Hierarchie 0.40/0.30/0.30 (statt 0.45/0.30/0.25) — Hierarchie bleibt weil definitorisch, aber Spread verringert. A und G unverändert. **Keine Items gestrichen** — alle Items bleiben in ihren Dimensionen. Begründung: Reifegrad 1 (keine empirische Basis für Differenzierung), Doppelgewichtungs-Risiko, Transparenz. Differenzierte Gewichte als Alternativszenario ab Welle 2 möglich. Alle Worked Examples und Formel-Hierarchie neu berechnet. |

---

*Erstellt von FehrAdvice & Partners AG, 2026-02-25.*
