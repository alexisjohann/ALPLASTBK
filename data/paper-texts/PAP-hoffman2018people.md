...
**Year:** 2018 (Revised April 2020)
**Journal:** NBER Working Paper No. 24360
**DOI:** 10.3386/w24360
...
ductivity diﬀerences across firms and countries (Syverson, 2011). While a growing literature
shows that management practices aﬀect firm performance and help explain these productivity
...
and occupations, providing granular quantification of managerial diﬀerences within the same
multinational firm.
...
firm outcomes on CEO fixed eﬀects, pioneering an approach used by a subsequent finance
...
intheotherwave. Section2furtherdescribesthedata. Section3coversourempiricalstrategy.
...
“regretted”voluntaryturnover. Beyondclassicalmeasurementerror,theIVstrategyaddresses
...
gies, some similar to those in the teacher VA literature (Chetty et al., 2014). Our first
...
1Many organizations survey employees about their managers, including Google (Garvin et al., 2013), RBC
(Shaw and Schifrin, 2015), Pepsi (Bracken et al., 2016), and the US federal government (Arrington and Dwyer,
2018). CultureAmp, a firm survey vendor, informed us that about 15-20% of its 1,000+ clients use upward
...
managementskillshavingastrong, causaleﬀectonattrition. Section5discussesheterogeneity
in MOR eﬀects.
...
Section 7establishes our secondary resultthat managers withbetterpeople management
...
switchinglocationsorfunctions, andbycharacterizingheterogeneityinestimatesbyhierarchy,
...
2Also, Bandiera et al. (2007) show that manager bonuses boost productivity. Bandiera et al. (2015) study
...
Second, itrelatestostudiesofcompensationinfirms(e.g.,Bakeretal.,1994a), providing
...
2014), and we define people management skills as a manager’s ability to eﬀectively interact
...
ager’s peoplemanagement skillsand a worker’s cognitive skills. Thisis noteworthy as it occurs
even within the firm’s relatively high-skilled workforce.
Fourth, itexpandstheliteratureonknowledge-basedemployees(e.g.,Bakeretal.,1994a;
...
3Surveysexploredinpsychologyandmanagementseemfocusedondiﬀerentissueslikeconsistencyofratings
...
anonymityofresponses, surveyinformationaboutone’smanagerisonlycollectedonmanagers
...
4In the Y1 survey, the threshold was 3 employees, whereas in Y2, the threshold was 5 employees. We do not
...
5The manager questions are part of a longer survey covering other topics (e.g., firm decision-making, worker
engagement). To preserve firm confidentiality, the wording may be slightly modified from the original.
6It is common practice to break the 5-answer scale into 2 or 3 parts (Garvin et al., 2013). Using the share
...
Salary increases. Onewayofproxyinganemployee’sproductivityimprovementsisthe
...
7Other factors may limit persistence of MOR. First, managers may provide better supervision for certain
...
basedonMOR.Conversationswithseveralexecutivesconfirmthatthisisnotdoneinpractice;
...
8Workers disclose inventions to the legal department, who decides whether to file a patent application.
...
Tocreateoursample,werestrictattentiontoworker-monthswhereanemployeehasamanager
...
we only observe the noisy survey measure, mj. In our data, we have the two waves of the
survey, giving us two manager scores mj,1 and mj,2, with mj,τ= mj +uj,τ, τ ∈ {1,2}. t is at
...
are not selecting their own survey score.9 For simplicity, we also assume that var(uj,τ) = σ2
u
...
suppress i and j subscripts. For OLS, we use plim(bOLS) =cov(yt,mτ)
var(mτ) plus Assumption 1 to
...
same manager during diﬀerent periods. Using plim(bIV ) =cov(yt,m−τ)
cov(mτ,m−τ), we get:
plim(bIV−β) =−
cov(uτ,u−τ)
cov(εt,u−τ)
...
Assignment Bias
In (5), the numerator of the first term has cov(uτ,u−τ) in place of σ2
u in (4). Thus, if the
...
9People answering questions about themselves are often subject to desirability bias (Bound et al., 2001),
...
σ2
m+cov(uτ,u−τ) instead of σ2
m+σ2
u. Thus, IV amplifies assignment bias if cov(uτ,u−τ) < σ2
u.
...
Asynchronously Corr. ME
Assignment Bias
RelativetotheIV,adisadvantageofthereducedformisthatthereisstillthesameattenuation
bias as for OLS. A potential advantage is that assignment bias is scaled by σ2
m + σ2
u in the
denominator instead of σ2
m +cov(uτ,u−τ).
...
diﬀerent parts of the firm and then observe employee outcomes. Still, even if we do not wish
to rule out better managers selecting better people, we need to address the possibility of the
firm optimally sorting managers and employees together (“assignment bias”).
Control variables. Adding control variables helps address the possibility that MOR
11
and employee outcomes may diﬀer systematically. We control for the firm’s diﬀerent business
...
diﬀerent variation and addressing diﬀerent threats to identification, all three designs provide
complementaryevidencethatpeoplemanagementskillsubstantiallyreducesattrition. Section
...
10p10 of a standard normal is 1.28σ below the mean, corresponding to a monthly attrition rate of 1.374-
...
results for both quits and fires. The coeﬃcient is larger in absolute magnitude for quits, but
...
a very large firm, they are unlikely to have substantial information about diﬀerences in people
managementskillsacrossmanagersthatwouldenablethemtochoosetheirmanagers. Further,
...
tionshipbetweenMORandattritioninTable4isqualitativelysimilar. Lookingatallattrition
11Turnover is labeled as voluntary, involuntary, or missing/not assigned. We don’t use missing turnover
events in Panels B and C (they are included in Panel A). Involuntary exits or “fires” include both true fires
and layoﬀs. The “fires” results become even stronger if months with layoﬀs are excluded.
12It could also reflect high-MOR managers attracting less attrition-prone workers, though Section 4.3 shows
...
Table 3, though it just misses statistical significance at 10%. The coeﬃcient implies that a 1σ
...
sortingofexistingemployeestomanagers,followingRothstein(2010,2017)—wehappentofind
...
as in the joiners analysis, the workers in this design do not influence the instrument.
...
conventional statistical significance (p = 0.16). In contrast, the coeﬃcient for quitting (panel
14
B) is slightly larger in magnitude than the one in Table 3. The negative quit coeﬃcient is
driven by regretted quits, where the MOR coeﬃcient is statistically significantly negative.
...
changesinthesecondperiod.13 Asseeninpanel(a),therelationbetweenMORandturnoveris
negative,butissmallinmagnitudeduringthefirstsixmonthsafteramanagerchange. Rather,
...
and whether the firm has designated a worker as a “key individual” whom they strongly want
...
13After joining the firm or changing manager in period 2, 17% of workers experience one additional change
...
assignment bias does not drive our findings because by aggregating, we focus on diﬀerences
...
where δl, δf, δτ are location, job function, and period fixed eﬀects, respectively; and θl,f,τ
...
to help ensure that diﬀerences across cells reflect diﬀerences in manager quality as opposed to
...
14Observed matching of employees and managers reflects not only potential assignment bias by the firm, but
...
unobserveddeterminantsofattrition,conditionaloncontrols.15 Tocontrolforpossiblechanges
...
the key identifying assumption is diﬃcult to test, there were no evident eﬀorts by the firm
...
16%. Thus, ahighershareofjob functionchangesseemtooccurfrom promotionscomparedto
...
the residuals on worker fixed eﬀects. Fixed eﬀects above the median are classified as high-
15For even greater control, one might wish to control for location-function fixed eﬀects instead of location
fixed eﬀects and function fixed eﬀects. However, we do not have enough power to do so. Instead, we control
...
Switching managers. Beyondleadingpeopletoexitthefirm,poorpeoplemanagement
skills could also produce other types of “exits.” Instead of quitting the firm, a worker may
...
asothermanagercharacteristicsarecontrolledfor(Gloveretal.,2017). Whileourdatacontain
...
16A manager’s underlying VA is γ. Let ˆ γ1 = γ +u1 and ˆ γ2 = γ +u2 be estimated VA in two split samples,
where u1 and u2 are errors. Under the stated assumptions, cov(ˆ γ1,
ˆ
γ2) = var(γ) (see also Silver (2016)).
17High hierarchy jobs include those such as Senior Director, Principal Design Engineer, and Distinguished
Research Scientist (our sample does not include the very small share of people at the top of the firm hierarchy).
...
occupation, and firm rank explain only a modest portion of the variation in MOR scores,
...
mediumorhighpositioninthefirmhierarchy. TheinstrumentsaremanagerMORintheother
...
betweenemployeecognitiveskillsandemployee socialskills(Weinberger,2014;Deming,2017).
...
whencontrollingfortheinteractionofMORandadummyfortheworkerhavingabovemedian
20
...
medium, or high position in the firm’s hierarchy, respectively.18 However, Figure 4 is similar
...
non-attritionoutcomesofsubordinateemployees. BaselineIVresultsshowamoderatepositive
18As we believe is common at other high-tech firms, there are some individuals who are high up in the firm’s
...
19Another explanation is that being higher up in the hierarchy reflects geography, as we also tend to see some
...
20There are also alternative explanations. First, the result could additionally reflect complementarity with
...
0.06% increase in IV, both insignificant. The mean salary increase per year is confidential, but
is between 4% and 8%. With 95% confidence, our IV coeﬃcients rule out that a 1σ increase
...
confidence interval is 0.24. With a mean monthly promotion rate in our sample of 1.5pp, we
can rule out that a 1σ increase in MOR would increase the promotion probability by about
15%. Recall that we find 1σ decreases attrition by 28%, so the top of the confidence interval
...
21A null result also occurs if the outcome is log salary or log restricted stock holdings. These results suggest
...
Rj,t= bmj,τ(t) +θj,t (8)
whereRj,t isareward(e.g.,subjectiveperformancescore,orstockgrants)achievedbymanager
...
in span of control by 0.3 individuals, but it is not statistically significant (though the standard
...
diﬀerence in the firm’s hierarchy.
Anotherquestionis,forhighMORmanagers,howmuchcostdoesthefirmincurinhigher
...
also help us address a more refined research question. VA papers answer: how much do
managersmatteroverall? Weanswer: howmuchdopeople management skills, orinterpersonal
skills for dealing with one’s subordinates, matter? Upward feedback surveys are used by many
firms, but we have little hard evidence on the importance of people management skills.
...
While our conclusions are specific to one firm, our main findings on the importance of
...
U.S. jobs, and workers where the firm regards attrition as bad instead of good). Method-
ologically, we illustrate the value of analyzing personnel data from a large firm, giving the
researcher significant information and detail about the type of jobs that workers are doing,
...
References
Arrington, Glenda and Rocky Dwyer,“CanFourGenerationsCreateHarmonyWithinaPublic-
...
Atkins, Paul WB and Robert E Wood, “Self-versusOthers’RatingsasPredictorsofAssessment
...
, Luigi Guiso, Andrea Prat, and Raﬀaella Sadun, “Matching firms, managers, and incen-
...
Lazear, Edward P., Kathryn Shaw, and Christopher Stanton,“TheValueofBosses,”Journal
...
-1.5 -1 -.5 0 .5
0 1 2 3
Quarter relative to manager change
0 1 2 3
Quarter relative to manager change
(a) Attrition
(b) Quits
...
(e) Non-regretted Quits
(f) Worker changes manager
Notes: The dotted lines show 95% confidence intervals on coeﬃcients, with standard errors clustered by manager. This figure
...
instrument) are interacted with quarters since getting a new manager. 31
...
firm hierarchy. The full specification appears in Panel E of Appendix Table C15. In the figure here, the coeﬃcients on “Low in Hierarchy” correspond
...
Share of records, employee in US Share of records from managers Share of records for engineers Worker is co-located with their manager Worker has same function as their manager Average manager span (employees/mgr) 9.35
Managers per employee in the sample Managers per employee (weighted by tenure) Worker was hired during the sample period Low level in the firm hierarchy Medium level in the firm hierarchy High level in the firm hierarchy 0.70
0.21
0.36
0.81
0.86
...
number of exits which are not classified in the data as voluntary or involuntary.
33
34
...
variable for a location being in the US. The questions from the survey are listed in the main text in Section 2.1. * significant at 10%; ** significant at
5%; *** significant at 1%
...
they had during September Y1 when the first survey was administered). The sample size is about 1/4 the
size of that in Table 3. * significant at 10%; ** significant at 5%; *** significant at 1%
37
...
worker’s first change in manager during the second period. An observation is an employee-month occurring during period 1 (January Y1-September
...
column 5 are robust to using a confidence interval based on Moreira’s conditional likelihood ratio test. Panel C presents the reduced form regression
of employee outcomes on the new manager’s MOR as measured during period 2. * significant at 10%; ** significant at 5%; *** significant at 1%
...
(0.097) (0.222)
Mean dep. var. F-stat on excl instrument 1.458 1.458 1.458 1.458
24.24 27.93
Panel B: Quits
...
(0.060) (0.131)
Mean dep. var. F-stat on excl instrument 0.722 0.722 0.722 0.722
24.24 27.93
Panel C: Fires
...
(0.045) (0.118)
Mean dep. var. F-stat on excl instrument 0.238 0.238 0.238 0.238
24.24 27.93
Panel D: Regretted Quits
...
(0.054) (0.120)
Mean dep. var. F-stat on excl instrument 0.606 0.606 0.606 0.606
24.24 27.93
Panel E: Non-regretted Quits
...
(0.031) (0.067)
Mean dep. var. F-stat on excl instrument 0.116 0.116 0.116 0.116
24.24 27.93
...
control for period. * significant at 10%; ** significant at 5%; *** significant at 1%
39
...
variable is patent applications per month, with coeﬃcients multiplied by 100 for readability. In column 5, “citation-weighted patents” is defined as a
