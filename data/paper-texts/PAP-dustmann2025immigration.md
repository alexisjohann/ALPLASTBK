...
**Authors:** Dustmann, Christian and Otten, Sebastian and Schönberg, Uta and Stuhler, Jan
**Year:** 2025
...
Interpretation*
Christian Dustmann 1 , Sebastian Otten 2 , Uta Schönberg 3 ,
and Jan Stuhler 4
...
acknowledges funding from the Comunidad de Madrid (CAM-EPUC3M11) and MICIU/AEI (CEX2021001181-M).
1
...
2
University of Duisburg-Essen, Department of Economics and CReAM, sebastian.otten@uni-due.de.
3
University of Hong Kong, University College London, Department of Economics, Institute for Employment
Research Nuremberg (IAB), and CReAM, u.schoenberg@ucl.ac.uk.
4
Universidad Carlos III de Madrid, Department of Economics and CReAM, jstuhler@eco.uc3m.es.
...
Examples include Card (1990), Hunt (1992), Dustmann, Fabbri and Preston (2005), Borjas (2006), Glitz (2012),
...
Our central argument is that changes in regional outcomes in response to a regionspecific immigration shock can differ substantially from changes in worker-level outcomes
...
Other papers that leverage a sharp increase in the inflow or outflow of cross-border commuters include Beerli et
...
Using SIAB data for German districts from 1985 to 2015, we find a correlation of 0.353 between regional
employment growth and the share of incumbent workers remaining employed in the following year—a workerlevel measure. Similarly, the correlation between wage growth for continuously employed workers and regional
...
While Bratsberg and Raaum (2012), Fallah, Krafft, and Wahba (2019), Ortega and Verdugo (2022), and
...
immigration-induced changes in the price of labor (the pure wage effect), and immigrationinduced changes in the workforce composition.
...
𝐿𝑁 = ∑𝑘 𝜃𝑘 𝐸𝑘 , where 𝜃𝑘 is the productive efficiency of worker type 𝑘 and 𝐸𝑘 is the number of
...
Accordingly, the average regional wage is equal to 𝑤
̅𝑅 =
...
𝑤, where ∑𝑘 𝐸𝑘 𝑘 measures
𝑘 𝑘

For instance, while Autor et al. (2014) examine how trade shocks affect individual workers, we develop a
...
worker types (i.e., 𝑑𝑙𝑜𝑔𝑤𝑘 /𝑑𝐼 𝐸 = 𝑑𝑙𝑜𝑔𝑤/𝑑𝐼 𝐸 ∀ 𝑘, where 𝑑𝐼 𝐸 is the immigration shock in
efficiency units).
...
and native) labor supply (i.e., 𝜑 = 𝑑𝑙𝑜𝑔𝑤/(𝑑𝐼 𝐸 + 𝑑𝑙𝑜𝑔𝐿𝑁 )). In Appendix C.1, we show that
...
an efficiency-weighted elasticity (i.e., 𝜂̅ 𝐸 = ∑𝑘 𝜃𝑘 𝐸𝑘 𝑘 ) and a population-weighted elasticity
𝑘 𝑘 𝑘

∑ 𝐸 𝜂

(𝜂̅ 𝑃 = ∑𝑘 𝑘 𝑘 ), as follows:
𝑘 𝐸𝑘
...
where 𝑐 is the ratio between the immigration shock in efficiency units 𝑑𝐼 𝐸 and the immigration
𝑑𝐼 𝐸

shock in head counts 𝑑𝐼 𝑃 (𝑐 = 𝑑𝐼𝑃 ). The labor supply elasticities 𝜂̅ 𝐸 and 𝜂̅ 𝑃 determine to what
...
Regional Wage Responses. Regional wages 𝑤
̅ 𝑅 will adjust to immigration not only due to
...
equal to the population-weighted labor supply elasticity (i.e., 𝜂̅ 𝐸 = 𝜂̅ 𝑃 ). However, the two
...
depends on whether the group-specific labor supply elasticity 𝜂̅ 𝑃 is positively or negatively
correlated with productive efficiency 𝜃𝑝 . Suppose that low-productivity workers exhibit a
higher labor supply elasticity than high-productivity workers. In this case, 𝜂̅ 𝐸 < 𝜂̅ 𝑃 ; an
immigration shock will therefore improve worker quality in the region, possibly masking a
negative pure wage effect (if 𝜂̅ 𝐸 ≥ 𝜂̅ 𝑃 − 1), or even flipping its sign (if 𝜂̅ 𝐸 < 𝜂̅ 𝑃 − 1).
The aggregate labor supply elasticities 𝜂̅ 𝐸 and 𝜂̅ 𝑃 reflect various employment
...
to the immigration shock in head counts Δ𝐼𝑟 (corresponding to 𝑑𝐼 𝑃 in the canonical model):
𝐸𝑟1 − 𝐸𝑟0
= ∆𝑎 + 𝛽 𝑅 Δ𝐼𝑟 + Δ𝜖𝑟 ,
𝐸𝑟0
...
𝐼 −𝐼
where Δ𝐼𝑟 = 𝐸𝑟1 +𝐼̃𝑟0 ; 𝐼̃𝑟𝑡 denotes the stock of immigrants in region r at time 𝑡 for 𝑡 = 0,1, and
𝑟0
...
total impact of the immigrant share on (log) employment of natives, 𝛽 𝑅 , is the parameter of
interest. It corresponds to the native employment effect of immigration in equation (1b) and is

6

Suppose that the treatment regions are small relative to the control regions so that any spillover effects of the
immigration shock to control regions are unlikely.
7
The corresponding level equation is given by log 𝐸𝑟𝑡 = 𝑎𝑡 + 𝑏𝑟 + 𝛽 𝑅 𝐼𝑟𝑡 + 𝜖𝑟𝑡 , where 𝑎𝑡 and 𝑏𝑟 denote time and
...
employment in the base period. Equation (2) follows as log 𝐸𝑟1 − log 𝐸𝑟0 ≈ 𝑟1
8
...
In our empirical implementation, this will be achieved by appropriate instrumentation.

10

increasing in the aggregate labor supply elasticities 𝜂̅ 𝐸 and 𝜂̅ 𝑃 and the inverse labor demand
...
unaffected regions.” We refer to 𝛽 𝑅 as the “regional” employment effect of immigration.
The parameter 𝛽 𝑅 is, however, not informative about the underlying individual
...
Many U.S. studies consider the change in the local employment-to-population rate rather than the change in
...
Examples from this policy debate include “Do immigrants take jobs from American-born workers?” in the New
York Times (January 6, 2015) or “Do immigrants “steal” jobs from American workers?” by the Brookings
Foundation (August 24, 2017).
11
Amior and Manning (2018), Monras (2020b) and Amior (2021) also emphasize the role of inflows as a means
of adjustment to local shocks.
12
For example, these “missing inflows” may have found jobs in regions less affected by immigration.
...
to the regional employment effect, 𝛽 𝑅 . It follows that 𝛽 𝑅 can only be interpreted as the
displacement effect if crowding-out and relocation effects are equal to zero. Similarly, regional
𝐸 −𝐸𝑟0

employment growth ( 𝑟1𝐸

𝑟0

𝐸

) and worker-level employment changes ( 𝐸𝑟,𝑁 ) will be perfectly
𝑟0
...
population-weighted) labor supply elasticities 𝜂̅ 𝐸 and 𝜂̅ 𝑃 can be thought of as the sum of a
...
base period; see footnote 7), and 𝛾 𝑊 is the “pure” wage effect of immigration, corresponding
13
...
Δlog 𝑤𝑟 = Δ𝑐 + 𝛾 𝑅 Δ𝐼𝑟 + Δ𝑣𝑟 ,

(5)

where Δlog 𝑤𝑟 = log 𝑤𝑟1 − log 𝑤𝑟0 and Δ𝑣𝑟 = 𝑣𝑟1 − 𝑣𝑟0 . 14 The coefficient 𝛾 𝑅 provides an
estimate of the “regional” wage effect of immigration, corresponding to the coefficient in
equation (1c) of our theoretical model. The regional wage effect of immigration 𝛾 𝑅 equals the
“pure” wage effect 𝛾 𝑊 (see equation (4)) only if the composition of workers employed in
regions hit by immigration is not affected by the immigration shock. In terms of our theoretical
model, this is the case if the efficiency-weighted labor supply elasticity equals the populationweighted elasticity (i.e., 𝜂̅ 𝐸 = 𝜂̅ 𝑃 in equation (1c)). However, the composition of employed
...
identified from estimates of the regional wage and employment responses 𝛾 𝑅 and 𝛽 𝑅 .
To see that, consider the decomposition of the difference in regional wages between
periods 0 and 1 (LHS of equation (5)):

13

Recent examples include Edo (2020), Monras (2020a) or Delgado-Prieto (2024).
...
15
The composition of employed workers may also change if different groups are imperfect substitutes in
...
− (𝐸[𝑙𝑜𝑔𝑤
⏟
𝑖𝑟1 |𝐼𝑟1 , 𝑑𝑖𝑟0 = 1, 𝑑𝑖𝑟1 = 1] − 𝐸[𝑙𝑜𝑔𝑤𝑖𝑟1 |𝐼𝑟1 , 𝑑𝑖𝑟0 = 0, 𝑑𝑖𝑟1 = 1]) Pr(𝑑𝑖𝑟0 = 0| 𝐼𝑟1 , 𝑑𝑖𝑟1 = 1) ,
(iii) Inflows
...
limit of the OLS estimate for 𝛾 𝑅 in equation (5) can be decomposed as follows:
(7)
...
+ 𝜕((𝐸([𝑣
⏟
𝑖𝑟0 |𝐼𝑟0 , 𝑑𝑖𝑟0 = 1, 𝑑𝑖𝑟1 = 1] − 𝐸[𝑣𝑖𝑟0 |𝐼𝑟0 , 𝑑𝑖𝑟0 = 1, 𝑑𝑖𝑟1 = 0])Pr (𝑑𝑖𝑟1 = 0|𝐼𝑟0 , 𝑑𝑖𝑟0 = 1))/𝜕Δ𝐼𝑟
(i i)

− 𝜕((𝐸[𝑣
𝑖𝑟1 |𝐼𝑟1 , 𝑑𝑖𝑟0 = 1, 𝑑𝑖𝑟1 = 1] − 𝐸[𝑣𝑖𝑟1 |𝐼𝑟1 , 𝑑𝑖𝑟0 = 0, 𝑑𝑖𝑟1 = 1])Pr (𝑑𝑖𝑟0 = 0|𝐼𝑟1 , 𝑑𝑖𝑟1 = 1))/𝜕Δ𝐼𝑟
⏟
(i i i)
...
immigration 𝛾 𝑊 ; (i-b) a selectivity bias referring to continuously employed natives; and
...
periods identifies 𝛾 𝑊 under the assumption that the term (i-b) equals zero. This will be the case
...
to changes in worker quality, 𝑑log ∑𝑘 𝐸𝑘 𝑘 /𝑑𝐼. Estimates of components (ii) and (iii) are
𝑘 𝑘
...
See, e.g., Lubotsky (2007) for a similar assumption in the context of addressing the estimation bias of
immigrants’ assimilation induced by return migration.

16

𝜕Δ𝐼𝑟 and 𝜕Pr(𝑑𝑖𝑟1 = 0|𝐼𝑟0 , 𝑑𝑖𝑟0 = 1) /𝜕Δ𝐼𝑟 ). For a given level of selection, larger crowdingout, displacement, or relocation effects (see equation (3)) amplify these compositional changes.
...
16 and older than 65 from our sample. Our analysis of employment effects is based on fulland part-time workers, with part-time work (below 30 hours per week) down-weighted into
full-time equivalent units. Without information on hours worked, our wage analysis is based

17

In 2001, 77.2 percent of all workers in the German economy were covered by social security and are hence
...
percent vs 70.9 percent, see Appendix Table A.3. Czech commuters are also relatively lowskilled (as reported by their German employers), with 50.4 percent having no further postsecondary degree, compared to 25.4 percent of native workers.
...
distance to the border (corresponding to the common time trend assumption in difference-indifferences settings). Moreover, distance to the border should affect local and individual labor
...
For the year 1991, we use the inflow of Czech workers between 1991 and 1990, scaled by 1990 total
employment, as the key regressor of interest.
19
Our estimated effects on both places and people contrast outcomes in municipalities that are more versus less
...
immigration (i.e., − 𝐸𝑟,𝑁 , term 1 of RHS of equation (3), where 𝐸𝑟0 is the stock of natives in
𝑟0
...
We can decompose this effect further, to show that about two-thirds (0.491/0.768) of the crowding-out effect is
...
(𝜂̅ 𝐸 and 𝜂̅ 𝑃 ) are large, resulting in only a small increase in total labor supply. From equations
(1a) and (1b), we can back out the population-weighted labor supply elasticity 𝜂̅ 𝑃 by dividing

21

In the individual wage growth regressions, we control for age to account for the possibility that immigration
induces older workers to leave employment and older workers experience lower wage growth than younger
workers. That is, we add the term 𝛿1 𝑎𝑔𝑒𝑖𝑡 + 𝛿2 𝑎𝑔𝑒2𝑖𝑡 to the wage level regression in (4).
22
While the point coefficient is slightly larger for 1995 than 1993, this difference is not statistically significant.
...
the regional employment (𝛽 𝑅 in equation (2)) by the pure wage effect (𝛾 𝑊 ; 𝜂̅ 𝑃 = 𝛽 𝑅 ⁄𝛾 𝑊 ). We
obtain an estimate of 𝜂̅ 𝑃 = 4.64 (-0.873/(-0.188)).
Using this and the ratio between the regional and pure wage effects, we can pin down the
𝛾𝑅

efficiency-weighted labor supply elasticity 𝜂̅ 𝐸 (i.e., 𝜂̅ 𝐸 = 𝛾𝑊 − 1 + 𝜂̅ 𝑃 , see equation (1c)). We
obtain an estimate of 𝜂̅ 𝐸 = 3.68 (− 0.008⁄(−0.188) − 1 + 4.64). The difference between the
...
𝛾 𝑊 ⁄(𝑐 + 𝜂̅𝑃 𝛽 𝑅 ), where 𝑐 is the ratio between the immigration shock in efficiency units 𝑑𝐼 𝐸
and the immigration shock in headcounts 𝑑𝐼 𝑃 . To approximate 𝑐, we compare how much
...
While the immigration shock in our empirical analysis was defined as the (full-time equivalent) employment
92 −𝐶𝑧𝑒𝑐ℎ 90
𝑊𝑎𝑔𝑒𝐶𝑧𝑒𝑐ℎ 92 −𝑊𝑎𝑔𝑒𝐶𝑧𝑒𝑐ℎ 90
share of Czech workers, 𝐶𝑧𝑒𝑐ℎ𝑇𝑜𝑡𝑎𝑙
...
90
92
where 𝑊𝑎𝑔𝑒𝐶𝑧𝑒𝑐ℎ and 𝑊𝑎𝑔𝑒𝐶𝑧𝑒𝑐ℎ are the total wage bill of Czech workers in 1990 and 1992, and
...
estimated regional instead of the pure wage effect of immigration (𝛾̂ 𝑅 = −0.008 instead of
𝛾̂ 𝑊 = −0.188) would yield very different and implausible results: an inverse labor demand
elasticity of 𝜑 = −0.08 (implying a demand elasticity of 1/−0.08 = −12.5) and a labor
supply elasticity of 𝜂̅ 𝑃 = 109.
...
Similarly, D’Amuri and Peri (2014) find that new hires are more positively selected after an immigration shock.
The composition and pure wage effects do not fully sum to the regional effect, as we control for age in the wage
...
When estimating the pure wage effect of immigration, 𝛾 𝑊 , our “base population” consists of
...
2
variance 𝜎Δ𝑒
, estimating equation (4) in differences for natives who are employed in the same
...
where 𝜌 is the correlation coefficient between 𝑢𝑖𝑟 and Δ𝑒𝑖𝑟 and 𝜆(𝜋𝑟 ) = Φ(𝜋𝑟 ) is the inverse
𝑟
...
|𝜋=0.377 = −0.547. 27 Moreover, the marginal effect of immigration Δ𝐼𝑟 on 𝜋 is 𝜕Δ𝐼 =
𝑟
...
pure wage effect of −0.188 (column (2) of Table 2) and the bias due to selection on timeconstant unobserved individual characteristics of 0.107 (column (3) of Table 2).
...
As employment in period 1 implies employment in period 0 (before the migration shock), we only need to
consider selection out of employment in period 1, as in a panel attrition model.
27
We estimate 𝜋 using a Probit model and obtain 𝑎̂ = 0.381 and 𝑏̂ = −0.271, implying a latent index of 0.377
...
Exceptions include Boustan, Fishback and Kantor (2010), who show that in-migration during the Great
...
29
We define these as workers who have no employment spell in 1990, but at least one employment spell between
...
hence 𝐸𝑁(𝑟)0 = 𝐸𝑁(𝑟),𝑟 + 𝐸𝑁(𝑟),𝑟̅ + 𝐸𝑁(𝑟),𝑁 (recall that the subscripts indicate the labor market
...
natives who find jobs in period 1, 𝑁(𝑟),𝑟
𝐸
...
that earlier period and period 0 (see Appendix B.3). Denoting this wage by 𝑙𝑜𝑔 𝑤
̃ 𝑖𝑟0 , we then
estimate equation (4) in differences at the individual level using 𝑙𝑜𝑔 𝑤𝑖𝑟1 − 𝑙𝑜𝑔 𝑤
̃ 𝑖𝑟0 as LHS
...
We control for age in the individual first difference regression, thus allowing for the immigration-induced
...
to routine occupations. The term 𝑟,𝑟
...
See footnote 25 in Peri and Sparber (2009), which clarifies that their main outcome variables are the
...
Moreover, access to foreign commuters might affect firms’ willingness to provide training (Oswald-Egg and
Siegenthaler 2023).
...
(4)
(5)
Outflows to NonOutflows to Other Upgrades from
Inflows
Employment
Regions
Routine to
...
We consider only West German districts of similar urban density, and we do not match preexisting time trends. We potentially match multiple control districts for each treated district to
...
Because employers are required to update records only at the end of each year, this variable may also capture
...
employment. Our analysis of the employment effects is thus based on regular full- and parttime workers, with part-time work (i.e., less than 30 hours per week) down-weighted into fulltime equivalent units by 0.67 (18-30 hours) or 0.5 (less than 18 hours). Our wage analysis is
...
log wage as:
𝑙𝑜𝑔 𝑤
̃ 𝑖𝑟0 = 𝑙𝑜𝑔 𝑤𝑖𝑟𝑡 + (𝑙𝑜𝑔 𝑤
̅ 0 − 𝑙𝑜𝑔 𝑤
̅ 𝑡 ),
2

where 𝑙𝑜𝑔 𝑤𝑖𝑟𝑡 is the logarithm of worker i’s wage in region r in year 𝑡 (𝑡 < 1990). 𝑙𝑜𝑔 𝑤
̅0
and 𝑙𝑜𝑔 𝑤
̅ 𝑡 represent the mean log wages of all full-time employees in 1990 and in year 𝑡,
respectively. This adjustment assumes that, had she remained employed, her wage would have
grown at the average rate earned by full-time employees. Thus, the adjusted wage 𝑙𝑜𝑔 𝑤
̃ 𝑖𝑟0
...
Assume that output Y is produced by combining capital K and labor L according to a CobbDouglas production function,
𝑌 = A𝐿1−α 𝐾 𝛼 .
Capital is supplied to the labor market according to 𝑟 = 𝐾 𝜆 , where 𝑟 denotes the price of
...
𝑙𝑜𝑔𝑤 = 𝑙𝑜𝑔[(1 − 𝛼)A] + 𝛼[𝑙𝑜𝑔𝐾 − 𝑙𝑜𝑔𝐿]

(C1.2)

𝑙𝑜𝑔𝑟 = 𝑙𝑜𝑔[𝛼 A] + (𝛼 − 1)[𝑙𝑜𝑔𝐾 − 𝑙𝑜𝑔𝐿].
...
immigrants and natives are perfect substitutes in production. Let 𝑑𝐼 𝐸 denote the immigration
∑ 𝜃 𝐼

shock in efficiency units (i.e., 𝑑𝐼 𝐸 = ∑ 𝑘 𝜃 𝑘𝐸𝑘 where 𝐼𝑘 denotes the number of incoming
𝑘 𝑘 𝑘
...
𝑑𝑙𝑜𝑔𝐿𝑆 = 𝑑𝐼 𝐸 + 𝑑𝑙𝑜𝑔 𝐿𝑁 ,
where 𝐿𝑁 = ∑𝑘 𝜃𝑘 𝐸𝑘 denotes the labor supply of incumbent native workers in efficiency units.
...
𝑑𝑙𝑜𝑔𝐸𝑘 ⁄𝑑𝑙𝑜𝑔𝑤 ). It then follows that 𝑑𝑙𝑜𝑔𝐿𝑁 = 𝜂̅ 𝐸 𝑑𝑙𝑜𝑔 𝑤 where 𝜂̅ 𝐸 = ∑𝑘 𝜃𝑘 𝐸𝑘 𝑘 is the
𝑘 𝑘 𝑘
...
and the immigration shock 𝑑𝐼 𝐸 (in efficiency units):
𝜑

(C2.1𝑎)

𝑑𝑙𝑜𝑔𝑤 = 1−𝜑𝜂̅𝐸 𝑑𝐼 𝐸 and
...
where 𝜂̅ 𝑃 = ∑𝑘 𝐸𝑘 𝑘 is the population-weighted elasticity of labor supply.
𝑘 𝑘

In the empirical analysis, we measure the immigration shock in head counts, and not in
∑ 𝐼

efficiency units. Let 𝑑𝐼 𝑃 denote the immigration shock in head counts (i.e., 𝑑𝐼 𝑃 = ∑ 𝑘𝐸𝑘 ). In
𝑘 𝑘

the empirical analysis, we therefore recover 𝑑𝑙𝑜𝑔𝑤/𝑑𝐼 𝑃 and 𝑑𝑙𝑜𝑔𝐸 / 𝑑𝐼 𝑃 . Denoting 𝑐 =
𝑑𝐼 𝐸 /𝑑𝐼 𝑃 —that is, the ratio between the immigration shock in head counts and efficiency
...
Totally differentiating 𝑙𝑜𝑔𝑤
̅ 𝑅 , we therefore have
(C3.1)

∑ 𝜃 𝐸

𝑑𝑙𝑜𝑔𝑤
̅ 𝑅 = 𝑑𝑙𝑜𝑔𝑤 + 𝑑𝑙𝑜𝑔 ∑𝑘 𝐸𝑘 𝑘 .
𝑘 𝑘
...
As 𝑑𝑙𝑜𝑔𝐸𝑘 = 𝜂̅ 𝑃 𝑑𝑙𝑜𝑔 𝑤, we can rewrite equation (C3.1) as
(C3.2)
...
= 𝑑𝑙𝑜𝑔𝑤 + ∑𝑘 𝜃𝑘 𝐸𝑘 𝑘 𝑑𝑙𝑜𝑔 𝑤 − ∑𝑘 𝐸𝑘 𝑘 𝑑𝑙𝑜𝑔 𝑤.
...
𝜕 Pr(𝑑𝑖(𝑘)𝑟1 = 1|𝑑𝑖(𝑘)𝑟0 = 0) Pr(𝑑𝑖(𝑘)𝑟0 = 0)
...
wage change may affect local labor supply also via its effects on inflows from nonemployment or other regions (crowding-out elasticity) or its effects on outflows to
employment in other regions (relocation elasticity). Similarly, the aggregate labor supply
elasticity can be thought of as a weighted average of the type-specific displacement, crowdingout and relocation elasticities, where either population or efficiency shares are used as weights
to compute the average.

8

Appendix C.5: Selection Based on Observables
Consider the following augmented individual wage regression where we split up the individualspecific and time-constant wage component 𝜃𝑖 as contained in 𝑣𝑖𝑟𝑡 in equation (4) into an
observable and unobservable component:
log 𝑤𝑖𝑟𝑡 = 𝑐𝑡 + 𝑓𝑟 + 𝛾 𝑊 𝐼𝑟𝑡 + ∑ 𝛿𝑘 𝐷𝑖𝑘 + 𝜃𝑖′ + 𝑒𝑖𝑟𝑡 ,
⏟𝑘
𝜃𝑖
...
This pseudo-panel regression identifies the pure wage effect of immigration 𝛾 𝑤 under the
...
immigration does not induce compositional changes of unobserved wage components 𝜃𝑖′ , such
̅ ′ |Δ𝐼𝑟 ] = 0.
that 𝐸[Δ𝜃𝑘𝑟
...
The results are similar if we account for education, age, and gender in individual-level rather than region-group
