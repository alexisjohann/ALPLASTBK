...
%          v3.2: Added 10 TODOs (TODO-UNMAPPED_IE-1 to TODO-UNMAPPED_IE-10) for Chapter 17-20 support
...
Appendix TKT (METHOD-TOOLKIT) provides the \textit{operational tools}.
...
% - Appendix UNMAPPED_AAA (CORE-WHO): Welfare hierarchy for target level
% - Appendix UNMAPPED_C (CORE-WHAT): FEPSDE dimensions for utility targeting
% - Appendix UNMAPPED_B (CORE-HOW): Complementarity for γ-structure
% - Appendix UNMAPPED_V (CORE-WHEN): Context framework for Ψ-dependence
% - Appendix UNMAPPED_BBB (CORE-WHERE): Parameter sources
% - Appendix UNMAPPED_AU (CORE-AWARE): Awareness function
% - Appendix UNMAPPED_AV (CORE-READY): Willingness function
% - Appendix UNMAPPED_AW (CORE-STAGE): Journey phase definitions
% - Appendix UNMAPPED_HI (CORE-HIERARCHY): Decision hierarchy
%
% DEPENDENTS (what REQUIRES this appendix):
% - Appendix TKT (METHOD-TOOLKIT): Operational implementation
...
\midrule
Wirken Interventionen zusammen? & Crowding $\gamma_{ij}$ (komplementär/substitutiv) & TODO-UNMAPPED_IE-3 \\
...
\item Appendix TKT: \textbf{Operations} --- 20-Field Schema, A1--A8, LLMMC protocols
\end{itemize}
...
The 10C CORE framework defines the \textit{space of behavior}. Appendix UNMAPPED_IE extends this by defining the \textit{space of interventions} that transform behavior:
...
\begin{itemize}[nosep]
    \item METHOD-TOOLKIT $\rightarrow$ Appendix TKT
...
\textbf{Type:} empirical (requires evidence: Transtheoretical Model \citep{prochaska1992stages}, intervention timing research)
...
\textbf{Level 3: Literature} ($C = 0.5$): Meta-analyses and published studies from BBB (Parameter Repository). Subject to publication bias and replication concerns \citep{camerer2016evaluating}.
...
\textbf{Level 5: Expert (Single)} ($C = 0.2$): Estimate from one behavioral economist or psychologist. Subject to anchoring on own research, overconfidence, and limited literature coverage \citep{dellavigna2016predict}.
...
\begin{itemize}[nosep]
    ...
    \item $\lambda = 2.25$: Loss aversion parameter is well-replicated \citep{tversky1992advances}
\end{itemize}
...
\midrule
\multicolumn{2}{@{}l@{}}{\textit{Coherence and Quality (Ch.7, Appendix UNMAPPED_NC)}} \\
...
\begin{itemize}[nosep]
    ...
    \item \textit{Evidence base:} Behavioral regularities are highly replicable \citep{camerer2016evaluating}
...
The discovery of \textbf{Nudging} \citep{sunstein2014nudge} provides a historical parallel for IE-CONJ-1:
...
\begin{itemize}[nosep]
\item \textbf{Non-Concavity and Fit:} See Appendix UNMAPPED_NC (FORMAL-NC) for $K$ vs.\ $Q$ distinction and why $\gamma > 0$ creates emergence
...
\item \textbf{LLMMC methodology:} See Appendix UNMAPPED_LLM (METHOD-LLMMC)
...
\item \textbf{Context-Specificity:} \citet{henrich2000cooperation} (WEIRD); \citet{camerer2016evaluating} (Replication)
\end{itemize}
...
\begin{itemize}[nosep]
\item \citet{sunstein2014nudge} --- Nudge theory and choice architecture
\item \citet{sunstein2014nudge} --- Libertarian paternalism
\item \citet{prochaska1992stages} --- Transtheoretical model (stages of change)
...
\begin{itemize}[nosep]
\item \citet{rogers2003diffusion} --- Diffusion of innovations (segment archetypes)
\item \citet{fehr1999theory} --- Theory of fairness (heterogeneous preferences)
\item \citet{gaechter2010socialpreferences} --- Heterogeneity in social preferences
\item \citet{camerer2003behavioralbehavioral} --- Behavioral game theory (types)
\end{itemize}

\textbf{Judgment and Decision Making:}
\begin{itemize}[nosep]
\item \citet{kahneman2011thinking} --- Thinking, Fast and Slow
\item \citet{tversky1974judgment} --- Judgment under uncertainty
\item \citet{kahneman1979prospectprospect} --- Prospect theory
\item \citet{thaler1985mental} --- Mental accounting
...
\item \citet{goldstein2008yesscientificallyroom} --- Descriptive social norms
...
\item \citet{bordalo2012salience} --- Salience theory of choice
\item \citet{gabaix2019behavioral} --- Behavioral inattention
\end{itemize}
...
\begin{itemize}[nosep]
    \item[$z$] \textbf{$I_{\text{WHO},o}$+$I_{\text{WHAT},F}$ ($\gamma = -0.3$):} Social norms + Financial incentives. \citet{frey1997cost}: ``Motivation Crowding Theory''; \citet{gneezy2000fine}: ``A Fine is a Price''; \citet{fehr2007motivation}: ``Intrinsic Motivation and Extrinsic Incentives''. \textit{Mechanism:} Financial incentives signal distrust, undermining social motivation.

    \item[$ab$] \textbf{$I_{\text{WHAT},F}$+$I_{\text{HOW}}$ ($\gamma = -0.3$):} Financial + Commitment. \citet{deci1971effects}: ``Effects of externally mediated rewards''; \citet{deci1999meta}: Meta-analysis of 128 studies. \textit{Mechanism:} External rewards undermine intrinsic commitment.

    \item[$x$] \textbf{$I_{\text{WHO}}$+$I_{\text{WHAT},F}$ ($\gamma = -0.2$):} Identity + Financial. \citet{benabou2003intrinsic}: ``Intrinsic and Extrinsic Motivation''; \citet{ariely2009predictably}. \textit{Mechanism:} Payments crowd out identity-based motivation.
...
\begin{itemize}[nosep]
    ...
    \item[$w$] \textbf{$I_{\text{WHO}}$+$I_{\text{WHO},o}$ ($\gamma = +0.4$):} Identity + Social. \citet{akerlof2000identity}: ``Economics and Identity''. \textit{Mechanism:} Social norms reinforce identity.

    \item[$n$] \textbf{$I_{\text{WHEN}}$+$I_{\text{WHEN},t}$ ($\gamma = +0.3$):} Contextual + Temporal. \citet{sunstein2014nudge}; \citet{benartzi2007save}. \textit{Mechanism:} Defaults + timing optimize choice architecture.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}[nosep]
        \item \textit{Example:} Paying volunteers to donate blood reduces donations \citep{gneezy2000fine}
...
Given the context-dependent complementarity matrix $\Gamma(\Psi)$, we now specify the algorithm that computes optimal intervention profiles. This addresses TODO-UNMAPPED_IE-10 (Portfolio Algorithm).
...
\textbf{Sources:} \citet{michie2013behavior} (BCT Taxonomy), \citet{sunstein2014nudge} (Choice Architecture), \citet{deci2000what} (Motivation Theory)
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{TODO-UNMAPPED_IE-1: Phase-Type Affinity Matrix $\alpha_{\text{BCJ}}$}]
...
\begin{itemize}[nosep]
    \item \citet{prochaska1992stages} --- Stage-matched interventions
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{TODO-UNMAPPED_IE-2: Segment Multiplier Matrix $\beta_{\text{BCS}}$}]
...
\textbf{Key sources to integrate:}
\begin{itemize}[nosep]
    \item \citet{fehr1999theory} --- Heterogeneous social preferences
    \item \citet{gaechter2010socialpreferences} --- Conditional cooperators vs.\ free-riders
    \item \citet{rogers2003diffusion} --- Innovators, early adopters, laggards
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{TODO-UNMAPPED_IE-3: Intervention Complementarity Matrix $\gamma_{ij}$ --- COMPLETED}]
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!75!black,
    title=\textbf{TODO-UNMAPPED_IE-4: Journey Emergence Function $h(\vec{S}_0)$}]
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!75!black,
    title=\textbf{TODO-UNMAPPED_IE-5: Effect-Horizon Function $g(\vec{I})$}]
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!75!black,
    title=\textbf{TODO-UNMAPPED_IE-6: Coherence Score Definition $C(P)$}]
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{TODO-UNMAPPED_IE-7: Reactance Model $\rho(\mathcal{I}, s)$}]
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{TODO-UNMAPPED_IE-8: Co-Evolution Dynamics $\frac{d\vec{S}}{dt}, \frac{d\vec{I}}{dt}$}]
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{TODO-UNMAPPED_IE-9: Cluster Emergence Theorem (K=8)}]
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=\textbf{TODO-UNMAPPED_IE-10: Portfolio Optimization Algorithm}]
...
\midrule
TODO-UNMAPPED_IE-1 & $\alpha_{\text{BCJ}}$ Matrix & $\alpha_{\text{emp}}$ & Ch. 18 & P1 \\
TODO-UNMAPPED_IE-2 & $\beta_{\text{BCS}}$ Matrix & $\alpha_{\text{emp}}$ & Ch. 19 & P1 \\
TODO-UNMAPPED_IE-3 & $\gamma_{ij}$ Crowding Matrix & $\alpha_{\text{emp}}$ & Ch. 20 & \checkmark DONE \\
TODO-UNMAPPED_IE-4 & $h(\vec{S}_0)$ Function & $\alpha_{\text{theo}}$ & Ch. 18 & P2 \\
TODO-UNMAPPED_IE-5 & $g(\vec{I})$ Function & $\alpha_{\text{theo}}$ & Ch. 18 & P2 \\
TODO-UNMAPPED_IE-6 & $C(P)$ Coherence Score & $\delta$ & Ch. 20 & P2 \\
TODO-UNMAPPED_IE-7 & Reactance Model $\rho$ & $\pi$ & Ch. 19 & P3 \\
TODO-UNMAPPED_IE-8 & Co-Evolution DGL & $\tau$ & Ch. 18 & P3 \\
TODO-UNMAPPED_IE-9 & $K=8$ Emergence & $\tau$ or $\alpha_{\text{emp}}$ & Ch. 20 & P3 \\
TODO-UNMAPPED_IE-10 & Portfolio Algorithm (EMERGE) & $\eta$ & Ch. 20 & \checkmark DONE \\
\bottomrule
\end{tabular}
\end{center}
