...
\textbf{Dependencies:} & Appendix UNMAPPED_HOW (Complementarity Theory) \\
 & Appendix UNMAPPED_WAT (FEPSDE Utility Structure) \\
 & Appendix UNMAPPED_EST (Parameter Calibration) \\
...
\begin{itemize}[nosep]
    \item Complementarity Theory $\rightarrow$ Appendix UNMAPPED_HOW
    ...
    \item \S\ref{sec:bbb-delta} $\rightarrow$ Appendix UNMAPPED_EST
\end{itemize}
...
Using \citet{enke2019globalpref} Global Preference Survey data, we test whether patience produces different growth effects depending on context.
...
\paragraph{Treatment Effect Source.}
Patience (time preference) measures from \citet{enke2019globalpref}: ``Global Evidence on Economic Preferences,'' \textit{Quarterly Journal of Economics}, 133(4), 1645--1692. Growth effects estimated by regressing GDP growth on patience controlling for standard covariates, then extracting country-specific coefficients.
...
Using the $\delta$-coefficients from Appendix UNMAPPED_EST, we calculate context-adjusted 
complementarities.
...
\textbf{Reference:} See Appendix UNMAPPED_EST (\S\ref{sec:bbb-delta}) for the complete 
...
\subsubsection{Connection to Appendix UNMAPPED_WHO (WHO)}
...
at the \textit{appropriate aggregation level}. See Appendix UNMAPPED_WHO for the complete 
...
$\Lambda = [\lambda_{kl}]$ matrix (see Appendix UNMAPPED_HOW, Teil IV):
...
\textbf{Key findings from Appendix UNMAPPED_HOW}:
\begin{itemize}
...
where:
\begin{itemize}
    \item $L$ = Aggregation level (from Appendix UNMAPPED_WHO)
    \item $d \in \{F, E, P, S, D, E\}$ = FEPSDE dimensions (from Appendix UNMAPPED_WAT)
    \item $\gamma_{dd'}(\Psi)$ = Context-dependent complementarity (from Appendix UNMAPPED_HOW)
...
\hrule
\vspace{0.5em}
\noindent\textit{Cross-references: Appendix UNMAPPED_WHO (Aggregation Levels), 
Appendix UNMAPPED_HOW (Complementarity Structure), Appendix UNMAPPED_EST (Parameter Calibration),
Appendix UNMAPPED_WAT (FEPSDE Matrix), Appendix UNMAPPED_OPS (Operationalization), 
Appendix UNMAPPED_SUN (Falsifiable Predictions), Appendix UNMAPPED_EPS (Epistemics of Deviation)}
...
\begin{enumerate}
    ...
    complementarity (Appendix UNMAPPED_HOW). This enables the full EBF model to adjust 
...
\textbf{Cross-Reference:} See Appendix UNMAPPED_MSC Section 11 for the full treatment of
