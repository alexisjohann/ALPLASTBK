...
% - Appendix UNMAPPED_REG (METHOD-REGISTRY): Model archive for option selection
% - Appendix UNMAPPED_EST (CORE-WHERE): Parameter sources
% - Appendix CTW (CORE-WHEN): Context specification
% - Appendix UNMAPPED_WAT (CORE-WHAT): Utility dimensions
% - Appendix UNMAPPED_HOW (CORE-HOW): Complementarity specification
%
...
% - Appendix UNMAPPED_REG (METHOD-REGISTRY): Stores models created via this workflow
%
% =============================================================================
...
Registry (Appendix UNMAPPED_REG) via similarity search (Axiom MD-2).
\end{abstract}
...
\begin{itemize}[nosep]
    \item METHOD-REGISTRY $\rightarrow$ Appendix UNMAPPED_REG
...
through similarity search in the Model Registry (Appendix UNMAPPED_REG):
\begin{equation}
...
\textbf{Implication:}
\begin{itemize}[nosep]
\item The Registry must exist and be maintained (see Appendix UNMAPPED_REG)
...
\begin{tikzpicture}[node distance=1.8cm, auto,
    ...
    \node[block, fill=gray!20, below=of predict] (register) {8. Register Model\\(Appendix UNMAPPED_REG)};
...
\textbf{Dependencies:}
\begin{itemize}[nosep]
\item Appendix UNMAPPED_WAT (CORE-WHAT): Defines available $C$ dimensions
\item Appendix UNMAPPED_HOW (CORE-HOW): Defines available $\gamma$ parameters
\end{itemize}
\end{tcolorbox}
...
\textbf{Dependencies:}
\begin{itemize}[nosep]
\item Appendix UNMAPPED_BBB (CORE-WHERE): Parameter sources and defaults
\item Appendix UNMAPPED_CAL (METHOD-LLMMC): LLM Monte Carlo estimation
\end{itemize}
\end{tcolorbox}
...
\textbf{See:} Appendix UNMAPPED_REG (METHOD-REGISTRY) for complete schema
\end{tcolorbox}
...
\begin{itemize}
\item Model ID: UNMAPPED_SHL-2026-001
\item Designer: [Name]
...
The LLM must reference the mapping tables from Appendix UNMAPPED_CFG to auto-populate defaults:
...
\begin{verbatim}
→ Lookup Table UNMAPPED_CFG-1 (Ψ defaults):
...
\begin{verbatim}
→ Lookup Table UNMAPPED_CFG-2 (C weights):
...
\begin{verbatim}
→ Lookup Table UNMAPPED_CFG-3 (γ interactions):
...
\begin{verbatim}
→ Lookup Table UNMAPPED_CFG-4 (Functional Form):
  Decision: Logistic
  Continuous: Linear/CES
  Process: Dynamic/BCJ
\end{verbatim}
...
\item \textbf{SEED-UNMAPPED_P06-02:} Steuer Compliance (Österreich) --- 45\% Ähnlichkeit
...
\begin{tcolorbox}[colback=green!10!white, colframe=green!75!black,
    title=\textbf{Appendix UNMAPPED_DSN: Summary}]
...
\begin{table}[htbp]
\centering
\caption{Symbols Introduced in Appendix UNMAPPED_DSN}
...
Appendix UNMAPPED_CFG (METHOD-CONFIG),
