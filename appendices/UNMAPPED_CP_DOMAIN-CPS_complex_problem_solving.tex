...
% - Appendix UNMAPPED_AU (CORE-AWARE): Awareness function A(t*)
% - Appendix UNMAPPED_V (CORE-WHEN): Context dimensions Ψ
% - Appendix UNMAPPED_B (CORE-HOW): Complementarity γ
% - Appendix UNMAPPED_HI (CORE-HIERARCHY): Decision levels L0-L3
% - Appendix UNMAPPED_C (CORE-WHAT): Utility dimensions (U_D)
% - Appendix UNMAPPED_AV (CORE-READY): Willingness WAX, threshold θ
%
% DEPENDENTS (what REQUIRES this appendix):
% - Domain applications in Education, Management, Training
% - Appendix TKT (METHOD-TOOLKIT): Intervention design for CPS contexts
%
% =============================================================================
...
\item For context dimensions: See Appendix UNMAPPED_V (CORE-WHEN)
\item For formal Awareness function: See Appendix UNMAPPED_AU
\end{itemize}
...
\textbf{Out-of-Scope:}
\begin{itemize}[nosep]
\item Detailed psychometric properties of MicroDYN (see \cite{greiff2013cps})
...
\begin{enumerate}
\item \textbf{Lohhausen Study (1983)}: Participants acted as mayor of a simulated town with 2000+ interconnected variables over 10 simulated years \citep{doerner1983lohhausen}.

\item \textbf{Logic of Failure (1989)}: Dörner identified systematic errors in complex situations: failure to consider side effects, tendency toward ``repair service behavior,'' and difficulty with exponential dynamics \citep{doerner1989logik}.

\item \textbf{MicroDYN Approach (2009-2017)}: Funke and Greiff developed minimal complex systems for standardized assessment, using linear equations with 3 input and 3 output variables \citep{greiff2017microdynamapproach}.

\item \textbf{PISA Integration (2012-2015)}: CPS became part of international large-scale assessments, with PISA 2015 assessing collaborative problem solving in 52 countries \citep{oecd2017pisa}.
\end{enumerate}
...
Following Cognitive Load Theory \citep{sweller1988cognitiveload}, awareness under load is:
...
\end{equation}
This functional form is justified by working memory capacity constraints \citep{sweller1988cognitiveload}.
\end{tcolorbox}
...
\item \textbf{Cognitive Load as Binding Constraint:} Working memory capacity limits awareness, making $A(\Psi_{\text{load}})$ the primary bottleneck in complex problem solving \citep{sweller1988cognitiveload}.
...
\begin{itemize}[nosep]
\item \cite{doerner1989logik} -- Foundational CPS theory
\item \cite{doerner1983lohhausen} -- Original Lohhausen study
\item \cite{funke2017cps} -- CPS definition and review
\item \cite{greiff2013cps} -- MicroDYN validation (N=855)
\item \cite{greiff2012microdynamvalidity} -- Incremental validity
\item \cite{greiff2017microdynamapproach} -- MicroDYN methodology
\item \cite{oecd2017problemsolving} -- OECD problem solving report
\item \cite{oecd2017pisa} -- PISA 2015 CPS assessment
\item \cite{sweller1988cognitiveload} -- Cognitive Load Theory
\end{itemize}
