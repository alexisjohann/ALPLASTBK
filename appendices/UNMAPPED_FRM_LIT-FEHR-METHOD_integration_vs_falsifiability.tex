...
\vspace{0.5em}
This is not a question about measurement or identification (Appendix UNMAPPED_CRT), but about the \textit{scientific status} of integrative frameworks themselves.
\end{tcolorbox}
...
\begin{itemize}[nosep]
    \item PREDICT-FALSIFIABLE $\rightarrow$ Appendix UNMAPPED_SUN
    \item WHO $\rightarrow$ Appendix UNMAPPED_WHO
    \item HOW $\rightarrow$ Appendix UNMAPPED_HOW
    \item WHAT $\rightarrow$ Appendix UNMAPPED_WAT
    \item WHEN $\rightarrow$ Appendix CTW
\end{itemize}

\textbf{Constraints (Anwendungsgrenzen):}
\begin{itemize}[nosep]
    \item \textbf{Does NOT apply:} Pure measurement questions (use Appendix UNMAPPED_LLM), single-factor models (no aggregation needed), non-behavioral domains (physics, chemistry)
...
Ernst Fehr's published work (Appendix UNMAPPED_FEH) demonstrates rigorous experimental methodology, but rarely engages in explicit philosophy of science. However, his methodological commitments imply a coherent epistemological position that we can reconstruct.
...
The standard critique (Appendix UNMAPPED_CRT, Argument 6) concerns:
\begin{itemize}[noitemsep]
...
\subsection{Connection to Appendix UNMAPPED_SUN}

The ten predictions in Appendix UNMAPPED_SUN (PREDICT-FALSIFIABLE) exemplify this approach:
...
Appendix UNMAPPED_SUN should be understood as the \textit{application} of the exclusion principle: it specifies what EBF predicts will \textit{not} happen (e.g., UCT effects will \textit{not} be equal across social capital levels).

\subsection{Connection to Appendix UNMAPPED_CRT}

The ten canonical arguments (Appendix UNMAPPED_CRT) concern \textit{technical} challenges:
\begin{itemize}[noitemsep]
...
\begin{itemize}[noitemsep]
    \item $\gamma$ claims generate novel predictions (Appendix UNMAPPED_SUN)
    \item Predictions are tested and some confirmed
    \item Failed predictions are documented (Appendix UNMAPPED_EPS)
\end{itemize}
...
Key terms (see also Appendix UNMAPPED_GLS):
...
\textbf{Response:} Correct. The response (exclusion principle) also applies generally. EBF is distinctive in making this explicit (Appendix UNMAPPED_SUN, this appendix).
...
\begin{enumerate}[noitemsep]
    \item \textbf{\citet{fehr1999theory}} --- Fehr-Schmidt inequity aversion model specifies \textit{exact} functional form with testable parameters $\alpha, \beta$. Does NOT claim universal applicability.

    \item \textbf{\citet{fehr2022normative}} --- Emphasizes that normative foundations must be \textit{empirically grounded}, not assumed.
...
We attach a 5-page summary of Appendix UNMAPPED_FRM (Integration vs. Falsifiability).
...
Appendix UNMAPPED_STA (STAGE).

\noindent\textit{Cross-references (FORMAL):}
Appendix UNMAPPED_DER (AXIOMS),
Appendix UNMAPPED_PRF (CSTAR).

\noindent\textit{Cross-references (LIT/PREDICT):}
Appendix UNMAPPED_GLS (Glossary),
Appendix UNMAPPED_FEH (LIT-FEHR),
Appendix UNMAPPED_CRT (LIT-CRITIQUE),
Appendix UNMAPPED_SUN (PREDICT-FALSIFIABLE),
Appendix UNMAPPED_CAM (Metatheory).
