...
% - Appendix UNMAPPED_C (CORE-WHAT): Needs WHO to specify whose utility dimensions
% - Appendix UNMAPPED_B (CORE-HOW): Needs WHO for inter-level complementarity
% - Appendix UNMAPPED_V (CORE-WHEN): Needs WHO for context-sensitive aggregation
% - Appendix UNMAPPED_AU (CORE-AWARE): Awareness exists at each level L
% - Appendix UNMAPPED_AV (CORE-READY): Willingness exists at each level L
% - Appendix UNMAPPED_AW (CORE-STAGE): Journey stages apply per level
% - Appendix UNMAPPED_HI (CORE-HIERARCHY): Decision hierarchy presupposes welfare hierarchy
% - Appendix UNMAPPED_IE (CORE-EIT): Interventions target specific levels
% - Appendix UNMAPPED_CAL (METHOD-LLMMC): Calibration methods for level-specific parameters
%
% =============================================================================
...
\item Appendix UNMAPPED_C (CORE-WHAT): \textbf{WHAT} --- Defines the dimensions of utility at each level
\item Appendix UNMAPPED_B (CORE-HOW): \textbf{HOW} --- Defines how utilities interact across levels
\item Appendix UNMAPPED_V (CORE-WHEN): \textbf{WHEN} --- Defines when context modifies level weights
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item Aggregation axioms (UNMAPPED_SRL-1 through UNMAPPED_SRL-8)
...
\begin{itemize}[nosep]
    \item Utility dimension content $\rightarrow$ Appendix UNMAPPED_C (CORE-WHAT)
    \item Inter-level complementarity mechanics $\rightarrow$ Appendix UNMAPPED_B (CORE-HOW)
    \item Context-dependent weighting $\rightarrow$ Appendix UNMAPPED_V (CORE-WHEN)
    \item Parameter estimation for $\omega_L$ $\rightarrow$ Appendix UNMAPPED_BBB (CORE-WHERE)
\end{itemize}

\textbf{Constraints:}
\begin{itemize}[nosep]
    \item Levels must be mutually exclusive and collectively exhaustive
    \item Aggregation must satisfy Pareto consistency (UNMAPPED_SRL-6)
    \item No utility without a bearer (UNMAPPED_SRL-1)
\end{itemize}

\textbf{Lieferobjekte (Deliverables):}
\begin{itemize}[nosep]
    \item Complete Level Taxonomy (Table~\ref{tab:who-levels})
    \item Axiom System UNMAPPED_SRL-1 through UNMAPPED_SRL-8
...
The axiom system UNMAPPED_SRL-1 through UNMAPPED_SRL-8 formalizes the conditions for well-defined
...
\begin{enumerate}
\item \textbf{Nesting}: Each level $L > 0$ contains members from level $ACE-1$
...
\begin{equation}
U_e^{(L)} = f_L\left( \{U_m^{(ACE-1)}\}_{m \in e}, \; \Xi_L \right)
\end{equation}

where:
\begin{itemize}
\item $\{U_m^{(ACE-1)}\}_{m \in e}$ are utilities of constituent members
...
% UNMAPPED_SRL-1
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom UNMAPPED_SRL-1: Existence of Utility Bearer]
\textbf{Statement:}
\begin{equation}
...
% UNMAPPED_SRL-2
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom UNMAPPED_SRL-2: Level Boundedness]
\textbf{Statement:}
\begin{equation}
...
% UNMAPPED_SRL-3
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom UNMAPPED_SRL-3: Non-Trivial Emergence]
\textbf{Statement:}
\begin{equation}
\exists L > 0: U_e^{(L)} \neq g\left(\{U_m^{(ACE-1)}\}_{m \in e}\right) \quad \forall g \in \mathcal{G}_{\text{additive}}
\end{equation}
...
% UNMAPPED_SRL-4
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom UNMAPPED_SRL-4: Downward Causation]
\textbf{Statement:}
\begin{equation}
...
% UNMAPPED_SRL-5
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom UNMAPPED_SRL-5: Level-Specific Context]
\textbf{Statement:}
\begin{equation}
...
% UNMAPPED_SRL-6
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom UNMAPPED_SRL-6: Pareto Consistency]
\textbf{Statement:}
\begin{equation}
...
% UNMAPPED_SRL-7
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom UNMAPPED_SRL-7: No Double-Counting]
\textbf{Statement:}
\begin{equation}
...
% UNMAPPED_SRL-8
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom UNMAPPED_SRL-8: Weight Normalization]
\textbf{Statement:}
\begin{equation}
...
Combining axioms UNMAPPED_SRL-6, UNMAPPED_SRL-7, and UNMAPPED_SRL-8, the total welfare function is:
...
See Appendix UNMAPPED_BBB (CORE-WHERE) for calibration methods.
...
\subsection{Connection to CORE-WHAT (Appendix UNMAPPED_C)}
...
\subsection{Connection to CORE-HOW (Appendix UNMAPPED_B)}
...
\subsection{Connection to CORE-WHEN (Appendix UNMAPPED_V)}
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Appendix UNMAPPED_AAA Summary: CORE-WHO]
...
\item Higher-level utilities are not mere aggregations (UNMAPPED_SRL-3: emergence)
\item Cross-level externalities require explicit modeling (UNMAPPED_SRL-4: downward causation)
\item Welfare weights $\omega_L$ are normative choices that should be transparent
\end{itemize}

\textbf{Next Steps:}
See Appendix UNMAPPED_C (CORE-WHAT) for the content of utility at each level.
See Appendix UNMAPPED_B (CORE-HOW) for complementarity across levels.
...
\begin{table}[htbp]
\centering
\caption{Symbols Introduced in Appendix UNMAPPED_AAA}
...
\textbf{Response:}
This is why Axiom UNMAPPED_SRL-7 introduces the inheritance coefficient $\pi_{L \leftarrow L'}$.
...
See Appendix UNMAPPED_C (CORE-WHAT) for dimensionless utility normalization.
