...
\begin{itemize}[nosep]
\item Appendix UNMAPPED_WHO (CORE-WHO): Defines levels $L$, uses LET implicitly
\item Appendix UNMAPPED_HOW (CORE-HOW): Defines $\gamma$, provides mathematical basis
\item Appendix UNMAPPED_WAT (CORE-WHAT): FEPSDE dimensions aggregated via LET
\item Appendix UNMAPPED_HIE (CORE-HIERARCHY): Decision hierarchy emerges via LET
\end{itemize}
...
the sum of utilities at level $UNMAPPED_ACE-1$. This ``emergence premium'' $\mathcal{E}$
...
\textbf{Core Question:} When and why does $U^L > \sum_i U^{UNMAPPED_ACE-1}_i$?

\textbf{The Emergence Equation:}
\begin{equation}
U^L = \sum_{i=1}^{n} \omega_i \cdot U^{UNMAPPED_ACE-1}_i + \underbrace{\sum_{i<j} \gamma_{ij} \cdot U^{UNMAPPED_ACE-1}_i \cdot U^{UNMAPPED_ACE-1}_j}_{\text{Emergence Term } \mathcal{E}}
\label{eq:let-aggregation}
\end{equation}

\textbf{Main Result (LET):}
\begin{equation}
\boxed{U^L > \sum_i U^{UNMAPPED_ACE-1}_i \quad \Longleftrightarrow \quad \bar{\gamma} > 0 \text{ and } \text{Var}(U^{UNMAPPED_ACE-1}) > 0}
\end{equation}

\textbf{Emergence Premium:}
\begin{equation}
\mathcal{E} = \frac{n(n-1)}{2} \cdot \bar{\gamma} \cdot \left[ \bar{U}^2 + \text{Var}(U^{UNMAPPED_ACE-1}) \right]
\end{equation}
...
(Appendix UNMAPPED_WHO, Appendix UNMAPPED_HOW), but has not \textit{proven} this formally.

\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=The Gap We Fill]
\textbf{Claim (implicit in AAA):} $\gamma > 0 \Rightarrow U^L > \sum U^{UNMAPPED_ACE-1}$
...
\end{itemize}
Each level $L$ aggregates $n_L$ entities from level $UNMAPPED_ACE-1$.
\end{definition}
...
\begin{equation}
U^L = \sum_{i=1}^{n} \omega_i \cdot U^{UNMAPPED_ACE-1}_i + \sum_{i<j} \gamma_{ij} \cdot U^{UNMAPPED_ACE-1}_i \cdot U^{UNMAPPED_ACE-1}_j
\label{eq:level-utility}
\end{equation}
where:
\begin{itemize}[nosep]
\item $U^{UNMAPPED_ACE-1}_i$ = utility of member $i$ at level $UNMAPPED_ACE-1$
...
\begin{equation}
\mathcal{E} = U^L - \sum_{i=1}^{n} U^{UNMAPPED_ACE-1}_i
...
For members $\{U^{UNMAPPED_ACE-1}_1, \ldots, U^{UNMAPPED_ACE-1}_n\}$, define:
\begin{align}
\bar{U} &= \frac{1}{n} \sum_{i=1}^{n} U^{UNMAPPED_ACE-1}_i \quad \text{(mean utility)} \\
\text{Var}(U) &= \frac{1}{n} \sum_{i=1}^{n} (U^{UNMAPPED_ACE-1}_i - \bar{U})^2 \quad \text{(variance)}
\end{align}
\end{definition}
...
Then:
\begin{equation}
U^L > \sum_{i=1}^{n} U^{UNMAPPED_ACE-1}_i \quad \Longleftrightarrow \quad \sum_{i<j} \gamma_{ij} \cdot U^{UNMAPPED_ACE-1}_i \cdot U^{UNMAPPED_ACE-1}_j > 0
\label{eq:let-condition}
\end{equation}

Moreover, the emergence premium decomposes as:
\begin{equation}
\boxed{\mathcal{E} = \frac{n(n-1)}{2} \cdot \bar{\gamma} \cdot \bar{U}^2 + \sum_{i<j} \gamma_{ij} \cdot (U^{UNMAPPED_ACE-1}_i - \bar{U})(U^{UNMAPPED_ACE-1}_j - \bar{U})}
...
\begin{equation}
U^L = \sum_{i=1}^{n} U^{UNMAPPED_ACE-1}_i + \sum_{i<j} \gamma_{ij} \cdot U^{UNMAPPED_ACE-1}_i \cdot U^{UNMAPPED_ACE-1}_j
\end{equation}
...
\begin{equation}
\mathcal{E} = U^L - \sum_{i=1}^{n} U^{UNMAPPED_ACE-1}_i = \sum_{i<j} \gamma_{ij} \cdot U^{UNMAPPED_ACE-1}_i \cdot U^{UNMAPPED_ACE-1}_j
\end{equation}

Therefore $\mathcal{E} > 0 \Leftrightarrow \sum_{i<j} \gamma_{ij} \cdot U^{UNMAPPED_ACE-1}_i \cdot U^{UNMAPPED_ACE-1}_j > 0$. \checkmark

\textbf{Step 3: Decompose the emergence term.}

Write $U^{UNMAPPED_ACE-1}_i = \bar{U} + \delta_i$ where $\delta_i = U^{UNMAPPED_ACE-1}_i - \bar{U}$.

Note that $\sum_i \delta_i = 0$ by definition of the mean.

\begin{align}
U^{UNMAPPED_ACE-1}_i \cdot U^{UNMAPPED_ACE-1}_j &= (\bar{U} + \delta_i)(\bar{U} + \delta_j) \\
&= \bar{U}^2 + \bar{U}(\delta_i + \delta_j) + \delta_i \delta_j
\end{align}

Sum over all pairs:
\begin{align}
\sum_{i<j} \gamma_{ij} \cdot U^{UNMAPPED_ACE-1}_i \cdot U^{UNMAPPED_ACE-1}_j &=
...
If $\bar{\gamma} > 0$ and $U^{UNMAPPED_ACE-1}_i > 0$ for all $i$, then $\mathcal{E} > 0$.
\end{corollary}

\begin{proof}
When all utilities are positive, $U^{UNMAPPED_ACE-1}_i \cdot U^{UNMAPPED_ACE-1}_j > 0$ for all pairs.
With $\bar{\gamma} > 0$, there exists a dominant positive contribution to
$\sum_{i<j} \gamma_{ij} U^{UNMAPPED_ACE-1}_i U^{UNMAPPED_ACE-1}_j > 0$. \qed
\end{proof}
...
\begin{equation}
\rho = \frac{U^L}{\sum_i U^{UNMAPPED_ACE-1}_i} = 1 + \frac{\mathcal{E}}{\sum_i U^{UNMAPPED_ACE-1}_i} = 1 + \frac{\mathcal{E}}{n \bar{U}}
...
\textbf{Requirement:} $U^{UNMAPPED_ACE-1}_i > 0$ for all (or most) members

\textbf{Interpretation:} Members must have positive baseline value.

\textbf{When Violated:} If some members have $U^{UNMAPPED_ACE-1}_i < 0$, the product
...
\textbf{Statement:} Adding a member with $U^{UNMAPPED_ACE-1} < 0$ (negative utility
...
level structure in Appendix UNMAPPED_WHO:
...
The $\gamma$ matrix from Appendix UNMAPPED_HOW is the key input to LET:

\begin{equation}
\text{LET uses: } \Gamma = [\gamma_{ij}]_{n \times n} \text{ from Appendix UNMAPPED_HOW}
\end{equation}
...
The Decision Hierarchy in Appendix UNMAPPED_HIE is an \textit{application} of LET:
...
\begin{tcolorbox}[colback=green!10!white, colframe=green!75!black,
    title=\textbf{Appendix UNMAPPED_LET Summary: The Level Emergence Theorem}]
...
\textbf{The Emergence Equation:}
\begin{equation*}
\mathcal{E} = U^L - \sum_i U^{UNMAPPED_ACE-1}_i = \frac{n(n-1)}{2} \cdot \bar{\gamma} \cdot \bar{U}^2 + O(\text{Var})
\end{equation*}

\textbf{Key Results:}
\begin{itemize}[nosep]
\item \textbf{Theorem:} $U^L > \sum U^{UNMAPPED_ACE-1} \Leftrightarrow \sum \gamma_{ij} U_i U_j > 0$
...
\begin{table}[htbp]
\centering
\caption{Symbols Introduced in Appendix UNMAPPED_LET}
...
$\mathcal{E}$ & Emergence premium & $U^L - \sum_i U^{UNMAPPED_ACE-1}_i$ \\
$\rho$ & Emergence ratio & $U^L / \sum_i U^{UNMAPPED_ACE-1}_i$ \\
