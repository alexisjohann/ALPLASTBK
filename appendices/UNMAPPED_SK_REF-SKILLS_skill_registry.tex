...
% - Appendix UNMAPPED_DP (REF-DEVPIPE): Developer Pipeline
% - Appendix UNMAPPED_APL (REF-APPLY): EBF Workflow Reference
% - Appendix UNMAPPED_BN (REF-SUPERKEY): Database Superkey System
%
...
%
% DEPENDENCIES (what this appendix REQUIRES):
% - Appendix UNMAPPED_BN (REF-SUPERKEY): Superkey formats (LEAD, PRJ, SKL)
...
\begin{itemize}[nosep]
\item Appendix UNMAPPED_DP (REF-DEVPIPE): Developer Pipeline architecture
\item Appendix UNMAPPED_APL (REF-APPLY): EBF Workflow steps
\item Appendix UNMAPPED_BN (REF-SUPERKEY): Database identifiers
...
\begin{itemize}[nosep]
    \item Developer implementation details $\rightarrow$ Appendix UNMAPPED_DP (REF-DEVPIPE)
    \item EBF workflow theory $\rightarrow$ Appendix UNMAPPED_APL (REF-APPLY)
    \item Database schemas $\rightarrow$ Appendix UNMAPPED_BN (REF-SUPERKEY)
\end{itemize}
...
\begin{itemize}[nosep]
\item Appendix UNMAPPED_DP (REF-DEVPIPE): Implementation details
\item Appendix UNMAPPED_APL (REF-APPLY): Workflow theory
\item Appendix UNMAPPED_BN (REF-SUPERKEY): Database identifiers
\item Appendix UNMAPPED_GLS (Glossary): Symbol definitions
\end{itemize}
...
For symbol definitions and terminology, see Appendix UNMAPPED_GLS (Central Glossary).
...
│       ↓                                                                 │
│  Appendix UNMAPPED_SK (this document)     ← Formal EBF documentation            │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
\end{verbatim}
...
\begin{description}
\item[Axiom UNMAPPED_SK-1 (SSOT Principle)] There exists exactly one authoritative source for skill definitions:
\texttt{data/skill-registry.yaml}. All other documentation derives from this source.

\item[Axiom UNMAPPED_SK-2 (EBF Integration)] Every skill $s \in S$ must specify its EBF integration:
\[
\text{ebf}(s) = \langle A_s, C_s, D_s \rangle
\]
where $A_s$ = referenced appendices, $C_s$ = referenced chapters, $D_s$ = 10C dimensions.

\item[Axiom UNMAPPED_SK-3 (CVA Dependency)] For skills in categories PRJ, FRM, CUS:
\[
\text{requires\_cva}(s) = \text{true} \Rightarrow \text{CVA must exist before execution}
\]

\item[Axiom UNMAPPED_SK-4 (Workflow Composition)] Skills can be composed into workflow chains:
\[
W = s_1 \rightarrow s_2 \rightarrow \ldots \rightarrow s_n
\]
where $\text{output}(s_i) \subseteq \text{input}(s_{i+1})$.

\item[Axiom UNMAPPED_SK-5 (Superkey Uniqueness)] Every skill has a unique identifier:
\[
...
\textbf{EBF Connection:} Chapters 17--20 (Intervention Design), Appendix TKT (METHOD-TOOLKIT)
...
\textbf{ID} & \textbf{Command} & \textbf{Description} \\
\hline
SKL-UNMAPPED_FRM-001 & \texttt{/design-model} & 9-step behavioral model design \\
SKL-UNMAPPED_FRM-002 & \texttt{/design-intervention} & 20-field intervention schema \\
SKL-UNMAPPED_FRM-003 & \texttt{/find-model} & Model registry search \\
SKL-UNMAPPED_FRM-004 & \texttt{/case} & Case registry query \\
SKL-UNMAPPED_FRM-005 & \texttt{/case-manage} & Case management \\
SKL-UNMAPPED_FRM-006 & \texttt{/intervention} & Intervention registry query \\
\hline
\end{tabular}
...
\item \texttt{/design-model} (SKL-UNMAPPED_FRM-001)
\item \texttt{/design-intervention} (SKL-UNMAPPED_FRM-002)
...
\begin{itemize}
\item Appendix UNMAPPED_BN (REF-SUPERKEY): Superkey format definitions
\item Appendix G (Glossary): Central glossary
\item Appendix EEE (METHOD-DESIGN): Model design workflow
\item Appendix TKT (METHOD-TOOLKIT): Intervention toolkit
\item Appendix UNMAPPED_VC (METHOD-VALIDATE): Validation framework
\end{itemize}
...
\small
\textbf{Cross-References:}
Appendix UNMAPPED_BN (Superkeys) $\rightarrow$
Appendix G (Glossary) $\rightarrow$
Appendix EEE (Model Design) $\rightarrow$
Appendix TKT (Intervention Toolkit)
