...
% questions documented in Appendix UNMAPPED_DA:
...
\textbf{Out-of-Scope:}
\begin{itemize}[nosep]
    \item Formal axioms (see Appendix UNMAPPED_DA)
    \item Detailed parameter documentation (see Appendix UNMAPPED_BBB)
    \item LLMMC methodology details (see Appendix UNMAPPED_LLM)
\end{itemize}
...
identified in Appendix UNMAPPED_DA. Each question has 6 response variants: 3 lengths
...
\end{equation}
d.h., für jeden Datensatz existiert eine Parameterkonfiguration, die ihn ``erklärt''. Dies entspricht Lakatos' Kritik an ``degenerativen Forschungsprogrammen'' \citep{kuhn1962structure}.
...
\end{equation}
Beispiel: $\lambda \in [1.5, 2.8]$ basierend auf der originalen Prospect Theory \citep{kahneman1979prospectprospect, kahneman1991loss} und nachfolgenden Meta-Analysen. Ein Modell, das $\lambda = 5.0$ benötigt, ist falsifiziert, da ausserhalb des empirisch etablierten Intervalls.
...
\end{equation}
Wenn $H_0$ die Daten systematisch besser erklärt (via Likelihood-Ratio, BIC, Cross-Validation), ist EBF falsifiziert. Die Komplementaritäts-These ist direkt aus der empirischen Literatur zu Interaktionseffekten abgeleitet \citep{gaechter2002altruistic, gaechter2001conditional}.
...
\end{equation}
Dies erfordert Pre-Registration der Vorhersage vor Datenerhebung---analog zur Methodologie in \citet{camerer2018evaluatingevaluating} für Replikationsstudien.

\textbf{4. Kreuzvalidierung über Kontexte:}
Parameter $\theta$, geschätzt in Kontext $\Psi_1$, müssen in Kontext $\Psi_2$ funktionieren (mit dokumentierter Kontext-Modulation). Systematisches Versagen der Übertragung falsifiziert die behauptete Universalität. Die Kontextabhängigkeit folgt der etablierten Literatur zu situativen Faktoren \citep{cialdini2006influence, laibson2003projection}.
...
\begin{itemize}[nosep]
    \item Prospect Theory: \citet{kahneman1979prospectprospect}, \citet{kahneman1991loss}
    \item Falsifikationismus: \citet{popper1959logic}
    \item Soziale Präferenzen: \citet{gaechter2002altruistic}, \citet{gaechter2001conditional}
    \item Replikationsmethodologie: \citet{camerer2018evaluatingevaluating}
\end{itemize}
...
\begin{enumerate}
    ...
    \end{equation}
    Mit expliziter Unsicherheitsquantifizierung und nachfolgendem Bayesian Updating gegen Literatur. Die Methodologie ist in Appendix UNMAPPED_LLM (METHOD-LLMMC) formalisiert und folgt der Logik von Expert Elicitation \citep{ohaganelicitation}.
...
\begin{enumerate}
    \item \textbf{Struktur-Entscheidungen:} 10C-Dimensionen sind framework-definiert, nicht LLM-generiert. Die Utility-Dimensionen (FEPSDE) folgen \citet{gaechter2002altruistic}.
    \item \textbf{Parameter-Werte:} Kommen aus BBB (Literatur), nicht aus LLM-``Wissen''. Beispiel: Loss Aversion $\lambda$ aus \citet{kahneman1979prospectprospect}.
    \item \textbf{Kausal-Aussagen:} Identifikation erfordert Forschungsdesign \citep{camerer2003behavioralbehavioral}, nicht LLM-Korrelationen.
...
\begin{itemize}[nosep]
    ...
    \item Modell: Default + Social Proof, $\gamma_{\text{default,social}} = +0.15$ \citep{johnson2003defaults, allcott2011social}
...
\textbf{Schlüsselreferenzen:}
\begin{itemize}[nosep]
    \item Behavioral Economics Framework: \citet{camerer2003behavioralbehavioral}, \citet{thaler2015misbehaving}
    \item Default-Effekte: \citet{johnson2003defaults}, \citet{choi2004better}, \citet{benartzi2007save}
    \item Soziale Normen: \citet{cialdini2006influence}, \citet{allcott2011social}
    \item Kooperation: \citet{fehr2002cooperation}, \citet{gaechter2001conditional}
\end{itemize}
...
$\lambda = 2.25$ basiert auf \citet{kahneman1979prospectprospect}, repliziert in >100 Studien. Meta-Analyse-Schätzung: $\hat{\lambda} = 2.25$ mit 95\%-CI $[1.93, 2.57]$.
...
\begin{itemize}[nosep]
    \item Originalschätzung: $\lambda \approx 2.25$ \citep{kahneman1979prospectprospect}
...
\begin{itemize}[nosep]
    \item Quasi-hyperbolisch: $\beta \in [0.7, 0.9]$ \citep{laibson1997golden, odonoghue1999doingdoing}
    \item Kontextabhängig: Gesundheit vs. Finanzen zeigen unterschiedliche $\beta$
\end{itemize}

\textit{Social Preference Parameter ($\sigma$):}
\begin{itemize}[nosep]
    \item Inequity Aversion: $\alpha \approx 0.85$, $\beta \approx 0.30$ \citep{fehr1993reciprocity}
    \item Conditional Cooperation: $\sigma \in [0.10, 0.25]$ \citep{gaechter2001conditional}
\end{itemize}
...
\begin{itemize}[nosep]
    \item Prospect Theory: \citet{kahneman1979prospectprospect}, \citet{kahneman1991loss}
    \item Hyperbolisches Diskontieren: \citet{laibson1997golden}, \citet{odonoghue1999doingdoing}
    \item Soziale Präferenzen: \citet{fehr1993reciprocity}, \citet{gaechter2001conditional}, \citet{gaechter2002altruistic}
    \item Methodik: \citet{camerer2003behavioralbehavioral}, \citet{camerer2018evaluatingevaluating}
\end{itemize}
...
\textbf{Tier 1 (Stark):} Die 2,328 integrierten Papers liefern RCT-Evidenz für einzelne Mechanismen (Loss Aversion \citep{kahneman1979prospectprospect}, Social Proof \citep{cialdini2006influence, allcott2011social}, Defaults \citep{johnson2003defaults,choi2004better}). Diese Evidenz ist robust.
...
\textbf{Geplante Evaluation:} Pre-Registration von Vorhersagen + systematisches Outcome-Tracking als pragmatische Alternative zu RCT, analog zur Methodologie in \citet{camerer2018evaluatingevaluating}.
...
Social Proof & $+2\%$ bis $+10\%$ & \citet{allcott2011social}, \citet{schultz2007normative} \\
Loss Framing & $+5\%$ bis $+15\%$ & \citet{kahneman1979prospectprospect}, \citet{benartzi1995myopic} \\
Commitment & $+10\%$ bis $+20\%$ & \citet{benartzi2007save}, \citet{bryan2010commitment} \\
\end{tabular}
\end{center}

Diese Effekte sind in hunderten von RCTs repliziert und robust über Kontexte \citep{camerer2003behavioralbehavioral, thaler2015misbehaving}.
...
\textbf{Schlüsselreferenzen:}
\begin{itemize}[nosep]
    \item Komponenten-Evidenz: \citet{kahneman1979prospectprospect}, \citet{johnson2003defaults}, \citet{allcott2011social}
    \item Defaults in Retirement: \citet{choi2004better}, \citet{benartzi2007save}
    \item Methodologie: \citet{camerer2003behavioralbehavioral}, \citet{camerer2018evaluatingevaluating}
...
\begin{itemize}[nosep]
    \item \textbf{DA (REF-DEFENSE):} Vollständige Axiome UNMAPPED_DA-1 bis UNMAPPED_DA-20
...
\begin{enumerate}
    ...
    \item \textbf{Consistency:} All responses derive from the same underlying axioms (UNMAPPED_DA-0 to UNMAPPED_DA-20).
