...
\subsection{CORE-WHO: The Welfare Hierarchy (Appendix UNMAPPED_WHO)}
...
\item Axioms UNMAPPED_SRL-1 through UNMAPPED_SRL-8 defining level structure
...
\subsection{CORE-WHAT: The Welfare Dimensions (Appendix UNMAPPED_WAT)}
...
\subsection{CORE-HOW: The Interaction Structure (Appendix UNMAPPED_HOW)}
...
\subsection{CORE-WHERE: Parameter Estimation (Appendix UNMAPPED_EST)}
...
\subsection{CORE-AWARE: The Awareness Function (Appendix UNMAPPED_AWA)}
...
\subsection{CORE-READY: The Willingness Function (Appendix UNMAPPED_REA)}
...
\subsection{CORE-STAGE: The Behavioral Change Journey (Appendix UNMAPPED_STA)}
...
\subsection{CORE-EIT: Emergent Intervention Theory (Appendix UNMAPPED_IE)}
...
\textbf{For Practitioners:} See Appendix TKT (METHOD-TOOLKIT), Section ``Quickstart Guide: Using the IE Master Matrix'' for step-by-step intervention design protocol.
...
\subsection{CORE-PRINCIPLES: The Seven Fundamental Axioms (Appendix UNMAPPED_PR)}
...
\textbf{Key Content:}
\begin{itemize}[nosep]
\item 7 formal axioms (UNMAPPED_PR-1 to UNMAPPED_PR-7) defining EBF membership
...
IV & FORMAL-LET & Level Emergence Theorem & $\mathcal{E} = U^L - \sum U^{UNMAPPED_ACE-1}$ when $\bar{\gamma} > 0$ \\
