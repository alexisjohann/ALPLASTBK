...
% - Appendix UNMAPPED_LLM (METHOD-LLMMC): Core LLMMC theory (LLMs as stochastic estimators)
% - Appendix TKT (METHOD-TOOLKIT): Intervention design schema
% - Appendix UNMAPPED_EST (CORE-WHERE): Parameter repository for anchors
% - Appendix CTW (CORE-WHEN): Context framework for Ψ
% - Appendix UNMAPPED_DSN (METHOD-DESIGN): Model design workflow
%
% DEPENDENTS (what REQUIRES this appendix):
% - Appendix TKT: Uses R-Score for intervention prioritization
% - Appendix UNMAPPED_DSN: Uses calibrated α for model parameters
...
\textbf{Context:} This appendix covers LLMMC (Tier 3 calibration). For the complete parameterization methodology including all three tiers and method selection, see \textbf{Appendix UNMAPPED_PM (REF-PARAMETH)}.
...
for LLMMC estimates \citep{xie2013population}.
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=Axiom UNMAPPED_CAL-5: Population Statistical Realism (Xie Criterion)]
...
\begin{itemize}[nosep]
\item \citet{xie2013population}: Population Heterogeneity and Causal Inference
\item \citet{holland1986statistics}: Statistics and Causal Inference
\item \citet{brand2010benefits}: Heterogeneous Returns to Education
\item \citet{xie2012estimating}: Estimating HTE with Observational Data
\end{itemize}
...
\citep{kasy2021adaptive, caria2024refugees}.
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=Axiom UNMAPPED_CAL-6: Welfare-Reward Distinction (Kasy Criterion)]
...
need more precise estimates \citep{kasy2021adaptive}.
...
through incentive-compatible payments \citep{vickrey1961counterspeculation, clarke1971multipart, groves1973incentives}

\item \textbf{Pigouvian taxation} reveals social costs through optimal corrective
taxes \citep{pigou1920welfare}

\item \textbf{BDM mechanism} elicits willingness-to-pay through incentive-compatible
lotteries \citep{becker1964bdm}
\end{itemize}
...
(Appendix UNMAPPED_IE) can be learned through adaptive mechanisms, not just estimated from
historical data.
\end{tcolorbox}
...
framework \citep{chernozhukov2018double} provides valid inference:
...
Causal Forests \citep{athey2016recursive, wager2018causalforest} enable
...
\textbf{References for this section:}
\begin{itemize}[nosep]
\item \citet{kasy2021adaptive}: Adaptive Treatment Assignment (Econometrica 2021)
\item \citet{kasy2025adaptive}: Adaptive Welfare Maximization (Econometrica 2025)
\item \citet{chernozhukov2018double}: Double/Debiased ML (Econometrics Journal 2018)
\item \citet{athey2016recursive}: Causal Trees (PNAS 2016)
\item \citet{wager2018causalforest}: Causal Forests (JASA 2018)
\item \citet{caria2024refugees}: Adaptive Field Experiment (JEEA 2024)
\item \citet{becker1964bdm}: BDM Mechanism (Behavioral Science 1964)
\end{itemize}
...
causal attribution \citep{pearl2015causes, dawid2022effects}.
...
The \textbf{Probability of Causation (PoC)} framework \citep{tian2000probabilities}
...
\textbf{informative bounds} can be derived \citep{tian2000probabilities}:
...
Recent work extends PoC to continuous outcomes \citep{luo2025attribution}:

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=Axiom UNMAPPED_CAL-7: Posterior Causal Attribution (Luo-Geng Criterion)]
...
statistical evidence into legally relevant probabilities \citep{greenland2000epidemiology}.

\item \textbf{Segment-to-Individual Inference:}
Dawid and Musio's work on ``group to individual'' inference \citep{dawid2022individual}
...
\textbf{References for this section:}
\begin{itemize}[nosep]
\item \citet{pearl2015causes}: Causes of Effects and Effects of Causes (SMR 2015)
\item \citet{dawid2022effects}: Effects of Causes and Causes of Effects (Ann Rev Stats 2022)
\item \citet{tian2000probabilities}: Probabilities of Causation: Bounds and Identification
\item \citet{luo2025attribution}: Causal Attribution for Continuous Outcomes (ICML 2025)
\item \citet{greenland2000epidemiology}: Epidemiology, Justice, and PoC (Jurimetrics 2000)
\item \citet{hart1985causation}: Causation in the Law (OUP 1985)
\end{itemize}
...
Syrgkanis and Zhan \citep{syrgkanis2025postrl}.
...
The \textbf{blip function} \citep{robins2000msm, syrgkanis2025postrl}
isolates the causal effect of treatment at stage $t$, conditional on history:

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=Axiom UNMAPPED_CAL-8: Blip Decomposition (Syrgkanis-Zhan)]
...
A \textbf{dynamic treatment regime} (DTR) \citep{murphy2003optimal} is a sequence
...
(MSM) \citep{robins2000msm} handle time-varying confounders:
...
The classic \textbf{Oaxaca-Blinder decomposition} \citep{oaxaca1973decomposition}
...
recommendation systems, use post-RL inference methods \citep{syrgkanis2025postrl}
to account for non-stationarity.
\end{enumerate}

\textbf{References for this section:}
\begin{itemize}[nosep]
\item \citet{syrgkanis2025postrl}: Post RL Inference (Operations Research 2025)
\item \citet{zhang2024personalized}: Personalized Policy Learning (Management Science 2024)
\item \citet{robins2000msm}: Marginal Structural Models (Epidemiology 2000)
\item \citet{murphy2003optimal}: Optimal Dynamic Treatment Regimes (JRSS-B 2003)
\item \citet{oaxaca1973decomposition}: Male-Female Wage Differentials (IER 1973)
\item \citet{athey2021policy}: Policy Learning with Observational Data (ECMA 2021)
\end{itemize}
...
\midrule
Yu Xie & Population Heterogeneity & \S\ref{sec:iii-validation-psr} & UNMAPPED_CAL-5 \\
Maximilian Kasy & ML, Causal Inference, Economics & \S\ref{sec:iii-adaptive} & UNMAPPED_CAL-6 \\
Simon Deakin (Essay) & Causation in Law \& Statistics & \S\ref{sec:iii-coe} & UNMAPPED_CAL-7 \\
Susan Athey / Ruohan Zhan & Sequence Data \& Decompositions & \S\ref{sec:iii-seq} & UNMAPPED_CAL-8 \\
\bottomrule
\end{tabular}
\caption{CIFW05 content integrated into Appendix UNMAPPED_CAL.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Glossary of symbols. See also Appendix UNMAPPED_GLS (Central Glossary).}
\end{table}
...
Appendix TKT (METHOD-TOOLKIT) $\leftarrow$ uses R-Score |
Appendix UNMAPPED_DSN (METHOD-DESIGN) $\leftarrow$ uses calibrated $\alpha$ |
Appendix UNMAPPED_EST (CORE-WHERE) $\rightarrow$ provides anchors |
Appendix XX (METHOD-EVIDENCE) $\rightarrow$ evidence tier requirements for LLMMC input |
Appendix UNMAPPED_GLS (Glossary) $\rightarrow$ symbol definitions |
\textbf{Appendix UNMAPPED_PM (REF-PARAMETH)} $\leftarrow$ methodology navigation hub |
\textbf{Appendix UNMAPPED_DP (REF-DEVPIPE)} $\leftarrow$ Stage 3 (Calibration) in 6-stage pipeline |
\textbf{Appendix UNMAPPED_DR (REF-DATAREQ)} $\rightarrow$ data source inventory |
\textbf{Appendix UNMAPPED_VC (METHOD-VALIDATE)} $\leftarrow$ validates estimated parameters (DCV-3, DCV-4)
\end{tcolorbox}
