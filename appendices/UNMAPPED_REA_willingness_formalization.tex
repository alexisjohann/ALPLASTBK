...
% The 7th of the 10C CORE questions in the EBF Framework
%
% PURPOSE:
...
%------------------------------------------------------------------------------
\begin{tcolorbox}[
  title={\textbf{Die 9 Fragen des EBF — Appendix UNMAPPED_REA Formalisierung}},
  colback=green!3,
...
$\checkmark$~1. WIE denkt er/sie? & $\tau$ \\
...
\begin{tabular}{@{}p{0.75\textwidth}r@{}}
$\circ$~0. Kontext und Zielverhalten & [Chapter 6, Appendix UNMAPPED_AWA] \\
$\circ$~2. Welchen Nutzen? & [Chapter 10] \\
$\circ$~3. Wie nimmt er/sie wahr? & [Chapter 11, Appendix UNMAPPED_AWA] \\
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=\textbf{Appendix UNMAPPED_REA: CORE-READY}]

\begin{tabular}{@{}ll@{}}
\textbf{Appendix:} & AV (Willingness Formalization) \\
\textbf{Category:} & CORE (7th of 10C Framework) \\
\textbf{Scope:} & Complete specification of WAX function and 99 axioms \\
\textbf{Version:} & 3.0 \\
\textbf{Last Updated:} & January 2026 \\
\textbf{Dependencies:} & Appendix UNMAPPED_AWA (CORE-AWARE: Awareness function) \\
                       & Appendix UNMAPPED_WAT (CORE-WHAT: Dimension-specific willingness) \\
                       & Appendix UNMAPPED_HOW (CORE-HOW: Complementarity) \\
                       & Appendix CTW (CORE-WHEN: Context $\Psi$) \\
                       & Appendix UNMAPPED_EST (CORE-WHERE: Threshold calibration) \\
\textbf{Feeds into:}   & Appendix UNMAPPED_STA (CORE-STAGE: Phase transitions) \\
\textbf{Outputs:} & Willingness function $WAX = f(\tau, U^*, \Psi)$ \\
...
intention-behavior gap by integrating three components: (1) the fusion parameter $\tau$ determining
...
\begin{tcolorbox}[colback=purple!5!white, colframe=purple!75!black,
    title=\textbf{CORE-READY: The Seventh Fundamental Question (10C)}]
...
Where:
\begin{itemize}[nosep]
    \item $WAX$ = Willingness to Act (combines $\tau$, $U^*$, context)
    \item $\theta$ = Behavioral threshold (barriers, friction, inertia)
    \item The \textbf{gap} $\Delta = WAX - \theta$ determines action probability
\end{itemize}

\textbf{Integration with 10C:}
\[
WAX = f\Bigl(\underbrace{\tau}_{\text{HOW think}}, \underbrace{A(\cdot) \times U}_{\text{AWARE}}, \underbrace{\Psi}_{\text{WHEN}}\Bigr) \quad \text{vs.} \quad \theta = g(\theta_{base}, \Psi)
\]

\end{tcolorbox}

\section*{Appendix UNMAPPED_REA: CORE-READY — The Willingness Function}
\addcontentsline{toc}{section}{Appendix UNMAPPED_REA: CORE-READY — The Willingness Function}
...
\textbf{In-Scope:}
\begin{itemize}[nosep]
    \item Overview and Structure
    \item Definitions and Notation
    \item The WAX Master Equation
    \item The Critical Comparison: WAX vs. $\theta$
    \item Fundamental Axioms (WAX-A1 to WAX-A18)
\end{itemize}

\textbf{Out-of-Scope (delegiert an andere Appendices):}
\begin{itemize}[nosep]
    \item CORE-AWARE: Awareness function $\rightarrow$ Appendix UNMAPPED_AWA
    \item CORE-WHAT: Dimension-specific willingness $\rightarrow$ Appendix UNMAPPED_WAT
    \item CORE-HOW: Complementarity $\rightarrow$ Appendix UNMAPPED_HOW
    \item CORE-WHEN: Context $\Psi$ $\rightarrow$ Appendix CTW
    \item CORE-WHERE: Threshold calibration $\rightarrow$ Appendix UNMAPPED_EST
\end{itemize}
...
axioms, following the 8-factor structure established in Appendix UNMAPPED_AWA for Awareness axioms.
...
\label{def:tau}
$\tau \in [0,1]$ determines the relative weight of complementary (norm-bound) versus additive (utility-maximizing) decision logic:
\begin{itemize}
\item $\tau = 0$: Pure additive logic (homo oeconomicus)
\item $\tau = 1$: Pure complementary logic (homo sociologicus)
\item $0 < \tau < 1$: Hybrid (realistic case)
\end{itemize}
\end{definition}
...
\begin{definition}[Tau Dynamics]
\label{def:tau-dynamics}
The evolution of $\tau$ over time follows a stochastic differential equation:
\begin{equation}
d\tau = \mu(\tau^* - \tau)dt + \sigma dW_t
\end{equation}
where:
\begin{itemize}
\item $\mu > 0$: Mean-reversion speed
\item $\tau^*$: Context-determined equilibrium
...
\label{def:tau-star}
$\tau^*(\Psi)$ is the long-run equilibrium value of $\tau$ given context $\Psi$:
\begin{equation}
\tau^* = f(AWX, SEG, IDN_s, REG) - g(UNS, KOM)
\end{equation}
where $f$ increases $\tau^*$ and $g$ decreases it.
\end{definition}
...
The \textbf{context-free limit} occurs when $\tau = 0$ and $\Psi = \mathbf{0}$:
\begin{equation}
WAX|_{\tau=0, \Psi=0} = \sum_i \alpha_i \cdot U_i
\end{equation}
...
The \textbf{norm-bound limit} occurs when $\tau = 1$:
\begin{equation}
WAX|_{\tau=1} = \min_i \{U_i^{eff}\}
\end{equation}
...
%------------------------------------------------------------------------------
% 2.8 Tau Decomposition and Estimation
%------------------------------------------------------------------------------

\subsection{Tau Decomposition and Estimation}
\label{sec:tau-estimation}

The fusion parameter $\tau$ decomposes into a person-specific baseline and a context-dependent modulation:

\begin{definition}[Tau Decomposition]
\label{def:tau-decomposition}
\begin{equation}
\tau_{i,t} = \underbrace{\tau_{0,i}}_{\text{Person (Type)}} + \underbrace{\Delta\tau(\Psi_t)}_{\text{Context (Situation)}}
\end{equation}
where:
\begin{itemize}
\item $\tau_{0,i} \in [0,1]$ is the stable individual baseline (``type'')
\item $\Delta\tau(\Psi_t)$ is the situational modulation
\item $i$ indexes persons, $t$ indexes decision situations
\end{itemize}
\end{definition}

\begin{definition}[Baseline Tau (Person Type)]
\label{def:tau-baseline}
$\tau_{0,i}$ captures stable individual differences in decision logic:
\begin{table}[htbp]
\centering
\begin{tabular}{@{}lcp{6cm}@{}}
\toprule
\textbf{Type} & $\tau_0$ & \textbf{Description} \\
\midrule
...
\label{def:tau-context}
$\Delta\tau(\Psi)$ is the situational shift determined by all 8 $\Psi$-dimensions:
\begin{equation}
\Delta\tau(\Psi) = \sum_{j=1}^{8} \gamma_j \cdot \Psi_j
\end{equation}

\begin{table}[htbp]
\centering
\caption{$\Psi$-Effects on $\tau$}
\label{tab:psi-tau-effects}
\begin{tabular}{@{}lcl@{}}
\toprule
...
\subsubsection{GARP-Based Estimation of $\tau$}

The Generalized Axiom of Revealed Preference (GARP) provides an empirical pathway to estimate $\tau$ and its components.
...
\begin{definition}[GARP-Based Tau Interpretation]
\label{def:garp-tau}
GARP violations in EBF are not ``irrationality'' but evidence of context-dependent $\tau$:
\begin{table}[htbp]
\centering
\begin{tabular}{@{}ll@{}}
\toprule
...
\midrule
Fulfilled across contexts & $\tau$ stable OR $\Psi$ constant \\
Violated across contexts & $\tau$ varies with context \\
Violated at norm-conflicts & Complementary logic ($\tau \rightarrow 1$) active \\
\bottomrule
\end{tabular}
\end{table}
\end{definition}
...
To estimate $\tau_{0,i}$ and $\gamma_j$:
\begin{enumerate}
\item \textbf{Baseline}: GARP test in neutral context $\rightarrow$ estimate $\tau_{0,i}$
\item \textbf{Context variation}: GARP tests across systematically varied $\Psi$-conditions
\item \textbf{Decomposition}: 
\begin{equation}
\text{GARP violation rate}_{i,\Psi} = h(\tau_{0,i} + \sum_j \gamma_j \Psi_j)
\end{equation}
where $h(\cdot)$ is increasing---higher $\tau$ variation $\rightarrow$ more GARP violations
\item \textbf{Regression}: Estimate $\gamma_j$ from violation patterns across contexts
\end{enumerate}
\end{definition}

\begin{definition}[Critical Ratio for Tau Estimation]
\label{def:ccei}
...
\end{equation}
Lower CCEI in context $\Psi_A$ vs. $\Psi_B$ indicates: $|\tau(\Psi_A) - \tau(\Psi_B)| > 0$
\end{definition}
...
\subsection{Worked Example: $\tau$-Estimation via LLM-MC}
\label{sec:tau-example}

This section demonstrates the empirical estimation of $\tau$ and its context-sensitivity using LLM-based Monte Carlo simulation.
...
\item Baseline hypothesis: $\tau_0 \approx 0.25$ (moderately self-interested)
\end{itemize}
\end{definition}
...
\begin{table}[htbp]
\centering
\caption{Observed Donations and Estimated $\tau$}
\label{tab:tau-results}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Condition} & $\Psi_{SEG}$ & $\Psi_{REG}$ & \textbf{Donations} & \textbf{Mean} & \textbf{SD} & $\hat{\tau}$ \\
\midrule
...
\textbf{Interpretation}: The same person with identical preferences donates 10\% (anonymous) vs. 75\% (public + matching). No single utility function explains this. EBF explains it: $\tau$ varies with $\Psi$.

\subsubsection{Regression: Estimating $\gamma$-Coefficients}

From the four data points, we estimate:
\begin{equation}
\tau = \tau_0 + \gamma_{SEG} \cdot \Psi_{SEG} + \gamma_{REG} \cdot \Psi_{REG}
\end{equation}

\begin{table}[htbp]
\centering
\caption{Estimated Parameters for Thomas Müller}
\label{tab:tau-params}
\begin{tabular}{@{}lrl@{}}
\toprule
\textbf{Parameter} & \textbf{Value} & \textbf{Interpretation} \\
\midrule
$\tau_0$ & 0.22 & Baseline (moderately self-interested) \\
...
\textbf{Thomas's $\tau$-formula}:
\begin{equation}
\boxed{\tau_{Thomas} = 0.22 + 0.48 \cdot \Psi_{SEG} + 0.33 \cdot \Psi_{REG}}
\end{equation}
...
\begin{table}[htbp]
\centering
\caption{Model Fit: Observed vs. Predicted $\tau$}
\begin{tabular}{@{}lccc@{}}
\toprule
...
\subsection{Context-Specific $\tau$ Calculation}
\label{sec:tau-per-context}
...
\end{itemize}
Each context has its own $\Psi \rightarrow$ its own $\tau \rightarrow$ its own WAX.
\end{definition}

\begin{definition}[Per-Context $\tau$ Calculation]
\label{def:tau-per-context}
For \textbf{each} Behavioral Change Context, $\tau$ is calculated anew:
\begin{equation}
\tau_{i,c} = \tau_{0,i} + \sum_{j=1}^{8} \gamma_{j,i} \cdot \Psi_{j,c}
\end{equation}
where:
\begin{itemize}
\item $i$ = person (stable parameters $\tau_0$, $\gamma_j$)
...
% Person box at top
\node[box, fill=blue!10] (person) at (0,3) {\textbf{PERSON (stable)}\\$\tau_0 = 0.22$\\$\gamma_{SEG} = 0.48$\\$\gamma_{REG} = 0.33$};
...
% Result boxes
\node[box, fill=orange!20] (r1) at (-4,-2) {$\tau_1 = 0.22$};
\node[box, fill=orange!20] (r2) at (0,-2) {$\tau_2 = 0.64$};
\node[box, fill=orange!20] (r3) at (4,-2) {$\tau_3 = 0.70$};
...
\draw[arrow] (c3) -- (r3);
\end{tikzpicture}
\caption{Same person, different contexts $\rightarrow$ different $\tau$ values}
\label{fig:tau-per-context}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Context-Dependent $\tau$ Throughout a Day}
\label{tab:thomas-day}
\begin{tabular}{@{}llcccl@{}}
\toprule
\textbf{Time} & \textbf{Situation} & $\Psi_{SEG}$ & $\Psi_{REG}$ & $\tau$ & \textbf{Decision} \\
\midrule
...
\textbf{Key insight}: Thomas is not ``inconsistent''---his context changes, and with it his $\tau$. This is not irrationality; it is context-sensitive rationality.
...
% Insert after Tau Decomposition, before Scientific Foundation for φ
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{What is a Thinking Style? Conceptual Foundation}
\label{sec:thinking-style-concept}

Before examining the empirical evidence for $\tau$, we must precisely define what we mean by ``thinking style'' and how this concept relates to established constructs in psychology, behavioral economics, and decision science.
...
The EBF thinking style parameter $\tau$ relates to, but is distinct from, several established constructs:

\begin{table}[htbp]
\centering
\caption{Relation of $\tau$ to Established Constructs}
\label{tab:tau-constructs}
\begin{tabular}{p{3.5cm}p{4cm}p{5cm}}
\toprule
\textbf{Construct} & \textbf{Similarity to $\tau$} & \textbf{Key Difference} \\
\midrule
\textbf{Need for Cognition}
\newline (Cacioppo \& Petty, 1982) 
& Both capture individual differences in information processing 
& NFC measures \textit{enjoyment} of thinking; $\tau$ measures \textit{integration logic} \\[8pt]

\textbf{Holistic vs. Analytic Thinking}
\newline (Nisbett et al., 2001)
& Holistic $\approx$ high $\tau$; Analytic $\approx$ low $\tau$
...
& $\tau$ is orthogonal to processing speed \\[8pt]

\textbf{Maximizing vs. Satisficing}
\newline (Schwartz et al., 2002)
& Satisficers accept ``good enough'' across dimensions
& $\tau$ concerns integration, not aspiration level \\[8pt]

\textbf{Homo Economicus vs. Homo Sociologicus}
& $\tau=0$ $\approx$ homo economicus; $\tau=1$ $\approx$ homo sociologicus
& $\tau$ is a continuous parameter, not a type \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Mathematical Formalization: Why $\tau$ Captures ``Thinking Style''}

The WAX formula makes explicit how $\tau$ implements different integration logics:

\begin{equation}
WAX = \tau \cdot \underbrace{\min_i\{U_i^*\}}_{\text{Complementary}} + (1-\tau) \cdot \underbrace{\sum_i U_i^*}_{\text{Additive}}
\end{equation}
...
\item \textbf{The parameter $\tau$} is the \textit{mixing weight} between these two logics. It is not a preference over outcomes, but a preference over \textit{how to evaluate} outcomes.
\end{itemize}

\subsubsection{Philosophical Roots: Incommensurability}

The distinction between $\tau \to 1$ and $\tau \to 0$ maps onto a deep philosophical debate:

\begin{itemize}
\item \textbf{Value Pluralism} (Berlin, 1969; Raz, 1986): Some values are \textit{incommensurable}—they cannot be reduced to a single currency. A person with high $\tau$ treats financial and moral dimensions as incommensurable.

\item \textbf{Consequentialism} (Bentham, Mill): All values can be aggregated into a single utility metric. A person with low $\tau$ implicitly adopts this stance.

\item \textbf{Threshold Deontology} (Moore, 1997): Most values aggregate, but some create absolute constraints. This corresponds to intermediate $\tau$ with dimension-specific thresholds.
\end{itemize}

\begin{intuition}
\textbf{The Trolley Problem as $\tau$-Test:}

The classic trolley dilemma reveals thinking style:
\begin{itemize}
\item \textbf{Low $\tau$ (Consequentialist):} ``5 lives > 1 life. Pull the lever.''
\item \textbf{High $\tau$ (Deontological):} ``Actively killing is wrong, regardless of numbers. Don't pull.''
\end{itemize}

The person with high $\tau$ treats ``not actively killing'' as incommensurable with outcome utility.
\end{intuition}

\subsubsection{Economic Foundations: CES Utility and Complementarity}

The $\tau$ parameter has rigorous foundations in economic theory:
...
Our parameter $\tau$ provides an \textbf{intuitive behavioral interpretation} of what is otherwise an abstract substitution elasticity.
...
\label{tab:cultural-tau}
\begin{tabular}{p{3cm}p{3cm}p{6cm}}
\toprule
\textbf{Study} & \textbf{Finding} & \textbf{Implication for $\tau$} \\
\midrule
Nisbett et al. (2001) & East Asians more holistic; Westerners more analytic & Cultural baseline $\tau_0$ varies \\[4pt]

Henrich et al. (2010) & WEIRD populations are outliers in behavioral experiments & Western $\tau_0$ may be unusually low \\[4pt]

Talhelm et al. (2014) & Rice vs. wheat cultures differ in holistic thinking & Ecological factors shape $\tau_0$ \\[4pt]

Schulz et al. (2019) & Church exposure predicts individualism & Institutional history shapes $\tau_0$ \\
\bottomrule
\end{tabular}
\end{table}

\begin{note}[The WEIRD Problem]
Much behavioral research uses Western, Educated, Industrialized, Rich, Democratic (WEIRD) samples. Henrich et al. (2010) showed these populations are \textit{outliers} on many behavioral measures—potentially including $\tau$. EBF's context-dependent $\tau(\Psi)$ explicitly addresses this by making cultural context a modulator.
\end{note}
...
\item \textbf{Default Mode Network:} Self-referential processing (high $\tau_{IDN}$) activates default mode regions, while utilitarian calculation activates dorsolateral prefrontal cortex (Greene et al., 2004).

\item \textbf{Oxytocin and Prosociality:} Oxytocin increases in-group trust and norm-adherence, potentially shifting $\tau$ upward in social contexts (Kosfeld et al., 2005).
\end{itemize}
...
\item Captured by $\tau \in [0,1]$: complementary vs. additive logic
...
\begin{itemize}
\item $\tau$ is context-dependent: $\tau = \tau_0 + \Delta\tau(\Psi)$
\item Enables prediction of how situations shift thinking style
\item Provides intervention lever: change context to shift $\tau$
\end{itemize}
...
\subsection{Scientific Foundation for Context-Dependent $\tau$}
\label{sec:tau-scientific}

The decomposition of $\tau$ into a stable person component ($\tau_0$) and context-sensitive modulation ($\Delta\tau(\Psi)$) is supported by substantial empirical evidence across multiple research traditions.
...
\item \textbf{Fisman, Jakiela \& Kariv (2015)}: Extended GARP analysis to social preferences, showing that distributional choices violate GARP when contexts change---direct evidence for context-dependent $\tau$ \cite{fisman2015}.
\end{itemize}

\subsubsection{Cross-Cultural Evidence for $\tau_0$ Heterogeneity}

The baseline parameter $\tau_0$ varies systematically across cultures and institutions:

\begin{itemize}
\item \textbf{Henrich et al. (2001)}: The landmark ``In Search of Homo Economicus'' study in the \textit{American Economic Review} demonstrated dramatic cross-cultural variation in Ultimatum and Dictator Game behavior. Offers ranged from 26\% to 58\% across 15 small-scale societies---impossible to explain with a universal $\tau$ \cite{henrich2001}.

\item \textbf{Herrmann, Thöni \& Gächter (2008)}: Showed that antisocial punishment varies dramatically across societies, with cooperation norms ($\approx \tau$) being culturally and institutionally determined \cite{herrmann2008}.

\item \textbf{Bowles (1998)}: Argued in the \textit{Journal of Economic Literature} that preferences are endogenous to economic institutions---markets themselves shape $\tau_0$ over time \cite{bowles1998}.

\item \textbf{Fehr \& Hoff (2011)}: Demonstrated how caste, culture, and social identity systematically shape economic preferences, supporting the view that $\tau_0$ is socialized rather than innate \cite{fehrhoff2011}.
\end{itemize}
...
Institutions can both increase and decrease $\tau$:

\begin{itemize}
\item \textbf{North (1990)}: Nobel laureate's foundational work establishing that institutions fundamentally shape economic behavior and cooperation \cite{north1990institutions}.

\item \textbf{Ostrom (1990)}: Nobel laureate's analysis of commons governance showed how institutional design enables sustained cooperation (high $\tau$) without privatization or state control \cite{ostrom1990}.

\item \textbf{Gneezy \& Rustichini (2000)}: The famous ``A Fine Is a Price'' study in the \textit{Journal of Legal Studies} showed that introducing fines for late daycare pickup \textit{increased} lateness---monetary framing reduced $\tau$ by crowding out social norms. This demonstrates $\gamma_{REG}$ can be negative under certain conditions \cite{gneezy2000}.

\item \textbf{Frey \& Jegen (2001)}: Formalized motivation crowding theory in the \textit{Journal of Economic Surveys}, explaining when external interventions enhance vs. undermine intrinsic motivation ($\approx \tau$) \cite{frey2001}.
\end{itemize}

\subsubsection{Situational Activation: State vs. Trait}

$\tau$ is not merely a stable trait but responds to situational cues:

\begin{itemize}
\item \textbf{Rand et al. (2012)}: Published in \textit{Nature}, demonstrated that faster decisions are more cooperative---suggesting that intuitive processing activates higher $\tau$ than deliberation. Time pressure shifts $\tau$ within the same person \cite{rand2012}.

\item \textbf{Rand (2016)}: Extended this to a dual-process framework in \textit{Psychological Science}: $\tau_{intuitive} \neq \tau_{deliberative}$. The decision mode itself modulates $\tau$ \cite{rand2016}.

\item \textbf{Peysakhovich \& Rand (2016)}: Introduced ``social heuristics'' in \textit{Management Science}---people develop stable cooperative dispositions ($\tau_0$) but these are situationally activated, supporting our $\tau = \tau_0 + \Delta\tau(\Psi)$ decomposition \cite{peysakhovich2016}.
\end{itemize}
...
\item \textbf{Bénabou \& Tirole (2011)}: Analyzed how identity concerns create moral behavior and taboos, with self-image serving as a commitment device for high-$\tau$ choices \cite{benabou2011}.
...
\item \textbf{Bicchieri (2006)}: Established that norms activate conditionally based on context, directly supporting context-dependent $\tau$ \cite{bicchieri2006}.
\end{itemize}

\subsubsection{LLM-Based Behavioral Research: Methodological Validation}

The use of LLMs for $\tau$-estimation is supported by emerging validation studies:
...
\subsubsection{Summary: Empirical Support for $\tau(\Psi)$}

\begin{table}[htbp]
\centering
\caption{Empirical Evidence Mapping to $\tau$-Parameters}
\label{tab:tau-evidence}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}p{4.5cm}p{4cm}p{4cm}@{}}
\toprule
\textbf{Empirical Finding} & \textbf{Key Studies} & \textbf{$\tau$-Implication} \\
\midrule
GARP violations across contexts & Choi et al., Fisman et al. & $\tau$ varies with $\Psi$ \\
Cross-cultural cooperation differences & Henrich et al., Herrmann et al. & $\tau_0$ is culturally shaped \\
...
Fast decisions more cooperative & Rand et al. & $\tau$ is state-dependent \\
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Society} & \textbf{MI} & \textbf{UG Offer} & $\hat{\tau}$ \\
\midrule
...
\end{equation}
where the factor 0.5 converts $\tau$ to expected fair-share offers.
\end{corollary}
...
\end{equation}
where $KII \in [0,1]$ measures the strength of kin-based social organization. High KII societies restrict high-$\tau$ interactions to in-group members.
\end{axiom}
...
Kinship Intensity determines the \textbf{boundary} of high-$\tau$ interactions:
\begin{align}
\text{High KII:} \quad &\tau_{ingroup} \gg \tau_{outgroup} \\
\text{Low KII:} \quad &\tau_{ingroup} \approx \tau_{outgroup}
\end{align}
\end{proposition}
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Region} & \textbf{Cousin Marriage \%} & \textbf{KII} & $\hat{\tau}_{outgroup}$ \\
\midrule
...
KII provides a macro-level context parameter that modulates the \textbf{scope} rather than the \textbf{level} of $\tau$:

\begin{definition}[Scope-Adjusted $\tau$]
\label{def:scope-tau}
The effective $\tau$ toward a specific interaction partner depends on both baseline $\tau$ and the in-group/out-group distinction:
\begin{equation}
\tau_{eff}(partner) = \begin{cases}
\tau_0 + \Delta\tau(\Psi) & \text{if } partner \in \text{in-group} \\
(1 - KII) \cdot [\tau_0 + \Delta\tau(\Psi)] & \text{if } partner \in \text{out-group}
\end{cases}
\end{equation}
\end{definition}
...
Primary effect & $\tau$ level & $\tau$ scope \\
...
\begin{equation}
\tau_{outgroup} \approx \alpha + \beta_1 \cdot MI - \beta_2 \cdot KII
\end{equation}
...
\begin{tabular}{@{}lcl@{}}
\toprule
\textbf{$\Psi$-Dimension} & \textbf{To increase $\tau$} & \textbf{Intervention Examples} \\
\midrule
...
\end{itemize}
Expected effect: $\Delta\tau \approx +0.3$ to $+0.5$
\end{example}
...
\textbf{Calculate $\tau$:}
\begin{align}
\tau_A &= \tau_0 + \gamma_{SEG} \cdot \Psi_{SEG} + \gamma_{REG} \cdot \Psi_{REG} + \gamma_{AWX} \cdot \Psi_{AWX} + \gamma_{IDN} \cdot \Psi_{IDN} \nonumber \\
...
WAX_A &= \tau_A \cdot \min\{U\} + (1-\tau_A) \cdot \sum U \nonumber \\
...
\textbf{Calculate new $\tau$:}
\begin{align}
\tau_B &= 0.35 + 0.25 \times 0.7 + 0.30 \times 0.8 + 0.20 \times 0.8 + 0.15 \times 0.7 \nonumber \\
...
\begin{enumerate}
\item \textbf{$\tau$ increases} (HOW she thinks): from additive to complementary
...
\textbf{$\tau$ = HOW I think}
\begin{itemize}
\item Low $\tau$: ``What's in it for ME? What's in it for the environment?'' (separate questions)
\item High $\tau$: ``This only makes sense as a WHOLE --- me, Zürich, the future''
\end{itemize}
\textit{Metaphor:} Low $\tau$ = buffet (I take what I like); High $\tau$ = family dinner (we eat together what works for everyone)
...
\subsubsection{Why $\tau = 1$ Is Not Always Better}

\begin{proposition}[Complementarity Trade-off]
Higher $\tau$ makes the decision criterion \textbf{stricter}:
\begin{align}
\text{Additive } (\tau = 0): \quad WAX &= U_1 + U_2 + U_3 \quad \text{(strengths compensate weaknesses)} \\
\text{Complementary } (\tau = 1): \quad WAX &= \min\{U_1, U_2, U_3\} \quad \text{(weakest link determines all)}
\end{align}
\end{proposition}
...
This explains why someone with high $\tau$ may \textbf{not} do something even though it seems ``rational'' --- because one dimension doesn't fit.
...
Unlike the $\gamma$-coefficients that modulate $\tau$ (how one thinks), the $\delta$-coefficients modulate $\theta$ (the barrier to action).
...
\begin{table}[htbp]
\centering
\caption{Comparison: $\gamma$ (for $\tau$) vs. $\delta$ (for $\theta$)}
\label{tab:gamma-vs-delta}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Aspect} & \textbf{$\gamma$ (affects $\tau$)} & \textbf{$\delta$ (affects $\theta$)} \\
\midrule
Modulates & HOW one thinks & HOW HIGH the barrier \\
Effect direction & $\tau \uparrow$ helps if U's aligned & $\theta \downarrow$ always helps \\
...
\textbf{Step 1: Calculate $\tau$}
\begin{equation}
\tau = \tau_0 + \sum_j \gamma_j \cdot \Psi_j
\end{equation}
...
\begin{equation}
WAX = \tau \cdot \min\{U_{INU}, U_{KNU}, U_{IDU}\} + (1-\tau) \cdot (U_{INU} + U_{KNU} + U_{IDU})
\end{equation}
...
WAX-10.3 & WAX-A90 & Tau Estimation & 10: Measurement \\
\midrule
...
WAX-12.2 & WAX-A97 & Tau Path & 12: Meta \\
...
\begin{equation}
\boxed{WAX(t) = \tau_t \cdot \min_i\{U_i^{eff}\} + (1-\tau_t) \cdot \sum_i U_i^{eff}}
\label{eq:wax-master}
\end{equation}

where:
\begin{itemize}
\item $\tau_t \in [0,1]$ is the context-dependent fusion parameter
...
\begin{equation}
\frac{d\tau}{dt} = \alpha \cdot AWX - \beta \cdot UNS_{eff} - \gamma \cdot KOM + \sigma \cdot dW_t
\label{eq:tau-dynamics}
\end{equation}
...
\begin{equation}
\boxed{WAX = \tau \cdot \min\{U^*\} + (1-\tau) \cdot \sum U^*}
...
$\tau \rightarrow 1$ & \textbf{``I only act if EVERYTHING is right''}\\
...
$\tau \rightarrow 0$ & \textbf{``I look at the total benefit''}\\
...
\textbf{Anna ($\tau = 0.35$):} Mostly additive, but not entirely. Good financial benefits can partially compensate for weak environmental awareness, but she still needs \textit{some} minimum across all dimensions.
\end{intuition}
...
\begin{itemize}
\item Increase $\tau$ (shift mindset): ``The Müllers next door got one too!''
...
\begin{align*}
\text{Thinking style:} \quad & \tau = 0.35 \quad \text{(mostly additive)}\\
...
\item ``The Müllers got one!'' (social norm) $\rightarrow$ $\tau$: $0.35 \rightarrow 0.55$
...
\text{New thinking:} \quad & \tau = 0.55 \quad \text{(more complementary)}\\
...
\begin{align*}
WAX &= \tau \cdot \min\{AWX \cdot U\} + (1-\tau) \cdot \sum(AWX \cdot U) && \text{(Willingness)}\\
...
\factor{Formal Statement}
\begin{equation}
WAX = \tau \cdot WAX_{compl} + (1-\tau) \cdot WAX_{add}
\end{equation}
where $WAX_{compl} = \min_i\{U_i^{eff}\}$ and $WAX_{add} = \sum_i U_i^{eff}$.

\factor{$\Psi$-Sensitivity}
\begin{tabular}{lll}
AWX & $\uparrow \Rightarrow \tau \uparrow$ & Awareness activates norm considerations \\
UNS & $\uparrow \Rightarrow \tau \downarrow$ & Uncertainty triggers calculative mode \\
KOM & $\uparrow \Rightarrow \tau \downarrow$ & Complexity induces INU-focus \\
SEG & $\uparrow \Rightarrow \tau \uparrow$ & Strong groups reinforce norms \\
IDN & $\uparrow \Rightarrow \tau \uparrow$ & Salient identity increases norm-binding \\
INA & Modulates & Power structures affect decision logic \\
ZHO & $\uparrow \Rightarrow \tau \uparrow$ & Long horizons favor norm-following \\
REG & $\uparrow \Rightarrow \tau \uparrow$ & Strong regulation reinforces complementary logic \\
\end{tabular}
...
\factor{Testable Prediction}
Under time pressure, $\tau$ increases. $\frac{\partial \tau}{\partial TimePressure} > 0$
...
\factor{Intervention Lever}
$\tau \uparrow$: Increase norm salience, activate group identity.
$\tau \downarrow$: Provide clear information, reduce complexity.
...
\factor{Formal Statement}
\begin{equation}
\tau = 0 \Rightarrow WAX = WAX_{add} = \sum_i \alpha_i \cdot U_i^{eff}
\end{equation}
Pure compensatory logic; trade-offs fully possible.

\factor{$\Psi$-Sensitivity}
\begin{tabular}{lll}
UNS & $\uparrow$ & Pushes toward $\tau = 0$ \\
KOM & $\uparrow$ & Pushes toward $\tau = 0$ \\
...
\factor{Intervention Lever}
Create conditions for $\tau \rightarrow 0$: reduce norm salience, simplify, emphasize individual benefits
...
\factor{Formal Statement}
\begin{equation}
\tau = 1 \Rightarrow WAX = WAX_{compl} = \min_i\{U_i^{eff}\}
\end{equation}
Pure non-compensatory logic; weakest link determines outcome.

\factor{$\Psi$-Sensitivity}
\begin{tabular}{lll}
AWX & $\uparrow$ & Pushes toward $\tau = 1$ \\
SEG & $\uparrow$ & Strong groups push toward $\tau = 1$ \\
IDN & $\uparrow$ & Salient identity pushes toward $\tau = 1$ \\
REG & $\uparrow$ & Strong regulation pushes toward $\tau = 1$ \\
\end{tabular}
...
\factor{Formal Statement}
\begin{equation}
0 < \tau < 1 \Rightarrow WAX = \tau \cdot \min\{U_i^{eff}\} + (1-\tau) \cdot \sum U_i^{eff}
\end{equation}
Hybrid: partial compensation with binding constraints.

\factor{$\Psi$-Sensitivity}
All 8 Ψ-dimensions modulate $\tau$ within $(0,1)$ range.
...
\factor{Testable Prediction}
Most real decisions show $\tau$ between 0.3 and 0.7.
\textbf{Test}: Estimate $\tau$ from field data; expect distribution centered away from extremes.

\factor{Interactions}
WAX-A27 to WAX-A29, WAX-A31, WAX-A97

\factor{Intervention Lever}
Fine-tune $\tau$ through context design; accept that pure cases are rare
...
\begin{axiomspec}{WAX-A31}{Endogenität von $\tau$}

\factor{Formal Statement}
\begin{equation}
\tau = \tau(\Psi) = \tau(Norms, Institutions, Stakes, Complexity)
\end{equation}
$\tau$ is not a fixed parameter but endogenously determined by context.

\factor{$\Psi$-Sensitivity}
\begin{tabular}{lll}
All 8 dimensions & Determine $\tau$ & $\tau$ is a function of the full Ψ-vector \\
\end{tabular}
...
\factor{Testable Prediction}
$\tau$ varies systematically across cultures, institutions, and decision contexts.
\textbf{Test}: Cross-cultural experiments; measure $\tau$ variation with context factors.
...
\factor{Formal Statement}
Salient norms increase weight $\tau$ on collective utility:
\begin{equation}
...
\factor{Formal Statement}
\begin{equation}
\frac{d\tau}{dt} \uparrow \text{ with digitalization}
\end{equation}
...
\factor{Formal} $\tau > 1$ in prospect theory: losses loom larger
...
\factor{Formal} Each segment $s$ has its own $\tau_s$
...
\factor{Formal} $\frac{d\tau}{dt} = \mu(\tau^* - \tau)$
...
\factor{Formal} $d\tau = \mu dt + \sigma dW$
...
\factor{Formal} $AWX > \theta_{extreme} \Rightarrow \tau \downarrow\downarrow$
...
\factor{Formal} Without anchoring: $\tau \rightarrow \tau_0$
...
\begin{axiomspec}{WAX-A90}{Tau Estimation}
\factor{Formal} $\hat{\tau} = \frac{WAX_{obs} - WAX_{add}}{WAX_{compl} - WAX_{add}}$
...
\factor{Formal} $\frac{\partial \tau}{\partial Inc_{fin}} > 0$ if $Norm > \theta_{norm}$
...
\begin{axiomspec}{WAX-A97}{Tau Path}
\factor{Formal} $\frac{d\tau}{dt} = \alpha \cdot AWX - \beta \cdot UNS_{eff} - \gamma \cdot KOM$
...
\factor{Formal} $WAX_t = \bar{WAX} + A \cdot \sin(\omega t + \tau)$ periodic patterns
...
%------------------------------------------------------------------------------
\subsection{CF-2: ``The hybrid function $\tau$ is unfalsifiable''}
%------------------------------------------------------------------------------

\textbf{The Objection:} Any observed behavior can be ``explained'' by adjusting $\tau$. If someone acts additively, set $\tau = 0$. If complementarily, set $\tau = 1$. This is post-hoc fitting, not prediction.

\textbf{Response:} $\tau$ is empirically measurable and context-dependent:
\begin{itemize}[nosep]
    \item Measurable via CCEI (Afriat 1972, Varian 1982)
    \item Predictably shifts with $\Psi$-dimensions (WAX-A21 to WAX-A28)
    \item Pre-registered predictions: high $\Psi_{UNS}$ $\to$ lower $\tau$
\end{itemize}
The framework makes falsifiable predictions about when and how $\tau$ changes.
...
\begin{itemize}[nosep]
    \item $\Psi_{UNS} \uparrow$ $\Rightarrow$ $\tau \downarrow$, $\theta \uparrow$ (WAX-A22)
...
\begin{itemize}[nosep]
    \item $\tau \to 0$ captures heuristic/automatic processing
...
%------------------------------------------------------------------------------
\subsection{OI-1: Neural correlates of $\tau$-switching}
%------------------------------------------------------------------------------

\textbf{The Question:} What neural mechanisms underlie the shift between additive ($\tau \to 0$) and complementary ($\tau \to 1$) processing?
...
%------------------------------------------------------------------------------
\subsection{OI-3: Cross-cultural variation in $\tau_{base}$}
%------------------------------------------------------------------------------

\textbf{The Question:} Do collectivist vs. individualist cultures have systematically different baseline $\tau$ values?

\textbf{Hypothesis:} Collectivist cultures: higher $\tau_{base}$ (more complementary thinking).

\textbf{Research Direction:} Cross-cultural experiments with matched stimuli measuring $\tau$ via CCEI.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CORE COMPLIANCE: 10C CROSS-REFERENCE MAP
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{10C CORE Cross-Reference Map}
...
\begin{table}[htbp]
\centering
\caption{AV (CORE-READY) Integration with 10C CORE Framework}
\label{tab:av-9c-integration}
\renewcommand{\arraystretch}{1.4}
...
\textbf{B} (HOW) & Wie interagieren? & $\gamma$ complementarity & $\tau$ operationalization \\
...
\subsection{Integration with Appendix TKT (METHOD-TOOLKIT)}
\label{sec:av-hhh-integration}

\begin{tcolorbox}[colback=orange!5!white, colframe=orange!75!black,
    title=\textbf{HHH Integration: Intervention Design Toolkit}]

Appendix TKT (METHOD-TOOLKIT) provides the systematic framework for designing
...
\begin{equation}
WAX_{\text{post}} = WAX_{\text{pre}} + E(\mathcal{I}) \cdot \mu(\tau, \tau) \cdot \sigma_s
\label{eq:av-hhh-wax-change}
\end{equation}
where:
\begin{itemize}[nosep]
    \item $E(\mathcal{I})$ = intervention effectiveness from HHH
    \item $\mu(\tau, \tau)$ = type-phase alignment factor (HHH Axiom HHH-A3)
    \item $\sigma_s$ = segment multiplier (HHH Axiom HHH-A5)
\end{itemize}

\textbf{Cross-Reference:} See Appendix TKT Section 2 (Nine Intervention Types) for complete taxonomy
...
$\tau$ & Tau (Thinking Style) & 
``Wie denke ich?'' \\
& & $\tau \to 1$: ``Alles muss stimmen'' (verbunden) \\
& & $\tau \to 0$: ``Summe zählt'' (getrennt) \\
...
& & $\tau = \tau_0 + \Delta\tau(\Psi)$ \\[6pt]

$\gamma_j$ & Gamma (Tau-Sensitivity) & 
``Wie stark beeinflusst $\Psi_j$ mein $\tau$?'' \\
& & $\Delta\tau = \sum_j \gamma_j \cdot \Psi_j$ \\[6pt]
...
& & Hoch $\to$ $\tau \downarrow$, $\theta \uparrow$ \\[4pt]
...
& & Hoch $\to$ $\tau \uparrow$ (Normen wichtiger) \\[4pt]

$\Psi_{IDN}$ & Identity & 
``Wie zentral ist das für mein Selbstbild?'' \\
& & Hoch $\to$ $\tau \uparrow$, $U_{IDU} \uparrow$ \\[4pt]
...
\begin{equation*}
WAX = \tau \cdot \min\{U^*\} + (1-\tau) \cdot \sum U^* \quad \text{wobei } U^* = AWX \cdot U
\end{equation*}
...
\begin{table}[htbp]
\centering
\caption{Summary of Key Results in Appendix UNMAPPED_REA}
...
WAX-R2 & Hybrid Function & $WAX = \tau \cdot \min + (1-\tau) \cdot \sum$ \\
...
HOW ($\tau$ fusion), WHEN ($\Psi$ context), WHERE (parameter estimation), AWARE (salience filter).
...
\begin{table}[htbp]
\centering
\caption{Appendix UNMAPPED_REA Summary}
...
$\tau$ & Fusion parameter (complementary vs. additive) & Definition \ref{def:tau} \\
...
$\Psi$ & Context vector (8 dimensions) & Appendix CTW \\
$AWX$ & Awareness function & Appendix UNMAPPED_AWA \\
...
\textbf{See also:} Appendix UNMAPPED_GLS (Central Glossary and Notation) for complete symbol definitions across all EBF appendices.
...
LLM-MC methods (Appendix LLM1) provide practical estimation approaches.
...
\paragraph{Issue 2: Dynamic $\tau$ Estimation.}
The fusion parameter $\tau$ may vary not just across individuals but
within individuals over time. Real-time estimation of $\tau$ from
...
suggests $\tau$ relates to prefrontal-limbic balance, but more research
is needed.
...
\section{References for Appendix UNMAPPED_REA}
\label{sec:av-references}

\begin{itemize}[nosep]
    \item \citet{kahneman2011}: System 1/2 foundation for $\tau$ dynamics
    ...
    \item \citet{thaler2008}: Nudge theory and threshold manipulation
\end{itemize}
...
\textbf{Depends on:}
\begin{itemize}[nosep]
    \item Appendix UNMAPPED_WHO (CORE-WHO): Utility holders across levels
    \item Appendix UNMAPPED_WAT (CORE-WHAT): Utility dimensions FEPSDE
    \item Appendix UNMAPPED_HOW (CORE-HOW): Complementarity matrix $\Gamma$
    \item Appendix CTW (CORE-WHEN): Context $\Psi$ specification
    \item Appendix UNMAPPED_EST (CORE-WHERE): Parameter calibration
    \item Appendix UNMAPPED_AWA (CORE-AWARE): Awareness function $A(\cdot)$
\end{itemize}
...
\textbf{Reading Path:}
$\rightarrow$ Appendix UNMAPPED_AWA (awareness) $\rightarrow$ This Appendix (willingness)
