...
\title{Appendix UNMAPPED_PAP: Formal Framework for Intervention Equilibria and Tipping Points}
...
Following Appendix UNMAPPED_FRM (LIT-FEHR-METHOD), we note:
...
\begin{itemize}[nosep]
    \item Given a behavioral change context, how do we operationalize equilibrium stability $\xi$
and critical threshold $\kappa_{\text{crit
\end{itemize}

\textbf{In-Scope:}
\begin{itemize}[nosep]
    \item The Three Counterfactual Equilibria
    \item Operationalizing Equilibrium Stability: $\xi$
    \item Operationalizing Critical Threshold: $\kappa_{\text{crit
    \item Mathematical Model: Non-Linear Decay with Context-Dependent $\lambda(\kappa)$
    \item Diagnostic Tool: Questionnaire + Calculator
\end{itemize}

\textbf{Out-of-Scope (delegiert an andere Appendices):}
\begin{itemize}[nosep]
    \item LIT-FEHR-METHOD $\rightarrow$ Appendix UNMAPPED_FRM
    \item METHOD-TOOLKIT $\rightarrow$ Appendix TKT
    \item FORMAL-NC $\rightarrow$ Appendix UNMAPPED_NC
    \item Willingness $\rightarrow$ Appendix UNMAPPED_AV
\end{itemize}
...
This Appendix UNMAPPED_PAP provides the \textbf{diagnostic and strategic foundation}:

\begin{enumerate}
    \item \textbf{Phase 1 – Characterize Your Context (Appendix UNMAPPED_PAP):}
    \begin{itemize}
    ...
    \item \textbf{Phase 2 – Segment Your Population (Chapter 14 / Appendix PAP1):}
    \begin{itemize}
    ...
    \item \textbf{Phase 3 – Understand Journey Dynamics (Chapter 13 / Appendix UNMAPPED_STA):}
    \begin{itemize}
...
Recall W11-W13 (Appendix UNMAPPED_REA):
...
\textbf{Cross-Reference:} See Appendix TKT Section 3 (20-Field Schema) for complete specification.
...
This Appendix UNMAPPED_PAP rests on:
...
See \textbf{Appendix UNMAPPED_GLS} (\textit{Glossary and Notation}) for comprehensive definitions.
...
\begin{itemize}[nosep]
    ...
    \item W11, W12, W13 --- Dynamic Modulation Axioms (from Appendix UNMAPPED_REA)
\end{itemize}
...
\begin{itemize}
    \item Appendix UNMAPPED_REA: Willingness Formalization (W11-W13)
    \item Appendix UNMAPPED_STA: Behavioral Change Journey (5-stage model)
    ...
    \item Appendix PAP1: Formal Segmentation (axioms of segment emergence)
...
\label{sec:equ-crossref}

\begin{tcolorbox}[colback=gray!5!white, colframe=gray!50!black]
\textbf{Critical Link:} \textbf{Appendix UNMAPPED_NC (FORMAL-NC)}---Non-Concavity, Fit, and Coherence.

Multiple equilibria in intervention systems emerge from the same mathematical structure formalized in NC:
\begin{itemize}[nosep]
\item $\gamma > 0$ creates non-concave landscapes $\Rightarrow$ multiple stable states (equilibria)
\item Tipping points correspond to saddle points between peaks
\item $\kappa_{\text{crit}}$ thresholds mark valley boundaries in the landscape
\item Coherence $K$ and quality $Q$ determine which equilibrium is optimal
\end{itemize}

\textbf{Related Appendices:}
\begin{itemize}[nosep]
\item B (CORE-HOW): Complementarity $\gamma$ as the core mechanism
\item VI (FORMAL-MEQ): Multiple Equilibria Theorem
\item HIE: Decision Hierarchy (L0-L3 structure)
\end{itemize}
\end{tcolorbox}
