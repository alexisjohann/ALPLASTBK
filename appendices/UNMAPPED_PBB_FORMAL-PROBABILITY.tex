...
\title{Appendix UNMAPPED_PBB: FORMAL-PROBABILITY\\
The 10-Axiom Probabilistic Foundation for Behavioral Change}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Appendix UNMAPPED_PAP (FORMAL-EQUILIBRIA):} Tipping points and critical thresholds
    \item \textbf{Appendix UNMAPPED_REA (CORE-READY):} Willingness threshold dynamics (P3 anchor)
    \item \textbf{Appendix UNMAPPED_STA (CORE-STAGE):} Journey-stage probability transitions
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Appendix UNMAPPED_WAT (FEPSDE):} Utility dimensionality (6 dimensions of net utility N)
    \item \textbf{Appendix UNMAPPED_GLS (Glossary):} Definitions of all axiom terms
    \item \textbf{Appendix UNMAPPED_AWA (CORE-AWARE):} Awareness measurement and modulation
...
\textbf{Usage:} See Chapter 16 for practical application; Appendix UNMAPPED_GLS for glossary.
\end{tcolorbox}
...
\begin{itemize}[nosep]
    \item FORMAL-EQUILIBRIA $\rightarrow$ Appendix UNMAPPED_PAP
...
\textbf{Evidence:} Higher net utility (combining instrumental, knowledge, identity dimensions) increases motivation for action. See Appendix UNMAPPED_WAT (FEPSDE Matrix).
...
\textbf{Formalization:} Social preference component $\omega^S$ in FEPSDE Matrix (Appendix UNMAPPED_WAT) and Behavioral Change Journey (AW) stage dynamics.
...
%------------------------------------------------------------------------------
\subsection{Integration with Appendix TKT: Intervention Effects on Probability}
...
\item Domain-specific calibration recommended (see Appendix UNMAPPED_BBB for methods)
...
George Ainslie demonstrated that temporal discounting follows a hyperbolic rather than exponential form \citep{ainslie1975specious}:
\begin{equation}
...
\item Network externalities \citep{rogers2003diffusion}
\end{itemize}
...
Technology adoption follows an S-curve driven by innovators → early adopters → majority → laggards \citep{rogers2003diffusion}:
\begin{equation}
...
\end{itemize}
Institutional change is typically slow but can exhibit punctuated equilibrium \citep{north1990institutionsinstitutions}.
...
\textbf{Usage:} These are order-of-magnitude estimates. Domain-specific calibration required via Appendix UNMAPPED_BBB methods.
...
\item Loss aversion amplifies inertia: $\lambda \approx 2$ \citep{kahneman1979prospectprospect, tversky1991loss}
\end{itemize}
...
\item Salience decay without reinforcement \citep{bordalo2012salience}
\end{itemize}
...
\item Contract design exploiting time inconsistency \citep{dellavigna2004contract}
...
\item Identity-based preferences become stable \citep{akerlof2000identity}
...
\item Market and technology dynamics \citep{rogers2003diffusion}
...
\begin{tcolorbox}[colback=orange!5!white, colframe=orange!75!black,
    title=\textbf{Cross-Reference: UNTCM moved to Appendix UNMAPPED_UN}]

\textbf{The complete Unified NTM-Context Model (UNTCM) is now documented in:}

\begin{center}
\Large\textbf{Appendix UNMAPPED_UN: FORMAL-UNTCM}
\end{center}

\textbf{UNTCM Content in Appendix UNMAPPED_UN:}
\begin{itemize}[nosep]
...
\textbf{Key Connection:} PBB provides the foundational NTM axioms (A1-A4) and theorems (T1-T5). Appendix UNMAPPED_UN extends this to the \emph{unified} model that couples NTM dynamics with context evolution.

\vspace{0.5em}
\textit{See Appendix UNMAPPED_UN (FORMAL-UNTCM) for the complete formalization.}
...
For definitions of key terms used in this appendix, see **Appendix UNMAPPED_GLS (REF-GLOSSARY)** which provides formal definitions for:
\begin{itemize}[nosep]
