...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=Quick Reference: Appendix UNMAPPED_SE]
...
\begin{itemize}[nosep]
    ...
    \item REF-GLOSSARY $\rightarrow$ Appendix G
    \item LIT-BECKER $\rightarrow$ Appendix UNMAPPED_GB
\end{itemize}
...
\label{ax:se-1}
\textbf{UNMAPPED_SE-1:} Dominant group members derive utility from relative group position and will invest resources to maintain hierarchies:
\begin{equation}
...
\textbf{EBF Integration:} Axiom UNMAPPED_SE-1 connects to CORE-WHAT (C) through positional utility and to CORE-WHO (AAA) through group-level welfare. Unlike Becker's exogenous taste, SE treats discrimination as endogenous to positional competition.

\textbf{Key Distinction from Becker (UNMAPPED_GB-2):}
\begin{itemize}
...
\label{ax:se-2}
\textbf{UNMAPPED_SE-2:} Wealth gaps exceed income gaps because:
\begin{enumerate}
...
\textbf{EBF Integration:} Axiom UNMAPPED_SE-2 explains why income-focused interventions (education, job training) fail to close wealth gaps---the bequest term $B_t$ perpetuates historical advantages.
...
\label{ax:se-3}
\textbf{UNMAPPED_SE-3:} Intergroup inequality compounds across generations through multiple mechanisms:
\begin{enumerate}
...
\textbf{EBF Integration:} Axiom UNMAPPED_SE-3 connects to CORE-STAGE (AW)---the ``behavioral change journey'' for subordinate groups is constrained by structural barriers, not just individual motivation.
...
\label{ax:se-4}
\textbf{UNMAPPED_SE-4:} Effective policy addressing intergroup inequality requires three components:
\begin{enumerate}
...
\label{ax:se-5}
\textbf{UNMAPPED_SE-5:} Baby Bonds address wealth inequality by:
\begin{enumerate}
...
The 2026 GSE framework \citep{aja2026global} extends SE internationally:

\begin{axiom}[Transnational Stratification Mechanisms]
\label{ax:se-6}
\textbf{UNMAPPED_SE-6:} Global stratification operates through:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item Problem: Even with equal education, wealth gap persists (UNMAPPED_SE-2)
...
\begin{itemize}
    ...
    \item Problem: Discrimination persists despite laws (UNMAPPED_SE-1)
...
\begin{itemize}
    ...
    \item Addresses: UNMAPPED_SE-2 (wealth vs. income), UNMAPPED_SE-3 (intergenerational transmission)
...
\begin{enumerate}
    \item \textbf{UNMAPPED_SE-1 (Group Position):} Dominant groups actively maintain hierarchies for positional utility
    \item \textbf{UNMAPPED_SE-2 (Wealth-Income):} Wealth gaps (10:1) exceed income gaps (2:1); different dynamics
    \item \textbf{UNMAPPED_SE-3 (Transmission):} Multiple mechanisms transmit advantage across generations
    \item \textbf{UNMAPPED_SE-4 (Three Rs):} Recognition, Redistribution, and Repair are all necessary
    \item \textbf{UNMAPPED_SE-5 (Baby Bonds):} Universal wealth grants can dramatically reduce gaps
    \item \textbf{UNMAPPED_SE-6 (GSE):} Global extension adds transnational mechanisms and historical genesis
\end{enumerate}
...
See Appendix G (REF-GLOSSARY) for term definitions and Appendix UNMAPPED_GB (LIT-BECKER) for comparison with taste-based discrimination theory.
