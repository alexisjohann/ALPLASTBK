...
% - Appendix UNMAPPED_GLS: Glossary definitions
...
full_text:
  ...
  content_level: L2
  structural_characteristics: [S1, S2, S3, S4]
...
\begin{itemize}[nosep]
    ...
    \item Case Registry schema $\rightarrow$ Appendix UNMAPPED_CAS
    \item Theory Catalog schema $\rightarrow$ Appendix UNMAPPED_MS
\end{itemize}
...
\textbf{Level Assignment (Structural):}
\begin{equation}
C(p) = \begin{cases}
L1 & \text{if } S_1 \in \phi_S(p) \land \texttt{ABSTRACT} \in \mathcal{T}_C(p) \\
L2 & \text{if } \{S_1, S_2, S_3, S_4\} \subseteq \phi_S(p) \land \texttt{KEY\_FINDINGS} \in \mathcal{T}_C(p)
\end{cases}
\end{equation}
...
\toprule
\textbf{Level} & \textbf{Name} & \textbf{Structural Req.} & \textbf{Content (Paper-Internal)} \\
\midrule
L0 & Metadata Only & No $S_i$ & Author, Title, Year, Journal, DOI \\
L1 & Research Question & $S_1$ & + Abstract, Research Question \\
L2 & Summary/Extract & $S_1$--$S_4$ & + Methodology, Sample, Key Findings \\
L3 & Full Text & $S_1$--$S_6$ + R1--R4 & + Complete original text, References \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=Critical: Content Level 3 Requirements (R1--R4)]
Content Level 3 requires \textbf{all four} conditions (R1--R4):

\begin{table}[H]
\centering
\begin{tabular}{@{}clp{6cm}@{}}
\toprule
\textbf{Req} & \textbf{Requirement} & \textbf{Validation} \\
\midrule
R1 & All original sections present & Section headings check \\
R2 & Complete references section & Reference count check \\
R3 & $>$10k words (article) / $>$5k (short paper) & \texttt{wc -w} on \texttt{.md} file \\
R4 & No EBF sections in fulltext & Grep for EBF keywords \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[nosep]
    \item \texttt{.md} file = ONLY original paper text (enforced by R4)
    \item \texttt{.yaml} file = ALL EBF metadata ($S_1$--$S_6$, \texttt{use\_for}, etc.)
\end{itemize}
...
\textbf{Key Insight:} Content Level is determined by \textbf{structural completeness}
($S_1$--$S_6$), not character count. A 2-page summary with $S_1$--$S_4$ is L2.
A 100-page document without methodology is L1.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=yellow!10!white, colframe=yellow!75!black,
    title=Learning INT-ACE-002: Always Check Main First]
...
Obtain full paper text including paper's own bibliography. Validate against R1--R4: all original sections (R1), complete references (R2), $>$10k words via \texttt{wc -w} (R3), no EBF sections in fulltext (R4).
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Complete formalization with axiom (e.g., UNMAPPED_RB-7)
    \end{itemize}

    \item \textbf{CORE Extensions} (only if 6-Factor positive)
    \begin{itemize}
        \item New axiom (e.g., UNMAPPED_CMP-11, AWX-A5)
...
\begin{itemize}
    \item CORE-HOW (B): UNMAPPED_CMP-11 ($\gamma$ sign reversal via narratives)
...
\begin{verbatim}
☐ Content Level validated (S1-S6 structural + R1-R4 for L3)
...
\midrule
INT-ACE-001 & LEVEL & Validate L3 against R1--R4 (structural completeness, not char count) \\
INT-ACE-002 & LEVEL & Check \texttt{origin/main} before fetching \\
INT-ACE-003 & LEVEL & Level 5 needs BOTH Appendix AND Chapter \\
INT-ACE-004 & LIT & Update \texttt{use\_for} after LIT assignment \\
INT-ACE-005 & BIBTEX & Use \texttt{\textbackslash citet\{\}} for subject, \texttt{\textbackslash citep\{\}} for parentheses \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Cross-Reference:} See Appendix UNMAPPED_BBB (CORE-WHERE) for parameter estimation
...
\begin{itemize}[nosep]
    \item Content L3: $S_1$--$S_6$ structural completeness + R1--R4 (full text)
...
\vspace{1em}
\textit{For complete symbol definitions, see Appendix UNMAPPED_GLS (Central Glossary).}
...
\begin{itemize}
    \item \textbf{Appendix UNMAPPED_GLS:} Central Glossary for term definitions
    \item \textbf{Appendix UNMAPPED_CAS:} Case Registry schema and structure
    \item \textbf{Appendix UNMAPPED_MS:} Theory Catalog and MS-XX-XXX format
    \item \textbf{LIT-* Appendices:} Literature reviews (LIT-FEHR, LIT-THALER, LIT-LEARNING, etc.)
    \item \textbf{Appendix UNMAPPED_BBB:} Parameter Registry for PAR-XXX entries
\end{itemize}
