...
\begin{itemize}[nosep]
\item Appendix UNMAPPED_AAA (CORE-WHO): Context varies by welfare level
\item Appendix UNMAPPED_C (CORE-WHAT): Context affects dimension weights $\alpha_d(\Psi)$
\item Appendix UNMAPPED_B (CORE-HOW): Context affects complementarity $\gamma(\Psi)$
\item Appendix UNMAPPED_AU (CORE-AWARE): Awareness depends on context
\end{itemize}
...
\textbf{Out-of-Scope:}
\begin{itemize}[nosep]
    \item What utility dimensions exist $\rightarrow$ Appendix UNMAPPED_C (CORE-WHAT)
    \item How components interact $\rightarrow$ Appendix UNMAPPED_B (CORE-HOW)
    \item Parameter estimation for $\Psi$ $\rightarrow$ Appendix UNMAPPED_BBB (CORE-WHERE)
...
by exposing agents to prosocial types (\citet{benabou2018narratives}).
...
\citet{becker1965theory}, who established time as an economic resource with
...
This framework (formalized as MS-TA-001 in the Theory Catalog, UNMAPPED_CAT-17) explains
phenomena from female labor supply growth to digital platform competition
(see Appendix UNMAPPED_PT, CAS-TA-001, UNMAPPED_CAS-853).
...
to goods-intensive activities (\citet{becker1965theory}).
...
\textbf{Cross-Reference:} See Appendix UNMAPPED_CP (DOMAIN-CPS) for the complete 10C-CPS mapping and intervention recommendations.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Appendix UNMAPPED_V Summary: CORE-WHEN]
...
\textbf{Next Steps:}
See Appendix UNMAPPED_BBB (CORE-WHERE) for parameter estimation methods.
See Appendix UNMAPPED_CAL (METHOD-LLMMC) for calibration procedures.
...
\begin{table}[htbp]
\centering
\caption{Symbols Introduced in Appendix UNMAPPED_V}
...
Empirical calibration (Appendix UNMAPPED_BBB) provides default values from meta-analyses.
...
\item \textbf{Network Structure:} \citet{benabou2018narratives} --- Network parameter $\rho$ and moral transmission
...
Appendix UNMAPPED_AU (CORE-AWARE: Moral Narratives \& Network Transmission),
Appendix UNMAPPED_RB (LIT-BENABOU: Network Parameter $\rho$),
