...
EVIDENCE_TIER_MULTIPLIER = {
    ...
    4: 0.4,    # Tier 4: Expert elicitation, limited evidence
    5: 0.3,    # Tier 5: Peripheral, methodology-only
...
def compute_gap_vector(base_path: Path) -> Dict[str, float]:
        ...
        if theory_catalog_path.exists():
            ...
            if theories and 'categories' in theories:
                for cat in theories['categories']:
                    # Support both 'theories' and 'models' key names
                    models = cat.get('theories', cat.get('models', []))
                    for model in models:
                        total_theories += 1
    ...
    # parameter-registry.yaml uses domain-specific keys (behavioral_parameters,
    # contextual_parameters, etc.) — NOT a single 'parameters' key
    ...
    try:
        if param_registry_path.exists():
            ...
            total_params = 0
            params_with_evidence = 0
            skip_keys = {'metadata', 'superkey_index'}
            if params:
                    ...
                    if isinstance(val, list):
                        for item in val:
                            if isinstance(item, dict) and item.get('id'):
                                total_params += 1
                                if (item.get('source') or item.get('bib_key')
                                        or item.get('bib_keys') or item.get('reference')):
                                    params_with_evidence += 1
    ...
    # Uses grep-based counting as primary method (robust against YAML parse errors)
    ...
    if case_registry_path.exists():
        try:
            with open(case_registry_path) as f:
                for line in f:
                    if line.startswith('  CAS-') and ':' in line:
                        case_count += 1
        except Exception:
            case_count = 0
    gap['case'] = max(0, 1 - (case_count / case_target)) if case_count > 0 else 0.5

    # g_LIT: LIT-Appendix coverage from use_for fields in paper YAMLs
    # Counts how many papers are assigned to each LIT-appendix via use_for.
    # Previous approach (author substring search in BibTeX) was unreliable.
    lit_coverage = defaultdict(int)
    lit_targets = {
        'LIT-FEHR': 150, 'LIT-THALER': 100, 'LIT-KT': 150,
        'LIT-SUNSTEIN': 80, 'LIT-ARIELY': 60, 'LIT-CAMERER': 60,
        'LIT-LOEWENSTEIN': 50, 'LIT-LAIBSON': 40, 'LIT-RABIN': 30,
        'LIT-SHLEIFER': 30, 'LIT-BARBERIS': 40, 'LIT-O': 500,
        'LIT-META': 60, 'LIT-ENK': 30, 'LIT-SUT': 30, 'LIT-BLINDER': 20
    }

    paper_refs_path = base_path / 'data' / 'paper-references'
    if paper_refs_path.exists():
        for yaml_file in paper_refs_path.glob('PAP-*.yaml'):
            try:
                with open(yaml_file) as f:
                    paper = yaml.safe_load(f)
                # Check both top-level and ebf_integration.use_for
                use_for = ''
                if isinstance(paper.get('ebf_integration'), dict):
                    use_for = str(paper['ebf_integration'].get('use_for', ''))
                if not use_for:
                    use_for = str(paper.get('use_for', ''))
                uf_upper = use_for.upper()
                for lit in lit_targets:
                    if lit in uf_upper:
                        lit_coverage[lit] += 1
            except Exception:
                pass

    # Compute average coverage gap (not max — avoids single group dominating)
    if lit_targets:
        gaps = []
        for lit, target in lit_targets.items():
            coverage = lit_coverage.get(lit, 0)
            gaps.append(max(0, 1 - (coverage / target)))
        gap['LIT'] = sum(gaps) / len(gaps)
    else:
        gap['LIT'] = 0.3
...
def estimate_content_level(paper: Dict) -> Tuple[str, float]:
    ...
    """
    # Priority 0: Trust prior_score.content_level (SSOT after structural validation)
    # fix_content_level_fields.py has validated all levels using Definition 2 (structural)
    ps = paper.get('prior_score', {})
    if isinstance(ps, dict):
        ps_level = ps.get('content_level')
        if ps_level in ('L0', 'L1', 'L2', 'L3'):
            return ps_level, CONFIDENCE_MULTIPLIER[ps_level]

    # Priority 1: Respect explicitly set content_level in YAML
    explicit_level = paper.get('content_level')
    if explicit_level in ('L0', 'L1', 'L2', 'L3'):
        return explicit_level, CONFIDENCE_MULTIPLIER[explicit_level]

    # Also check full_text.content_level (alternative location)
    full_text_info = paper.get('full_text', {})
    if isinstance(full_text_info, dict):
        ft_level = full_text_info.get('content_level')
        if ft_level in ('L0', 'L1', 'L2', 'L3'):
            return ft_level, CONFIDENCE_MULTIPLIER[ft_level]

    # Priority 2: Estimate from available content
    abstract = paper.get('abstract', '') or ''
    summary = paper.get('summary', {}) or {}

    # Calculate character count from full_text
    char_count = 0
    if isinstance(full_text_info, dict):
        char_count = full_text_info.get('character_count', 0) or 0
        # Estimate from lines if character_count missing
        if char_count == 0 and full_text_info.get('lines'):
            char_count = (full_text_info.get('lines', 0) or 0) * 80
    ...
    # Check both summary.abstract AND summary.abstract_extended
    has_abstract = (bool(abstract) or
                    bool(summary.get('abstract', '')) or
                    bool(summary.get('abstract_extended', '')))
    has_findings = (bool(summary.get('key_findings', '')) or
                    bool(paper.get('key_findings_structured')) or
                    bool(paper.get('key_findings')))
    has_ebf_relevance = bool(summary.get('ebf_relevance', ''))

    # Include ebf_integration.notes (rich research summaries)
    ebf_integration = paper.get('ebf_integration', {}) or {}
    notes = str(ebf_integration.get('notes', '') or '')
    has_notes = len(notes) > 100

    # Calculate template char count if available
    template_chars = 0
    if summary:
        for key in ['abstract', 'abstract_extended', 'key_findings', 'ebf_relevance', 'sources']:
    ...
    # Include notes in total chars (notes are research summaries with S1-S4 content)
    notes_chars = len(notes)

    # Use maximum of available content
    total_chars = max(char_count, template_chars + notes_chars, len(abstract) + notes_chars)
    ...
    elif has_abstract and has_findings and has_notes:
        # Structural L2: Abstract (S1) + Findings (S4) + Notes with methodology (S2/S3)
        # This captures papers with rich structured metadata below char threshold
        return 'L2', CONFIDENCE_MULTIPLIER['L2']
...
def estimate_integration_level(paper: Dict) -> Tuple[str, float]:
    ...
    """
    # Priority 0: Trust prior_score.integration_level (SSOT after validation)
    ps = paper.get('prior_score', {})
    if isinstance(ps, dict):
        ps_il = ps.get('integration_level')
        if ps_il in INTEGRATION_LEVEL_NUMERIC:
            q_I = INTEGRATION_LEVEL_NUMERIC[ps_il] / 5.0
            return ps_il, q_I

    # Extract EBF-specific fields
    # ebf_integration takes precedence over top-level (which may come from
    # BibTeX merge in load_paper_combined and use non-MS-xxx formats)
    ebf = paper.get('ebf_integration', {}) or {}
    use_for = ebf.get('use_for', '') or paper.get('use_for', '') or ''
    evidence_tier = ebf.get('evidence_tier') or paper.get('evidence_tier', None)
    theory_support = ebf.get('theory_support', '') or paper.get('theory_support', '') or ''
    parameter = ebf.get('parameter', '') or paper.get('parameter', '') or ''
...
def estimate_supply_vector(paper: Dict, base_path: Path) -> Dict[str, float]:
    ...
    # Handle 'authors' as structured list (family/given dicts)
    if not author and 'authors' in paper:
        authors_field = paper['authors']
        if isinstance(authors_field, list):
            names = []
            for a in authors_field:
                if isinstance(a, dict):
                    names.append(a.get('family', a.get('given', '')))
                elif isinstance(a, str):
                    names.append(a)
            author = ' '.join(names)
        elif isinstance(authors_field, str):
            author = authors_field
    # ebf_integration takes precedence over top-level (BibTeX merge may add non-MS-xxx)
    ebf = paper.get('ebf_integration', {}) or {}
    use_for = ebf.get('use_for', '') or paper.get('use_for', '') or ''
    theory_support = ebf.get('theory_support', '') or paper.get('theory_support', '') or ''
    param_field = ebf.get('parameter', '') or paper.get('parameter', '') or ''

    # Normalize all fields to lowercase strings (some YAMLs have list types)
    def _to_str(val):
        if isinstance(val, list):
            return ' '.join(str(v) for v in val)
        return str(val) if val else ''

    title = _to_str(title).lower()
    abstract = _to_str(abstract).lower()
    author = _to_str(author).lower()
    use_for = _to_str(use_for).lower()
    theory_support = _to_str(theory_support).lower()
    param_field = _to_str(param_field)
...
def compute_prior_score(paper: Dict, gap: Dict[str, float],
    ...
    tau = EVIDENCE_TIER_MULTIPLIER.get(evidence_tier, EVIDENCE_TIER_MULTIPLIER[None])
    ...
    # Step 6: Determine decision using NORMALIZED score
    # π_max = Σ wᵢ · gᵢ is the theoretical maximum (all sᵢ=1, τ=1, ρ=1)
    # Normalization ensures thresholds are meaningful regardless of gap vector state
    pi = result['prior_score']
    pi_max = sum(WEIGHTS[dim] * gap[dim] for dim in WEIGHTS)
    pi_norm = pi / pi_max if pi_max > 0 else 0

    result['pi_max'] = round(pi_max, 4)
    result['pi_normalized'] = round(pi_norm, 4)

    if pi_norm >= THRESHOLDS['high_priority']:
        result['decision'] = 'HIGH_PRIORITY'
        result['target_integration'] = 'I5'
    elif pi_norm >= THRESHOLDS['standard']:
        result['decision'] = 'STANDARD'
        result['target_integration'] = 'I3-I4'
    elif pi_norm >= THRESHOLDS['minimal']:
        result['decision'] = 'MINIMAL'
        result['target_integration'] = 'I1-I2'
    elif pi_norm < THRESHOLDS['reject']:
    ...
    result['max_possible_score'] = round(pi_max, 4)
...
def load_bibtex_entry(paper_id: str, base_path: Path) -> Dict:
    ...
    # Try both PAP-xxx and xxx formats, all entry types
    entry_types = ['article', 'book', 'techreport', 'incollection', 'misc', 'unpublished', 'inproceedings']
    patterns = []
    for etype in entry_types:
        patterns.append(f'@{etype}{{{paper_id},')
        patterns.append(f'@{etype}{{PAP-{paper_id},')
    ...
    if 'prior_score:' in content:
        ...
        for line in lines:
            if line.startswith('prior_score:'):
                ...
                new_lines.append(f"  pi_normalized: {score_data.get('pi_normalized', 0)}")
...
prior_score:
  ...
  pi_normalized: {score_data.get('pi_normalized', 0)}
...
def main():
    ...
    if args.score:
        ...
        print(f"  π_norm = {result.get('pi_normalized', 0):.4f}  (π/π_max, used for classification)")
        print(f"  π_max = {result.get('pi_max', 0):.4f}")
        print(f"  Raw Score = {result['raw_score']:.4f}")
        print(f"  Evidence Tier τ = {result['evidence_tier']} (multiplier: {EVIDENCE_TIER_MULTIPLIER.get(result['evidence_tier'], EVIDENCE_TIER_MULTIPLIER[None])})")
