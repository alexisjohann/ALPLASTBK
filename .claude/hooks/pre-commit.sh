...
# ─────────────────────────────────────────────────────────────────────────
# TASK-LOG CHECK: Enforce PFLICHT-Workflow (3-Tier → Coding-Mode → Log)
# Severity scales with number of staged files:
#   ≤2 files: No warning (trivial change)
#   3-5 files: Gentle reminder
#   ≥6 files: Strong warning with file list
# ─────────────────────────────────────────────────────────────────────────
...
if [ -f "$TASK_LOG" ]; then
  ...
  if [ "$HAS_OPEN_TASK" -eq 0 ]; then
    # Count staged files (excluding auto-generated and trivial)
    STAGED_COUNT=$(git diff --cached --name-only --diff-filter=ACM | grep -v '00_appendix_index\|00_chapter_index' | wc -l)

    if [ "$STAGED_COUNT" -ge 6 ]; then
      echo "" >&2
      echo "┌─────────────────────────────────────────────────────────────────────────┐" >&2
      echo "│  ⚠️  TASK-LOG WARNUNG: $STAGED_COUNT Dateien ohne offenen Task!                    │" >&2
      echo "├─────────────────────────────────────────────────────────────────────────┤" >&2
      echo "│                                                                         │" >&2
      echo "│  Kein Task mit status: in_progress in data/task-log.yaml.              │" >&2
      echo "│  Bei ≥6 Dateien ist der PFLICHT-Workflow zwingend:                      │" >&2
      echo "│                                                                         │" >&2
      echo "│    1. 3-Tier-Box zeigen (Quick Win / Mittel / Gross)                    │" >&2
      echo "│    2. User-Bestätigung abwarten                                        │" >&2
      echo "│    3. Coding-Mode wählen (TRADITIONAL / EXPERIMENTAL)                   │" >&2
      echo "│    4. TL-Entry in task-log.yaml anlegen (status: in_progress)           │" >&2
      echo "│    5. DANN erst arbeiten                                                │" >&2
      echo "│                                                                         │" >&2
      echo "│  Staged files:" >&2
      git diff --cached --name-only --diff-filter=ACM | head -8 | while IFS= read -r line; do
        printf "│    • %-63s│\n" "$line" >&2
      done
      if [ "$STAGED_COUNT" -gt 8 ]; then
        printf "│    ... und %d weitere%-48s│\n" "$((STAGED_COUNT - 8))" "" >&2
      fi
      echo "│                                                                         │" >&2
      echo "│  Letzte Verletzung: TL-070 (SLA-Rewrite, alle 6 Schritte übersprungen)│" >&2
      echo "└─────────────────────────────────────────────────────────────────────────┘" >&2
      echo "" >&2
    elif [ "$STAGED_COUNT" -ge 3 ]; then
      echo "" >&2
      echo "┌─────────────────────────────────────────────────────────────────────────┐" >&2
      echo "│  💡 TASK-LOG REMINDER: $STAGED_COUNT Dateien, kein offener Task.                    │" >&2
      echo "├─────────────────────────────────────────────────────────────────────────┤" >&2
      echo "│  Pflicht-Workflow: 3-Tier-Box → Coding-Mode → Task loggen → Arbeiten  │" >&2
      echo "│  Siehe CLAUDE.md: Experiment-First + Empfehlungs-Workflow               │" >&2
      echo "└─────────────────────────────────────────────────────────────────────────┘" >&2
      echo "" >&2
    fi
    # ≤2 files: no warning (trivial commits like typos, single fixes)
  fi
fi
...
# ─────────────────────────────────────────────────────────────────────────
# KNOWLEDGE CONSISTENCY: Validate chat-insights against canonical SSOTs
# Checks for BCM name errors, FEPSDE errors, Ψ-dimension errors
# ─────────────────────────────────────────────────────────────────────────
STAGED_CHAT_INSIGHTS=$(git diff --cached --name-only --diff-filter=ACM | grep 'data/knowledge/chat-insights/.*\.yaml$' || true)

if [ -n "$STAGED_CHAT_INSIGHTS" ]; then
  echo "" >&2
  echo "=== EBF PreCommit: Knowledge Consistency Check ===" >&2

  if [ -f "$CLAUDE_PROJECT_DIR/scripts/validate_knowledge_consistency.py" ]; then
    KC_RESULT=$(python "$CLAUDE_PROJECT_DIR/scripts/validate_knowledge_consistency.py" --strict 2>&1)
    KC_STATUS=$?

    if [ $KC_STATUS -eq 1 ]; then
      echo "❌ KNOWLEDGE CONSISTENCY FAILED: Chat-insights contradict canonical SSOTs" >&2
      echo "$KC_RESULT" | grep -E '(❌|Correct:|SSOT:)' | head -15 >&2
      echo "" >&2
      echo "   Auto-fix: python scripts/validate_knowledge_consistency.py --fix" >&2
      echo "   Details:  python scripts/validate_knowledge_consistency.py" >&2
      FAILED=1
    else
      echo "✅ All chat-insights consistent with canonical SSOTs" >&2
    fi
  fi
...
# Skip remaining checks if no .tex files staged (but respect earlier failures)
if [ -z "$STAGED_TEX" ] && [ -z "$STAGED_YAML_MAPPING" ]; then
  if [ "$FAILED" -eq 1 ]; then
    echo "⛔ Commit blocked: Knowledge consistency check failed" >&2
    exit 1
  fi
  exit 0
fi
...
# ─────────────────────────────────────────────────────────────────────────
# BIBTEX KEY FORMAT CHECK: BLOCK commits with non-canonical keys
# SSOT: docs/standards/bibtex-key-convention.md
# Format: {nachname}{jahr}{kurzwort} — Regex: ^[a-z]+\d{4}[a-z]+$
# Migration complete (2026-02-11): All 2,347 keys canonical. No exceptions.
# ─────────────────────────────────────────────────────────────────────────
if [ -n "$STAGED_BIB" ]; then
  if [ -f "$CLAUDE_PROJECT_DIR/scripts/validate_bibtex_key_format.py" ]; then
    echo "=== EBF PreCommit: BibTeX Key Format Check ===" >&2
    if ! python "$CLAUDE_PROJECT_DIR/scripts/validate_bibtex_key_format.py" --pre-commit 2>&1 >&2; then
      echo "❌ COMMIT BLOCKED: Non-canonical BibTeX key detected." >&2
      echo "   SSOT: docs/standards/bibtex-key-convention.md" >&2
      echo "   Fix:  python scripts/validate_bibtex_key_format.py --suggest KEY" >&2
      exit 1
    fi
    echo "" >&2
  fi
...
# ─────────────────────────────────────────────────────────────────────────
# AUTO-SYNC: Bidirectional use_for Synchronization (BibTeX ↔ YAML)
# When BibTeX or Paper-YAMLs change, keep use_for tags in sync
# ─────────────────────────────────────────────────────────────────────────
if [ -n "$STAGED_BIB" ] || [ -n "$STAGED_PAPER_REF" ]; then
  echo "" >&2
  echo "=== EBF PreCommit: Auto-Sync use_for (BibTeX ↔ YAML) ===" >&2

  SYNC_CHANGES=0

  # Direction 1: BibTeX → YAML (add missing tags to YAMLs)
  if [ -f "$CLAUDE_PROJECT_DIR/scripts/sync_bib_use_for_to_yaml.py" ]; then
    BIB2YAML_RESULT=$(python "$CLAUDE_PROJECT_DIR/scripts/sync_bib_use_for_to_yaml.py" 2>&1 || true)
    BIB2YAML_COUNT=$(echo "$BIB2YAML_RESULT" | grep "Updated:" | grep -oE '[0-9]+' || echo "0")
    if [ "$BIB2YAML_COUNT" -gt 0 ]; then
      echo "  ✅ BibTeX→YAML: $BIB2YAML_COUNT files synced" >&2
      SYNC_CHANGES=1
      # Stage modified YAMLs
      MODIFIED_YAMLS=$(git status --porcelain | grep '^ M.*paper-references/PAP-.*\.yaml$' | awk '{print $2}' || true)
      for yaml_file in $MODIFIED_YAMLS; do
        git add "$CLAUDE_PROJECT_DIR/$yaml_file" 2>/dev/null || true
      done
    fi
  fi

  # Direction 2: YAML → BibTeX (add missing tags to BibTeX)
  if [ -f "$CLAUDE_PROJECT_DIR/scripts/sync_yaml_use_for_to_bib.py" ]; then
    YAML2BIB_RESULT=$(python "$CLAUDE_PROJECT_DIR/scripts/sync_yaml_use_for_to_bib.py" 2>&1 || true)
    YAML2BIB_COUNT=$(echo "$YAML2BIB_RESULT" | grep "Updated:" | grep -oE '[0-9]+' || echo "0")
    if [ "$YAML2BIB_COUNT" -gt 0 ]; then
      echo "  ✅ YAML→BibTeX: $YAML2BIB_COUNT entries synced" >&2
      SYNC_CHANGES=1
      git add "$CLAUDE_PROJECT_DIR/bibliography/bcm_master.bib" 2>/dev/null || true
    fi
  fi

  if [ "$SYNC_CHANGES" -eq 0 ]; then
    echo "  ✅ use_for tags already in sync" >&2
  fi
...
# STAGED_PAPER_REF already defined above (near STAGED_BIB)
...
# ─────────────────────────────────────────────────────────────────────────
# LAYER 1 SMOKE TEST: Validate PCT + LLMMC + Parameter API pipeline
# Triggers when any Layer 1 script or its data dependencies are modified
# ─────────────────────────────────────────────────────────────────────────
STAGED_LAYER1=$(git diff --cached --name-only --diff-filter=ACM | grep -E '(scripts/(pct|llmmc_calibration|parameter_api|extract_measurement_contexts|validate_psi_scales|auto_extract_pipeline|orchestrator|translation_templates)\.py|data/(pct-|parameter-registry).*\.yaml)' || true)

if [ -n "$STAGED_LAYER1" ]; then
  echo "" >&2
  echo "=== EBF PreCommit: Layer 1 Pipeline Smoke Test ===" >&2
  echo "   Triggered by: $(echo $STAGED_LAYER1 | tr '\n' ', ')" >&2

  if [ -f "$CLAUDE_PROJECT_DIR/scripts/pct_smoke_test.py" ]; then
    SMOKE_RESULT=$(python "$CLAUDE_PROJECT_DIR/scripts/pct_smoke_test.py" --quiet 2>&1)
    SMOKE_STATUS=$?

    if [ $SMOKE_STATUS -ne 0 ]; then
      echo "" >&2
      echo "❌ LAYER 1 SMOKE TEST FAILED" >&2
      echo "   The PCT/LLMMC/Parameter API pipeline is broken." >&2
      echo "" >&2
      # Re-run verbose to show details
      python "$CLAUDE_PROJECT_DIR/scripts/pct_smoke_test.py" 2>&1 | sed 's/^/   /' >&2
      echo "" >&2
      echo "   Fix the failing stages before committing." >&2
      FAILED=1
    else
      echo "✅ Layer 1 pipeline: 8/8 stages passed" >&2
    fi
  else
    echo "⚠️  pct_smoke_test.py not found — skipping pipeline check" >&2
  fi

  # Also run the full test suites
  echo "" >&2
  ...
  # PCT core tests
  if [ -f "$CLAUDE_PROJECT_DIR/tests/test_pct.py" ]; then
    PCT_RESULT=$(python "$CLAUDE_PROJECT_DIR/tests/test_pct.py" 2>&1)
    PCT_STATUS=$?
    PCT_COUNT=$(echo "$PCT_RESULT" | grep "passed" | grep -oE '[0-9]+ passed' || echo "? passed")
    if [ $PCT_STATUS -ne 0 ]; then
      echo "   ❌ test_pct.py: FAILED" >&2
      echo "$PCT_RESULT" | grep -E '(FAIL|ERROR)' | head -5 | sed 's/^/      /' >&2
      TEST_FAILED=1
    else
      echo "   ✅ test_pct.py: $PCT_COUNT" >&2
    fi
  fi

  # LLMMC-PCT integration tests
  if [ -f "$CLAUDE_PROJECT_DIR/tests/test_llmmc_pct_integration.py" ]; then
    LLMMC_RESULT=$(python "$CLAUDE_PROJECT_DIR/tests/test_llmmc_pct_integration.py" 2>&1)
    LLMMC_STATUS=$?
    LLMMC_COUNT=$(echo "$LLMMC_RESULT" | grep "passed" | grep -oE '[0-9]+ passed' || echo "? passed")
    if [ $LLMMC_STATUS -ne 0 ]; then
      echo "   ❌ test_llmmc_pct_integration.py: FAILED" >&2
      echo "$LLMMC_RESULT" | grep -E '(FAIL|ERROR)' | head -5 | sed 's/^/      /' >&2
      TEST_FAILED=1
    else
      echo "   ✅ test_llmmc_pct_integration.py: $LLMMC_COUNT" >&2
    fi
  fi

  # Parameter API tests
  if [ -f "$CLAUDE_PROJECT_DIR/tests/test_parameter_api.py" ]; then
    API_RESULT=$(python "$CLAUDE_PROJECT_DIR/tests/test_parameter_api.py" 2>&1)
    API_STATUS=$?
    API_COUNT=$(echo "$API_RESULT" | grep "passed" | grep -oE '[0-9]+ passed' || echo "? passed")
    if [ $API_STATUS -ne 0 ]; then
      echo "   ❌ test_parameter_api.py: FAILED" >&2
      echo "$API_RESULT" | grep -E '(FAIL|ERROR)' | head -5 | sed 's/^/      /' >&2
      TEST_FAILED=1
    else
      echo "   ✅ test_parameter_api.py: $API_COUNT" >&2
    fi
  fi

  # Orchestrator tests
  if [ -f "$CLAUDE_PROJECT_DIR/tests/test_orchestrator.py" ]; then
    ORCH_RESULT=$(python "$CLAUDE_PROJECT_DIR/tests/test_orchestrator.py" 2>&1)
    ORCH_STATUS=$?
    ORCH_COUNT=$(echo "$ORCH_RESULT" | grep "passed" | grep -oE '[0-9]+ passed' || echo "? passed")
    if [ $ORCH_STATUS -ne 0 ]; then
      echo "   ❌ test_orchestrator.py: FAILED" >&2
      echo "$ORCH_RESULT" | grep -E '(FAIL|ERROR)' | head -5 | sed 's/^/      /' >&2
      TEST_FAILED=1
    else
      echo "   ✅ test_orchestrator.py: $ORCH_COUNT" >&2
    fi
  fi

  # Translation template tests
  if [ -f "$CLAUDE_PROJECT_DIR/tests/test_translation_templates.py" ]; then
    TRANS_RESULT=$(python "$CLAUDE_PROJECT_DIR/tests/test_translation_templates.py" 2>&1)
    TRANS_STATUS=$?
    TRANS_COUNT=$(echo "$TRANS_RESULT" | grep "passed" | grep -oE '[0-9]+ passed' || echo "? passed")
    if [ $TRANS_STATUS -ne 0 ]; then
      echo "   ❌ test_translation_templates.py: FAILED" >&2
      echo "$TRANS_RESULT" | grep -E '(FAIL|ERROR)' | head -5 | sed 's/^/      /' >&2
      TEST_FAILED=1
    else
      echo "   ✅ test_translation_templates.py: $TRANS_COUNT" >&2
    fi
  fi

  # End-to-end pipeline tests
  if [ -f "$CLAUDE_PROJECT_DIR/tests/test_end_to_end_pipeline.py" ]; then
    E2E_RESULT=$(python "$CLAUDE_PROJECT_DIR/tests/test_end_to_end_pipeline.py" 2>&1)
    E2E_STATUS=$?
    E2E_COUNT=$(echo "$E2E_RESULT" | grep "passed" | grep -oE '[0-9]+ passed' || echo "? passed")
    if [ $E2E_STATUS -ne 0 ]; then
      echo "   ❌ test_end_to_end_pipeline.py: FAILED" >&2
      echo "$E2E_RESULT" | grep -E '(FAIL|ERROR)' | head -5 | sed 's/^/      /' >&2
      TEST_FAILED=1
    else
      echo "   ✅ test_end_to_end_pipeline.py: $E2E_COUNT" >&2
    fi
  fi

  # Psi-scale validation tests
  if [ -f "$CLAUDE_PROJECT_DIR/tests/test_psi_scale_validation.py" ]; then
    PSI_RESULT=$(python "$CLAUDE_PROJECT_DIR/tests/test_psi_scale_validation.py" 2>&1)
    PSI_STATUS=$?
    PSI_COUNT=$(echo "$PSI_RESULT" | grep "passed" | grep -oE '[0-9]+ passed' || echo "? passed")
    if [ $PSI_STATUS -ne 0 ]; then
      echo "   ❌ test_psi_scale_validation.py: FAILED" >&2
      echo "$PSI_RESULT" | grep -E '(FAIL|ERROR)' | head -5 | sed 's/^/      /' >&2
      TEST_FAILED=1
    else
      echo "   ✅ test_psi_scale_validation.py: $PSI_COUNT" >&2
    fi
  fi

  # Measurement context generation tests
  if [ -f "$CLAUDE_PROJECT_DIR/tests/test_generate_measurement_contexts.py" ]; then
    GMC_RESULT=$(python "$CLAUDE_PROJECT_DIR/tests/test_generate_measurement_contexts.py" 2>&1)
    GMC_STATUS=$?
    GMC_COUNT=$(echo "$GMC_RESULT" | grep "passed" | grep -oE '[0-9]+ passed' || echo "? passed")
    if [ $GMC_STATUS -ne 0 ]; then
      echo "   ❌ test_generate_measurement_contexts.py: FAILED" >&2
      echo "$GMC_RESULT" | grep -E '(FAIL|ERROR)' | head -5 | sed 's/^/      /' >&2
      TEST_FAILED=1
    else
      echo "   ✅ test_generate_measurement_contexts.py: $GMC_COUNT" >&2
    fi
  fi

  # Quick psi-scale label consistency check (strict mode)
  if [ -f "$CLAUDE_PROJECT_DIR/scripts/validate_psi_scales.py" ]; then
    VALPSI_RESULT=$(python "$CLAUDE_PROJECT_DIR/scripts/validate_psi_scales.py" --strict 2>&1)
    VALPSI_STATUS=$?
    if [ $VALPSI_STATUS -ne 0 ]; then
      echo "   ❌ validate_psi_scales --strict: UNMAPPED LABELS FOUND" >&2
      echo "$VALPSI_RESULT" | grep -E 'UNMAPPED|ACTION' | head -5 | sed 's/^/      /' >&2
      TEST_FAILED=1
    else
      echo "   ✅ validate_psi_scales: all labels consistent" >&2
    fi
  fi

  # PCT coverage threshold gate (minimum 70%)
  PCT_COVERAGE_THRESHOLD=70
  if [ -f "$CLAUDE_PROJECT_DIR/scripts/validate_psi_scales.py" ]; then
    COV_RESULT=$(python "$CLAUDE_PROJECT_DIR/scripts/validate_psi_scales.py" --coverage --threshold $PCT_COVERAGE_THRESHOLD 2>&1)
    COV_STATUS=$?
    COV_LINE=$(echo "$COV_RESULT" | head -1)
    if [ $COV_STATUS -ne 0 ]; then
      echo "   ❌ $COV_LINE" >&2
      echo "      Run: python scripts/generate_measurement_contexts.py --update-all" >&2
      TEST_FAILED=1
    else
      echo "   ✅ $COV_LINE (threshold: ${PCT_COVERAGE_THRESHOLD}%)" >&2
    fi
  fi

  if [ $TEST_FAILED -eq 1 ]; then
    echo "" >&2
    echo "❌ LAYER 1 UNIT TESTS FAILED — fix before committing" >&2
    FAILED=1
  fi
...
if [ "$FAILED" -eq 1 ]; then
  echo "⛔ Commit blocked: Layer 1 pipeline validation failed" >&2
  exit 1
fi
