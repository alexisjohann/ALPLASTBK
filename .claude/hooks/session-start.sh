...
# ─────────────────────────────────────────────────────────────────────────
# GitHub Token Auto-Loader
# Loads token from .claude/.gh_token if available
# ─────────────────────────────────────────────────────────────────────────
GH_TOKEN_FILE="$CLAUDE_PROJECT_DIR/.claude/.gh_token"

if [ -f "$GH_TOKEN_FILE" ]; then
  GH_TOKEN_VALUE=$(cat "$GH_TOKEN_FILE" | tr -d '[:space:]')
  if [ -n "$GH_TOKEN_VALUE" ]; then
    export GH_TOKEN="$GH_TOKEN_VALUE"
    echo "┌─────────────────────────────────────────────────────────────────┐" >&2
    echo "│  ✅ GITHUB TOKEN: Automatisch geladen aus .claude/.gh_token     │" >&2
    echo "│     gh CLI ist einsatzbereit.                                   │" >&2
    echo "└─────────────────────────────────────────────────────────────────┘" >&2
  fi
else
  echo "┌─────────────────────────────────────────────────────────────────┐" >&2
  echo "│  🔑 GITHUB TOKEN: Nicht gefunden                                │" >&2
  echo "├─────────────────────────────────────────────────────────────────┤" >&2
  echo "│  Für gh CLI: Token in .claude/.gh_token speichern               │" >&2
