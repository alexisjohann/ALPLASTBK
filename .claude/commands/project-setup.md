...
### 3.1b Volumen, Team & Zeitdauer (PFLICHT)

> Diese drei Felder werden bei JEDER Projekteröffnung erfasst.
...
PROJEKTVOLUMEN (CHF)
  ...
  Volumen:
    ...
    [5] CHF 100-250k
    [6] > CHF 250k
    [7] Exakt: CHF _______________

  Abrechnungsmodell:
    [1] FIXED_PRICE        - Pauschale
    [2] TIME_AND_MATERIALS - nach Aufwand
    [3] RETAINER           - laufendes Mandat
    [4] BLENDED            - Mix (Pauschale + Aufwand)
...
PROJEKTMITARBEITENDE
  ...
  FehrAdvice-Team:
    Project Lead: [GF | EB | MR | AJ | MDL | ___]
      Auslastung: ___ % (z.B. 50)

    Weitere Mitarbeitende:
      [+] Name: _______________ | Rolle: _______________ | Auslastung: ___ %
      [+] Name: _______________ | Rolle: _______________ | Auslastung: ___ %

  Kunden-Team:
    [+] Name: _______________ | Rolle: _______________ | Verfügbarkeit: ___
    [+] Name: _______________ | Rolle: _______________ | Verfügbarkeit: ___

  Externe Partner (optional):
    [+] Name: _______________ | Organisation: _______________ | Rolle: ___
...
ZEITDAUER
  ...
  Dauer:     ___ Wochen / ___ Monate

  Phasen:
    Phase 1: _______________ | Dauer: ___
    Phase 2: _______________ | Dauer: ___
    [+] Weitere Phase

  Meilensteine:
    [+] _______________ | Datum: _______________ | HARD / SOFT
    [+] _______________ | Datum: _______________ | HARD / SOFT
```
...
(Budget, Timeline und Team → bereits in Schritt 3.1b erfasst)

Weitere Constraints:
  [+] _______________  (z.B. "FINMA-Compliance erforderlich")
  [+] _______________  (z.B. "Keine Cloud-Speicherung")
  [+] _______________

Blackout-Perioden:
  [+] _______________  (z.B. "Weihnachtsferien 23.12-06.01")
```

### 3.6 Stakeholder (Governance)

```
STAKEHOLDER (Governance-Rollen)

(Team-Zuordnung und Auslastung → bereits in Schritt 3.1b erfasst)

Kundenseite - Governance:

  Sponsor (Auftraggeber/Budget-Verantwortung):
    Name: _______________
    Rolle: _______________
  ...
  Weitere Stakeholder:
    [+] Name: _______________ | Rolle: _______________ | Typ: [KEY_USER | SUBJECT_EXPERT]
```
...
PROJEKT ERSTELLT ✓
  ...
  Status:       SCOPING
  Owner:        GF
  Volumen:      CHF 50-100k (T&M)
  Team:         GF (Lead, 50%), AB (Analyst, 100%)
  Zeitdauer:    12 Wochen (15.03 - 07.06.2026)
...
**Sektionen:**
0. Projekt-Identifikation (inkl. Volumen, Team, Zeitdauer)
1. Ziel (SMART)
2. In-Scope
3. Out-of-Scope
4. Lieferobjekte
...
☐ Volumen definiert (Betrag + Abrechnungsmodell)
☐ Projektmitarbeitende zugewiesen (FA + Kunde + Auslastung)
☐ Zeitdauer festgelegt (Start, Ende, Phasen, Meilensteine)
☐ Ziel SMART formuliert
☐ In-Scope / Out-of-Scope definiert
☐ Lieferobjekte mit Terminen
☐ Stakeholder (Governance) identifiziert
