...
|-----------|--------|--------|
| **Papers (BibTeX)** | 2,395 | `bibliography/bcm_master.bib` |
| **Papers (YAML-DB)** | 2,395 | `data/paper-references/` |
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  PAPER DATABASE STATUS (2,451 Papers)                                   │
...
│  ├── L0:    327 Papers (13%) - Metadata only (kein S1-S6)              │
│  ├── L1:  1,994 Papers (83%) - Research Question bekannt (S1)          │
│  ├── L2:     63 Papers       - Summary/Extract (S1-S4, kein Volltext)  │
│  └── L3:     11 Papers       - KOMPLETTER Originaltext + References    │
│  │                                                                      │
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  🚫 EXTERNE APIs IMMER ÜBER GITHUB ACTIONS NUTZEN!                      │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
...
│                                                                         │
│  ✅ LÖSUNG: GitHub Actions Workflows verwenden!                         │
│                                                                         │
│  VERFÜGBARE WORKFLOWS:                                                  │
│  ├── .github/workflows/doi-lookup.yml        - Einzelne DOI auflösen   │
│  ├── .github/workflows/doi-lookup-batch.yml  - Batch DOI-Lookup        │
│  └── .github/workflows/generate-papers.yml   - Paper-Generierung       │
│                                                                         │
│  WORKFLOW-TRIGGER:                                                      │
│  ├── Via GitHub UI: Actions → Workflow → "Run workflow"                │
│  ├── Via gh CLI:    gh workflow run doi-lookup-batch.yml               │
│  └── Via Push:      Einige Workflows triggern automatisch bei Push     │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

**ARCHITEKTUR: Lokal + GitHub Actions**

| Aufgabe | Lokal (Claude Code) | GitHub Actions |
|---------|---------------------|----------------|
| Paper-Suche | `search_bibliography.py` | - |
| Author-Matching | `match_authors_to_lit.py` | - |
| BibTeX-Parsing | `update_bibtex_fields.py` | - |
| DOI-Lookup | ❌ blockiert | `doi-lookup-batch.yml` ✅ |
| OpenAlex-Abfragen | ❌ blockiert | `match_papers_to_appendices.py` via Actions ✅ |
| ORCID-Validierung | ❌ blockiert | Geplant ✅ |

**VERBOTEN:**
```
❌ Direkte API-Calls mit requests/urllib aus Claude Code
❌ WebFetch auf api.crossref.org, api.openalex.org, etc.
...
✅ GitHub Actions für externe API-Calls triggern
✅ Ergebnisse aus GitHub Actions lokal weiterverarbeiten
✅ User informieren: "API-Zugriff erfordert GitHub Actions Workflow"
```
...
│  SCHRITT 3: PARAMETER                                                   │
├─────────────────────────────────────────────────────────────────────────┤
│  3a. LLMMC Prior generieren (Punkt-Schätzungen mit Unsicherheit)       │
│  3b. Bayesian Updating mit:                                             │
│      • BCM2 Kontext-Datenbank                                          │
│      • bcm_master.bib Papers                                           │
│  3c. Posterior-Tabelle zeigen                                          │
│                                                                         │
│  VERBESSERUNGSVORSCHLÄGE (bei STANDARD):                               │
│  ├── P1: + Mehr Quellen suchen                                         │
│  ├── P2: + Cross-Validation                                            │
│  ├── P3: + Gewichte anpassen                                           │
│  ├── P4: Keine Änderung (Weiter)                                       │
│  └── Alle                                                              │
│                                                                         │
│  → User-Wahl abwarten → Learning Log                                   │
└─────────────────────────────────────────────────────────────────────────┘
...
**Bei JEDER Kontextanalyse diese Reihenfolge einhalten:**
...
| `theory_papers.py --paper` | Theorien für ein Paper finden | `python scripts/theory_papers.py --paper kahneman1979prospect` |
...
│  │ → bib_keys: ["PAP-kahneman1979prospect", ...]                         │  │
│  └───────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────┘
```
...
#    → Ergibt: laibson1997golden, odonoghue1999doing, ...
```
