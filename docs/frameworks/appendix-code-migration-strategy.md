...
**Pattern matching:**
```
Appendix MUL1 → Appendix UNMAPPED_LAB
...
\item Appendix UNMAPPED_WHO (CORE-WHO): Levels $L$ on which dimensions operate

% After
See Appendix UNMAPPED_LAB for labor market applications
\item Appendix UNMAPPED_WHO (CORE-WHO): Levels $L$ on which dimensions operate
```
...
```
Appendix MUL1 → Appendix UNMAPPED_LAB
[Appendix MUL1](../appendices/AA_labor.tex) → [Appendix UNMAPPED_LAB](../appendices/LAB_labor.tex)
```
...
# Expected: ~20 (only CORE/FORMAL like "Appendix UNMAPPED_WAT", "Appendix UNMAPPED_HOW" which are intentional)
