...
\item Appendix STA (CORE-STAGE): Journey phases for F6 mapping
\item Appendix PAP1 (FORMAL-SEGMENT): Segment definitions for F9 targeting
\item Appendix WEN (CORE-WHEN): Context framework for F3 specification
\item Appendix PAP (FORMAL-EQUILIBRIA): Intervention equilibria theory
\end{itemize}
...
\textbf{EBF connection:} Informative interventions primarily target the Awareness function $A(\cdot)$ from Appendix AWA, increasing $A_{\text{info}}$.
...
\textbf{EBF connection:} Commitment interventions address the Willingness function $WAX$ from Appendix REA, specifically the gap between $WAX_{\text{stated}}$ and $WAX_{\text{actual}}$.
