...
\begin{itemize}[nosep]
    ...
    \item Parameter estimation methods $\rightarrow$ Appendix UNMAPPED_BBB (CORE-WHERE)
...
\textbf{Primär:}
\begin{itemize}[nosep]
\item Appendix TKT (METHOD-TOOLKIT) --- F9 (Target Segment), Axiom HHH-A5 (Segment Specificity)
\item Appendix PAP1 (FORMAL-SEGMENT) --- Segment-Definitionen, Clustering-Axiome
\end{itemize}

\textbf{Unterstützend:}
\begin{itemize}[nosep]
\item Appendix UNMAPPED_REA (CORE-READY): Willingness-Heterogenität nach Segment
\item Appendix UNMAPPED_EST (CORE-WHERE): Literatur-basierte $\sigma$-Schätzungen
\item Appendix UNMAPPED_FEH (LIT-FEHR): Fehr's Heterogenitäts-Forschung (Conditional Cooperators)
\item Appendix UNMAPPED_ACE (LIT-KAHNEMAN): Prospect Theory Segmente
...
\vspace{0.2cm}
\textit{Siehe: Chapter 11 §11.20 (EMERGE), Chapter 20 §20.7 (Constraints C6-C7), Appendix UNMAPPED_IE}
\end{tcolorbox}
...
Dieses Kapitel behandelt $\sigma_s$. Für $\lambda_L$, siehe Appendix UNMAPPED_AAA (CORE-WHO).
\end{tcolorbox}
...
Ernst Fehr und Kollegen haben systematisch Verhaltensheterogenität dokumentiert (Appendix UNMAPPED_FEH):
...
\textbf{Lösung:} Nutze \textbf{LLMMC (LLM Monte Carlo, Appendix UNMAPPED_LLM)} als Default-Methode für:
\begin{itemize}[nosep]
...
Reference: Appendix UNMAPPED_LLM (METHOD-LLMMC) for technical details on LLM Monte Carlo sampling.
...
\item \textbf{Emerging Domain: Political Segments.} Recent research extends behavioral segmentation to political attitudes. For instance, \citet{doew2024rechtsextremismus} document that $\pi_{\text{extreme}} = 10\%$ of the Austrian population hold extreme-right attitudes, with $\pi_{\text{replacement}} = 47\%$ endorsing ``Great Replacement'' conspiracy beliefs. These politically-defined segments show distinct behavioral parameters (RWA, SDO, conspiracy mentality) that predict response to interventions targeting identity, threat perception, and in-group solidarity. See Appendix UNMAPPED_OT (LIT-OTHER) §\ref{subsec:ot-political-psychology} and Case UNMAPPED_CAS-890 for the full 10C profile.
\end{itemize}
...
\begin{itemize}[nosep]
\item Appendix TKT (METHOD-TOOLKIT) --- F9 (Target Segment), Axiom HHH-A5
\item Appendix PAP1 (FORMAL-SEGMENT) --- Segment-Definitionen, Axiome PAP1-1 bis PAP1-3
\item Appendix UNMAPPED_FEH (LIT-FEHR) --- Fehr's Heterogenitäts-Forschung
\item Appendix UNMAPPED_EST (CORE-WHERE) --- Literatur-basierte $\sigma$-Schätzungen
\end{itemize}
...
\textbf{Für Vertiefung:} Appendix PAP1 (Segment-Axiome), Appendix UNMAPPED_FEH (Fehr's Heterogenitäts-Forschung), Appendix UNMAPPED_IE (EMERGE-SPEC)
\end{tcolorbox}
