...
\textbf{Out-of-Scope (delegiert an Appendix UNMAPPED_MP):}
\begin{itemize}[nosep]
    \item Full axiom catalog (140 axioms) $\rightarrow$ Appendix UNMAPPED_MP
    \item Complete theorem proofs (62 theorems) $\rightarrow$ Appendix UNMAPPED_MP
    \item Solution algorithms $\rightarrow$ Appendix UNMAPPED_LLM (METHOD-LLMMC)
    \item Empirical calibration $\rightarrow$ Appendix UNMAPPED_BBB (CORE-WHERE)
\end{itemize}
...
This chapter corresponds to \textbf{Appendix UNMAPPED_MP (CORE-MASTER)}, which provides the complete mathematical formulation of the EBF optimization problem.

\textbf{Relationship:}
\begin{itemize}[noitemsep]
    \item This chapter: Conceptual overview, intuition, accessibility
    \item Appendix UNMAPPED_MP: Complete formalism, 140 axioms, 62 theorems, full proofs
\end{itemize}
...
\begin{itemize}
    ...
    \item $I_t$ = 9D intervention vector at time $t$ (Appendix UNMAPPED_IE)
...
This formulation uses the \textbf{EXC-3 hybrid form} (Appendix UNMAPPED_FRM): veto factors multiply, while additive adjustments are summed within WEC.
...
\begin{tabular}{|c|l|l|}
\hline
\textbf{Ch.} & \textbf{Title} & \textbf{Contribution to UNMAPPED_MP-1} \\
\hline
...
\textbf{Completeness:} The full mapping (140 axioms, 62 theorems) is in Appendix UNMAPPED_MP.
...
\textbf{The Key Insight:} The optimization problem IS the framework. Understanding UNMAPPED_MP-1 is understanding EBF.
...
\begin{itemize}[noitemsep]
    ...
    \item \textit{Tool:} Context Checklist (see Appendix UNMAPPED_V)
\end{itemize}
...
\begin{itemize}[noitemsep]
    ...
    \item Parameter uncertainty $\to$ Step 1 (Context) $\to$ LLMMC (Appendix UNMAPPED_LLM)
...
\hline
Define context & Context Checklist (Appendix UNMAPPED_V) & Step 1 \\
...
\begin{enumerate}
    ...
    \item \textbf{Calibrate parameters:} Use Appendix UNMAPPED_BBB and empirical data to estimate $\Theta$
    \item \textbf{Solve computationally:} Use Appendix UNMAPPED_LLM (LLMMC) for parameter estimation under uncertainty
...
\textbf{Next:}
\begin{itemize}[noitemsep]
    \item For formal details: Appendix UNMAPPED_MP (CORE-MASTER)
    \item For solution methods: Appendix UNMAPPED_LLM (METHOD-LLMMC)
    \item For parameter calibration: Appendix UNMAPPED_BBB (CORE-WHERE)
