...
\paragraph{Empirical Challenge: Belief Updating Failures.}
Recent experimental evidence challenges the assumption that beliefs converge to
truth with evidence. \citet{bregolin2026advice} test the reputational cheap-talk
model (\citealt{sobel1985theory}) in a laboratory setting where participants can
purchase advice from advisors with observable track records. Their key finding:
\textbf{approximately 50\% of participants do not update their beliefs about
advisor quality} in response to new information about advisor accuracy.
Furthermore, demand for advice is independent of whether the advice confirms or
contradicts the participant's prior belief---contradicting the prediction that
information value depends on belief-action alignment. Instead, purchasing behavior
appears driven by hot-hand fallacy: participants buy more after advisors' correct
predictions, regardless of long-run accuracy. This suggests that even in domains
with clear feedback and low identity stakes, Bayesian updating often fails.
...
\begin{itemize}
    ...
    \item AWX-A87 (Supply Mechanisms), AWX-A88 (Domain Specificity), 
          AWX-A90 (Informed Benchmark): See Appendix UNMAPPED_AWA for full specification
\end{itemize}
