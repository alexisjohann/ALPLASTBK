...
\textbf{Primär:}
\begin{itemize}[nosep]
\item Appendix HHH (METHOD-TOOLKIT) --- F6 (Journey Phase), F14 (Path Function), Theorem HHH-T5, Axiom HHH-A3
\item Appendix STA (CORE-STAGE) --- Journey-Axiome STA-1 bis STA-4, $S(t)$ Progression Function
\end{itemize}

\textbf{Unterstützend:}
\begin{itemize}[nosep]
\item Appendix WEN (CORE-WHEN): Temporaler Kontext $\Psi_T$, kritische Zeitfenster
\item Appendix PAP (FORMAL-EQUILIBRIA): Übergangs-Dynamiken, Tipping Points
\item Appendix FEH (LIT-FEHR): Experimentelle Evidenz für Interventions-Effekte
\item Appendix EST (CORE-WHERE): Literatur-basierte $\alpha$-Schätzungen
...
Das Behavioral Change Journey Modell (Appendix STA) formalisiert eine fundamentale Einsicht der Verhaltensänderungsforschung: \textbf{Menschen durchlaufen distinkte psychologische Zustände auf dem Weg zur Verhaltensänderung}, und diese Zustände bestimmen, welche Interventionen wirksam sind.

\subsection{Das Axiom der Phasenabhängigkeit (STA-3)}

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=Axiom STA-3: Phase-Dependent Intervention Efficacy]
...
\textbf{WHERE} (BBB) & Parameter-Quellen & $\alpha$-Werte aus Appendix BBB (Literatur-Repository) \\
...
\begin{table}[htbp]
\centering
\caption{Archetypen-Affinity Matrix $\alpha_{a,\varphi}$ mit 95\% CI (Quellen: Appendix BBB)}
...
\textbf{Für Vertiefung:} Appendix STA (Journey-Axiome), Appendix HHH (F6, F14)
\end{tcolorbox}
