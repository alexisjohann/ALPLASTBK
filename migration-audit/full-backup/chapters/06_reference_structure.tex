...
\medskip
\textbf{Reading Guide:} This chapter provides intuition; Appendix PRF contains
...
Appendix PRF provides three independent derivations that all yield the same $C^*$.
...
\emph{See Appendix PRF, Part III.}
...
\emph{See Appendix PRF, Part IV.}

\begin{theorem}[Convergence---Appendix PRF, Theorem D.5]
...
\begin{enumerate}[nosep]
    \item \textbf{Literature-based:} Appendix BBB (CORE-WHERE) aggregates parameter values from 1,900+ papers
    \item \textbf{LLM Monte Carlo:} Appendix AN (METHOD-LLMMC) estimates parameters via LLM-based simulation
\end{enumerate}
...
\textbf{Three Derivations} (all in Appendix PRF):
\begin{enumerate}[nosep]
...
\textbf{For Complete Theory:} $\rightarrow$ Appendix PRF (FORMAL-CSTAR)

\textbf{For Practitioners:} $\rightarrow$ Chapter 7 (coherence) $\rightarrow$ Chapter 9 (context)

\textbf{For Calibration:} $\rightarrow$ Appendix EST $\rightarrow$ Appendix LLM1
\end{tcolorbox}
