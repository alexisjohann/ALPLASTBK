...
\textbf{Primary:} Appendix HHH (METHOD-TOOLKIT) --- Complete 20-field intervention schema (Theorems HHH-T1 to HHH-T8)

\textbf{Supporting:}
\begin{itemize}[nosep]
\item Appendix III (METHOD-LLMMC-CAL): Calibration pipeline for R-Score decisions
\item Appendix AW (CORE-STAGE): Journey phases for F6 mapping
\item Appendix BA (FORMAL-SEGMENT): Segment definitions for F9 targeting
\item Appendix V (CORE-WHEN): Context framework for F3 specification
\item Appendix BB (FORMAL-EQUILIBRIA): Intervention equilibria theory
\end{itemize}

\textbf{Emergence Documentation:}
\begin{itemize}[nosep]
\item \texttt{docs/frameworks/core-framework-definition.yaml}: Single Source of Truth (SSOT) for 9C $\rightarrow$ HHH mapping
\item \texttt{data/gamma\_matrix\_complete.json}: $\gamma(\Psi \rightarrow \text{FEPSDE})$ feasibility matrix
\end{itemize}

\textbf{For Prioritization:} Appendix III provides the GO/PILOT/NO-GO decision framework
...
\item \textbf{Section~\ref{sec:17-schema}}: The 20-field intervention schema
    \begin{itemize}[nosep]
    \item \textbf{Section~\ref{sec:17-emergence}}: Emergence from 9C Framework (14,400 $\rightarrow$ 2,000)
    \end{itemize}
\item \textbf{Section~\ref{sec:17-depths}}: Application depths (Light/Hybrid/Profound)
\item \textbf{Section~\ref{sec:17-axioms}}: The five intervention axioms (HHH-1 to HHH-5)
...
\item \textbf{Prioritization decisions}: The R-Score framework (Appendix III) enables GO/PILOT/NO-GO decisions based on calibrated fit parameters
\end{enumerate}
...
\textbf{EBF connection:} Informative interventions primarily target the Awareness function $A(\cdot)$ from Appendix AU, increasing $A_{\text{info}}$.
...
\textbf{EBF connection:} Commitment interventions address the Willingness function $WAX$ from Appendix AV, specifically the gap between $WAX_{\text{stated}}$ and $WAX_{\text{actual}}$.
...
\subsection{Theoretical Foundation: Emergence from the 9C Framework}
\label{sec:17-emergence}

The 20-field schema is not arbitrary---it \textbf{emerges systematically} from the 9C CORE framework. This theoretical grounding ensures completeness and enables computational prediction.

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=\textbf{The Emergence Architecture}]

\textbf{Step 1: 9C COREs define the classification dimensions}

Each CORE question generates a field dimension (see Appendix HHH, Theorems HHH-T1 to HHH-T7):
...
\textbf{Data source:} The $\gamma(\Psi \rightarrow \text{FEPSDE})$ matrix is estimated via LLM Monte Carlo (Appendix III) and stored in \texttt{data/gamma\_matrix\_complete.json}.
...
\begin{itemize}[nosep]
    \item \textbf{6 emergent fields} (F2, F4, F5, F6, F12, +F21) --- derived from 9C via theorems
...
\subsection{Axiom HHH-1: Completeness}
...
\subsection{Axiom HHH-2: Context Dependency}
...
\subsection{Axiom HHH-3: Journey Phase Alignment}
...
\subsection{Axiom HHH-4: Complementarity Structure}
...
\subsection{Axiom HHH-5: Segment Specificity}
...
\midrule
0 & 9C CORE Framework & 9 dimensions & \textcolor{green!60!black}{\textbf{Emergent}} & None (50+ years research) \\
...
\textbf{Mitigation:} Cardinalities can be increased if needed; continuous versions available in Appendix HHH.
\end{tcolorbox}
...
\textbf{Justification:} Matrix estimated via LLM Monte Carlo (Appendix III) with uncertainty quantification.
...
\item \textbf{Stage 2 (Simulate):} Estimate R-Score using Appendix III calibration pipeline
...
\begin{itemize}[nosep]
\item HHH-1: Completeness---specify all required fields
\item HHH-2: Context Dependency---no universal interventions
\item HHH-3: Journey Alignment---match type to phase
\item HHH-4: Complementarity---check $\gamma$ before combining
\item HHH-5: Segment Specificity---same intervention, different effects
\end{itemize}
...
\begin{itemize}[nosep]
\item Appendix HHH (METHOD-TOOLKIT): Complete 20-field intervention schema
\item Appendix III (METHOD-LLMMC-CAL): R-Score calibration and GO/PILOT/NO-GO decisions
\end{itemize}
\end{tcolorbox}
