...
\rowcolor{blue!10}
$\rightarrow$ & \textbf{1. WIE denkt er/sie? (Complementarity $\varphi$)} & \textbf{[DIESES KAPITEL]} \\
\rowcolor{blue!10}
...
\begin{enumerate}
\item How the person \textit{thinks} about multi-dimensional decisions (thinking style $\varphi$)
...
\textbf{Question 1: How does the person think? ($\varphi$)}

The \textbf{Complementarity Parameter} $\varphi \in [0,1]$ captures whether 
a person integrates multiple utility dimensions in a connected (complementary) 
or separated (additive) way:

\begin{center}
\begin{tabular}{lp{8cm}}
$\varphi \to 1$ & ``Everything must be right'' — weakest dimension dominates\\
$\varphi \to 0$ & ``Total benefit counts'' — dimensions add up\\
\end{tabular}
\end{center}
...
\begin{itemize}
\item \textbf{Section 12.2}: The Thinking Style Parameter $\varphi$ — How people integrate multiple dimensions
...
\item \textbf{Section 12.7}: Axiom Overview — The formal foundation in Appendix AV
...
all 99+ axioms, the $\gamma$-matrix for $\varphi$-estimation, the $\delta$-matrix 
for threshold modulation, and the 8-factor documentation framework, 
see \textbf{Appendix AV} (Willingness Formalization).
\end{cross-reference}
