...
%          emerges from the 9C Framework. Serves as foundation for all
...
%
% Prerequisites: Chapters 1-16 (complete 9C Framework understanding)
...
\item \texttt{docs/frameworks/core-framework-definition.yaml}: 9C SSOT
\end{itemize}
...
Dieses Kapitel gibt Anna---und allen Interventions-Designern---ein \textbf{systematisches Framework}, das aus dem 9C-Modell emergiert und evidenzbasierte Entscheidungen ermöglicht.
...
\item Woher kommen die Interventionskategorien? (Emergenz aus 9C)
...
\item \textbf{Teil I} (Section~\ref{sec:17-foundation}): Theoretische Fundierung
    \begin{itemize}[nosep]
    \item Emergenz aus 9C Framework
...
Interventionen im EBF-Framework sind keine ad-hoc Maßnahmen. Sie \textbf{emergieren systematisch} aus dem 9C CORE Framework. Diese theoretische Fundierung unterscheidet EBF von pragmatischen ``Nudge-Katalogen''.

\subsection{Von 9C zu 6D: Die Emergenz der Klassifikationsdimensionen}
\label{sec:17-emergence}

Jede der 9C CORE-Fragen generiert eine Klassifikationsdimension für Interventionen. Sechs dieser Dimensionen bilden den \textbf{6D-Klassifikationsraum} (Theorem HHH-T7).

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=\textbf{Die 6D-Emergenz aus 9C}]
...
Die $\gamma$-Matrix ist geschätzt via LLM Monte Carlo (Appendix III) und gespeichert in:
\begin{center}
...
\midrule
0 & 9C Framework & 6 Dimensionen & Emergent \\
...
Analog zu den Parameter-Level Epistemic Tags (Appendix BBB) klassifizieren wir Archetypen:
...
\midrule
1 & Emergent & F2, F4, F5, F6, F12, (+F21) & 9C via Theoreme \\
...
Diese Felder emergieren direkt aus dem 9C Framework:
...
\midrule
F9 & Target Segment & Appendix BA & Segment-ID \\
F10 & Complementarity & Appendix B & $\gamma$-Werte zu anderen Interventionen \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[nosep]
\item Zielgruppe (Segment aus Appendix BA)
\item Aktuelle Journey-Phase (aus Appendix AW)
\item Awareness-Level (aus Appendix AU)
\end{itemize}
...
wobei:
\begin{itemize}[nosep]
\item $\alpha_i$ = Gewicht für Fit-Komponente $i$ (aus Appendix III kalibriert)
...
\begin{enumerate}
\item \textbf{Emergenz:} Die Interventionslogik emergiert systematisch aus dem 9C Framework:
\begin{equation}
\text{9C} \rightarrow \text{6D} \rightarrow 14{,}400 \rightarrow \gamma(\Psi)\text{-Filter} \rightarrow \approx 2{,}000
\end{equation}
...
\begin{itemize}[nosep]
\item 8 Theoreme (HHH-T1 bis T8) für die 9C $\rightarrow$ HHH Emergence
...
\item Appendix III (R-Score Kalibrierung)
