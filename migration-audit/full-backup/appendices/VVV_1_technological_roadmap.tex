...
% - Appendix V (CORE-WHEN): Context framework ($\Psi$) for scenario analysis
% - Appendix BBB (CORE-WHERE): Parameter estimation for calibration
% - Appendix AN (METHOD-LLMMC): LLM Monte Carlo methods
%
...
\item For technology details: See Appendix AN (METHOD-LLMMC)
\end{itemize}
...
\textbf{Cross-References:} Appendix V (CORE-WHEN), Appendix BBB (CORE-WHERE),
Appendix AN (METHOD-LLMMC), Appendix VVV.2 (Product Roadmap)
\end{tcolorbox}
...
\item \textbf{Comprehensive framework}: 9C CORE covers the full behavioral space
...
\item \textbf{Depth over breadth}: Complete 9C CORE specification as moat
...
\textbf{Integration:} This appendix connects to Appendix V (CORE-WHEN) via
...
Appendix AN (METHOD-LLMMC: LLM Methods),
