...
\item Appendix PBB (FORMAL-PROBABILITY): EIH modifies $P(i \to j)$
\item Appendix STA (CORE-STAGE): Journey alignment condition
\item Appendix PAP1 (FORMAL-SEGMENT): Segment efficiency condition
\item Appendix HOW (CORE-HOW): Complementarity condition
\end{itemize}
...
\begin{itemize}[nosep]
\item Use Appendix STA diagnostic tools
...
\item Check $\gamma_{ij}$ for each pair (Appendix HOW, HHH Table)
