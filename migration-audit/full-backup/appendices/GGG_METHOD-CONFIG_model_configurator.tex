...
% - Appendix EEE (METHOD-DESIGN): Workflow that uses configurator
% - Appendix FFF (METHOD-REGISTRY): Stores generated models
% - Appendix AAA (CORE-WHO): Level definitions
% - Appendix B (CORE-HOW): Complementarity
% - Appendix C (CORE-WHAT): Utility dimensions
% - Appendix V (CORE-WHEN): Context dimensions
% - Appendix AU (CORE-AWARE): Awareness function [NEW]
% - Appendix AV (CORE-READY): Willingness function [NEW]
% - Appendix AW (CORE-STAGE): Behavioral change journey [NEW]
%
% DEPENDENTS (what REQUIRES this appendix):
% - Appendix EEE (METHOD-DESIGN): Uses configurator for model generation
...
\textbf{Reading Guide:}
\begin{itemize}[nosep]
\item For workflow context: Read Appendix EEE first
\item For configuration logic: This appendix provides complete specification
\item For model storage: See Appendix FFF (METHOD-REGISTRY)
\end{itemize}
...
\begin{abstract}
This appendix defines the Model Configurator---a systematic tool for generating
\textbf{9C-complete} behavioral models from four configuration dimensions: Level ($L$),
Geography ($\Psi_{geo}$), Domain, and Scope Type. The Configurator uses seven mapping
tables to auto-populate all 9C parameters: context ($\Psi$), utility dimensions ($C$),
...
an expert-dependent art into a structured, reproducible process with complete 9C coverage.
\end{abstract}

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=Quick Reference: Model Configurator (9C Complete)]

\textbf{Core Question:} How do we systematically generate \textbf{9C-complete} model specifications from high-level choices?
...
\item \textbf{7 Mapping Tables}: Dimension combinations $\rightarrow$ 9C model parameters
\item \textbf{9C Auto-Population}: WHO, WHAT, HOW, WHEN, WHERE, AWARE, READY, STAGE
...
The welfare hierarchy level from CORE-WHO (Appendix AAA).
...
Derived from Appendix AU (CORE-AWARE) and Chapter 11.
...
Derived from Appendix AV (CORE-READY) and Chapter 12.
...
\item Reducing $\theta$ often more effective than increasing $WAX$ (cf. Appendix AV, WAX-A15)
\end{itemize}


\subsection{Mapping 7: Domain × Complexity → Stage Defaults}

Derived from Appendix AW (CORE-STAGE) and Chapter 13.
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Algorithm CONFIG-1: Model Generation (9C Complete)]
...
\textbf{Output:} Complete 9C model specification ready for refinement

\textbf{9C Coverage:}
\begin{itemize}[nosep]
\item WHO: $L$ (input) $\rightarrow$ Appendix AAA
\item WHAT: $C$ (Mapping 2) $\rightarrow$ Appendix C
\item HOW: $\gamma$ (Mapping 3) $\rightarrow$ Appendix B
\item WHEN: $\Psi$ (Mapping 1) $\rightarrow$ Appendix V
\item WHERE: All tables $\rightarrow$ Appendix BBB
\item AWARE: $A$ (Mapping 5) $\rightarrow$ Appendix AU
\item READY: $WAX$, $\theta$ (Mapping 6) $\rightarrow$ Appendix AV
\item STAGE: $\tau$, $\rho$, $\kappa$ (Mapping 7) $\rightarrow$ Appendix AW
\end{itemize}
...
\begin{tcolorbox}[colback=green!10!white, colframe=green!75!black,
    title=\textbf{Appendix GGG: Summary}]
...
\begin{table}[htbp]
\centering
\caption{Symbols Introduced in Appendix GGG}
