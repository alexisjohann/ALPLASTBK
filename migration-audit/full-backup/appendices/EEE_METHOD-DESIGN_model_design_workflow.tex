...
% - Appendix FFF (METHOD-REGISTRY): Model archive for option selection
% - Appendix BBB (CORE-WHERE): Parameter sources
% - Appendix V (CORE-WHEN): Context specification
% - Appendix C (CORE-WHAT): Utility dimensions
% - Appendix B (CORE-HOW): Complementarity specification
%
...
% - Appendix FFF (METHOD-REGISTRY): Stores models created via this workflow
%
% =============================================================================
...
Registry (Appendix FFF) via similarity search (Axiom MD-2).
\end{abstract}
...
\textbf{Cross-References:} FFF (Registry), GGG (Configurator), BBB (Parameters), V (Context), C (Utility), B (Complementarity)
\end{tcolorbox}
...
through similarity search in the Model Registry (Appendix FFF):
\begin{equation}
...
\textbf{Implication:}
\begin{itemize}[nosep]
\item The Registry must exist and be maintained (see Appendix FFF)
...
\begin{tikzpicture}[node distance=1.8cm, auto,
    ...
    \node[block, fill=gray!20, below=of predict] (register) {8. Register Model\\(Appendix FFF)};
...
\textbf{$\Psi$ Dimensions to specify:} See Appendix V (CORE-WHEN) for full taxonomy
\end{tcolorbox}
...
\textbf{Dependencies:}
\begin{itemize}[nosep]
\item Appendix C (CORE-WHAT): Defines available $C$ dimensions
\item Appendix B (CORE-HOW): Defines available $\gamma$ parameters
\end{itemize}
\end{tcolorbox}
...
\item Appendix AN (METHOD-LLMMC): LLM-based estimation
\end{itemize}
\end{tcolorbox}
...
\textbf{See:} Appendix FFF (METHOD-REGISTRY) for complete schema
\end{tcolorbox}
...
Model Design Workflow (Appendix EEE).
...
2. SCHNELL    → 3 questions (PURPOSE, Gestaltungsanlass, Defaults OK?), 9C-complete via GGG
...
- SCHNELL: Ask only 3 questions, then generate 9C-complete model with GGG defaults (Mappings 1-7)
...
NOTE: SCHNELL is 9C-complete because GGG provides defaults for ALL 9C dimensions:
...
The LLM must reference the mapping tables from Appendix GGG to auto-populate defaults:
...
\begin{verbatim}
→ Lookup Table GGG-3 (γ interactions):
...
\begin{verbatim}
→ Lookup Table GGG-4 (Functional Form):
  Decision: Logistic
  Continuous: Linear/CES
  Process: Dynamic/BCJ
\end{verbatim}
...
Ich lade \textbf{9C-komplette} GGG-Defaults für [Gesundheit × L0 × AT]:
\begin{verbatim}
--- 5C KLASSISCH ---
...
\textbf{LLM:} Hier ist Ihr \textbf{9C-komplettes} Modell:
...
\item \textbf{SEED-AT-02:} Steuer Compliance (Österreich) --- 45\% Ähnlichkeit
...
\begin{tcolorbox}[colback=green!10!white, colframe=green!75!black,
    title=\textbf{Appendix EEE: Summary}]
...
\begin{itemize}[nosep]
    ...
    \item \textbf{SCHNELL:} 3 questions → \textbf{9C-complete} model via GGG Mappings 1--7
...
\begin{table}[htbp]
\centering
\caption{Symbols Introduced in Appendix EEE}
