...
% - Appendix B (CORE-HOW): Complementarity framework, γ-matrix
% - Appendix V (CORE-WHEN): Context dimensions Ψ
% - Appendix MS (REF-MODELSPACE): Theory catalog (Game Theory)
...
\item \textbf{Context Framework (Appendix V):} The eight $\Psi$-dimensions that modify utility functions and beliefs in crisis settings.

\item \textbf{Complementarity (Appendix B):} The $\gamma$-matrix capturing how context dimensions interact and reinforce each other in escalation dynamics.

\item \textbf{Parameter Estimation (Appendix BBB):} Literature-validated behavioral parameters ($\lambda$, $\sigma$, $\mu$) from Kahneman \& Tversky (1979), Fehr \& Schmidt (1999).

\item \textbf{LLMMC Method (Appendix CAL):} LLM Monte Carlo for generating informative priors when empirical data is limited or classified.
\end{enumerate}
...
\begin{itemize}[nosep]
\item $\rightarrow$ Appendix B (CORE-HOW): Complementarity framework
\item $\rightarrow$ Appendix V (CORE-WHEN): Context dimensions
\item $\rightarrow$ Appendix MS (REF-MODELSPACE): Theory catalog
\item $\rightarrow$ Appendix AN (METHOD-LLMMC): Estimation methodology
