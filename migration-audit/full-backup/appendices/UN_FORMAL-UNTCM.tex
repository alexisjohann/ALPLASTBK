...
\begin{itemize}[nosep]
    ...
    \item REF-GLOSSARY $\rightarrow$ Appendix GLS
\end{itemize}
...
The Natural Trajectory Model (NTM, Appendix PBB) established that Awareness, Willingness, and Threshold naturally decay over time (NTM-A1 through A4, T1-T5). The Context Drift Model (Appendix V) established that context $\vec{\Psi}$ evolves stochastically. UNTCM unifies these by recognizing they are \emph{not independent}: context affects decay rates, and aggregate behavior feeds back into context.
...
\item Institutional stability reduces economic uncertainty effects \citep{PAP-PAP-north1990institutionsinstitutions}
\item Technology adoption follows S-curves modulated by context \citep{PAP-rogers2003diffusion}
\end{itemize}
...
\begin{itemize}[nosep]
\item Regime I matches RCT decay observations (30--50\% pilot-to-scale) \citep{PAP-dellavigna2022rcts}
\item Regime II matches successful norm shifts (seatbelts, smoking decline) \citep{cialdini2004social}
\item Regime III matches fad dynamics (diet trends, exercise fads) \citep{PAP-rogers2003diffusion}
\end{itemize}
...
\textbf{Note:} All parameters require domain-specific calibration via Appendix BBB methods. Values shown are order-of-magnitude estimates from behavioral economics literature.
...
\midrule
1 & $\lambda_0, \mu_0, \delta_0$ & Literature meta-analysis & Appendix BBB lookup \\
...
Use Appendix BBB Parameter Repository:
\begin{equation}
...
\textbf{Connection to LLMMC (Appendix AN):}
...
Maintenance requirements differ by utility level (cf.\ Appendix AAA):
...
\textbf{Connection to Appendix AW (CORE-STAGE):}
...
\textbf{Connection to Appendix C (CORE-WHAT):}
...
\item \textbf{Teachable moments:} Life events create intervention windows \citep{PAP-thaler2008}
...
\item $\lambda_{LA} \approx 2.25$ = loss aversion coefficient \citep{PAP-PAP-kahneman1979prospectprospect}
...
\textbf{Evidence Base:}
\begin{itemize}[nosep]
\item \citet{PAP-kahneman1991knetsch}: Endowment Effect + Loss Aversion + Status Quo Bias connection
\item \citet{samuelson1988status}: Status Quo Bias in decision making
\item \citet{PAP-thaler1980towardendowment}: WTA/WTP gap demonstrates loss aversion in ownership
...
\begin{itemize}[nosep]
\item \textbf{Decay dynamics:} \citet{PAP-PAP-kahneman1979prospectprospect}, \citet{laibson1997golden}, \citet{frederick2002time}
\item \textbf{Context effects:} \citet{PAP-sunstein2014nudge}, \citet{cialdini2004social}
...
\item \citet{PAP-dellavigna2022rcts}: RCT scaling and decay patterns
\end{itemize}
...
\begin{itemize}[nosep]
\item \citet{PAP-gneezy2011when}: Financial incentives can backfire ($\gamma \approx -0.2$ to $-0.4$)
...
For definitions of key terms used in this appendix, see \textbf{Appendix GLS (REF-GLOSSARY)}.
...
\begin{itemize}[nosep]
    \item \citet{PAP-PAP-kahneman1979prospectprospect}: Prospect theory, reference dependence, loss aversion
    \item \citet{PAP-thaler1985mental}: Mental accounting, willingness to pay/accept
    \item \citet{laibson1997golden}: Hyperbolic discounting, time-inconsistent preferences
    \item \citet{PAP-fehr1999theory}: Social preferences, reciprocity, fairness
...
\begin{itemize}[nosep]
    ...
    \item \citet{PAP-rogers2003diffusion}: Diffusion of innovations, adoption curves
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \citet{PAP-akerlof2000identity}: Economics of identity
    \item \citet{bicchieri2005grammar}: Social norms as equilibria
\end{itemize}

\textbf{Field Evidence and Applications:}
\begin{itemize}[nosep]
    \item \citet{PAP-PAP-dellavigna2009psychologypsychology}: Psychology and economics integration
    \item \citet{PAP-gneezy2000fine}: Motivation crowding, intrinsic vs. extrinsic
    \item \citet{PAP-AllcottRogers2014}: Energy conservation, social comparison
    \item \citet{milkman2014fresh}: Fresh start effect, temporal landmarks
    \item \citet{PAP-sunstein2014nudge}: Nudge, choice architecture applications
\end{itemize}
