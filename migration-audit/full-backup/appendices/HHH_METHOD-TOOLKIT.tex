...
% - Appendix AAA (CORE-WHO): Welfare hierarchy L1-L4 for target level (HHH-T3)
% - Appendix C (CORE-WHAT): FEPSDE dimensions for utility targeting (HHH-T4)
% - Appendix AW (CORE-STAGE): Behavioral Change Journey phases (HHH-T5)
% - Appendix AU (CORE-AWARE): Awareness function for regime classification (HHH-T6)
% - Appendix AV (CORE-READY): Willingness function for W_eff structure (HHH-T1)
% - Appendix HI (CORE-HIERARCHY): Decision hierarchy L0-L3 for temporal scope (HHH-T2)
% - Appendix V (CORE-WHEN): Context framework
% - Appendix BA (FORMAL-SEGMENT): Segment axioms
% - Appendix BB (FORMAL-EQUILIBRIA): Intervention equilibria
% - Appendix IV (FORMAL-LET): Level Emergence Theorem for portfolio emergence
% - Appendix XVIII (LIT-FEHR-METHOD): Exclusion Principle for intervention bundling
%
% DEPENDENTS (what REQUIRES this appendix):
% - Chapter 17-20: The Intervention Design block
% - Appendix R (METHOD-EVAL): Evaluation of interventions
% - Appendix EEE (METHOD-DESIGN): Model design workflow
% - Appendix III (METHOD-LLMMC-CAL): Uses HHH for intervention specification
%
% =============================================================================
...
\item Chapter 20: Intervention Portfolios --- F10 (Complementarity), $\gamma_{ij}$ matrix
...
\item For journey integration: See Chapter 18 and Appendix AW (CORE-STAGE)
\item For segment targeting: See Chapter 19 and Appendix BA (FORMAL-SEGMENT)
\item For portfolio design: See Chapter 20 and Appendix BB (FORMAL-EQUILIBRIA)
\end{itemize}
...
Following Appendix XVIII (LIT-FEHR-METHOD), intervention design must respect the \textbf{Exclusion Principle}:
...
Following Appendix XVIII, the 8 primitive types represent \textbf{single-mechanism interventions}. The burden of proof lies with hybrid interventions:
...
\textbf{Given:} The HIERARCHY stratification (Appendix HI, Chapter 15):
\begin{itemize}[nosep]
...
Following Appendix XVIII, the temporal scope classification implies:
...
\textbf{Statement:} The welfare target level of an intervention emerges from the WHO framework (Appendix AAA).
...
\textbf{Statement:} The utility dimension targeted by an intervention emerges from the FEPSDE framework (Appendix C/CORE-WHAT).
...
\textbf{Statement:} The optimal journey phase for an intervention emerges from the Behavioral Change Journey (Appendix AW/CORE-STAGE).
...
\textbf{Statement:} The effectiveness of an intervention depends on the target population's awareness state (Appendix AU/CORE-AWARE).
...
Following Appendix XVIII, the multi-dimensional classification implies a hierarchy of parsimony:
...
Following Appendix XVIII, the constraint system itself embodies the Exclusion Principle:
...
After feasibility reduction, each archetype receives an \textbf{epistemic status} indicating the evidence level for that \textit{combination}. This extends the parameter-level epistemic tags from Appendix BBB to the archetype level.
...
\textbf{Lösung:}
\begin{enumerate}[nosep]
    \item Journey-Diagnose (Appendix AW) VOR Archetyp-Selektion durchführen
    \item Awareness-Messung (Appendix AU) validieren
...
\begin{enumerate}[nosep]
    ...
    \end{equation*}
    \item Kontext-spezifische Adjustments dokumentieren (Appendix BBB)
\end{enumerate}
...
\begin{enumerate}[nosep]
    \item[$\square$] \textbf{FM1:} Journey-Diagnose (Appendix AW) durchgeführt und validiert?
...
\begin{enumerate}[nosep]
    ...
    \item[$\square$] Learnings in Appendix BBB dokumentiert?
\end{enumerate}
...
\textbf{Lösung:} Integration mit \textbf{LLMMC} (Appendix AN) --- LLM Monte Carlo Estimation.
...
\textbf{Definition der Tiers (aus Appendix BBB):}
...
\textbf{Statement:} Wenn für einen Parameter $\theta$ kein Tier-1 oder Tier-2 Schätzung existiert, \textbf{MUSS} das LLMMC-Protokoll (Appendix AN) angewendet werden:
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom HHH-1: Intervention Completeness]
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom HHH-2: Context Dependency]
...
\end{equation}
where $\Psi = (\psi_1, \ldots, \psi_8)$ is the context vector from Appendix V.
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom HHH-3: Journey Phase Alignment]
...
\textbf{Implication:} Journey diagnosis (Appendix AW) must precede intervention selection.
\end{tcolorbox}

\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom HHH-4: Complementarity Structure]
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Axiom HHH-5: Segment Specificity]
...
The Level Emergence Theorem (Appendix IV) provides the mathematical justification
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=Updated Axiom HHH-4': Portfolio Emergence Condition]

Extending Axiom HHH-4 with LET:

\textbf{Statement (HHH-4'):} A portfolio $\mathcal{P}$ exhibits positive emergence if and only if:
\begin{equation}
...
\item \textbf{Diagnose Before Designing:} Always determine the target population's journey phase (via Appendix AW) before selecting intervention types.
...
\begin{tcolorbox}[colback=green!10!white, colframe=green!75!black,
    title=\textbf{Appendix HHH: Summary}]
...
\begin{itemize}[nosep]
    ...
    \item Integration with all 9 COREs via derived classification dimensions
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{LLMMC-Kalibrierung}: Tier-3 auf Tier-1/2 Anker kalibriert (Axiome CAL-1,2,3,4)
...
This section documents the current epistemological status of all categorization dimensions used in the intervention framework. Some dimensions are \textbf{emergent} (derived from 9C COREs via formal theorems), others are \textbf{operational} (empirically informed design parameters).

\subsection{Tier 1: Emergent Dimensions (Theoretically Derived)}

These dimensions emerge directly from the 9C CORE framework via Theorems HHH-T1 to HHH-T7.
...
\midrule
F9 & Target Segment & Appendix BA (Segmentation) & \textcolor{green!60!black}{Defined} \\
F10 & Complementarity & Appendix B ($\gamma$-Matrix) & \textcolor{green!60!black}{Defined} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Status:} \textcolor{orange}{\textbf{OPEN}} --- Reactance not yet in 9C framework
\end{tcolorbox}
...
\midrule
F7: Framing & Loss Aversion $\lambda$ & Appendix K (LIT-FEHR) & Medium \\
F8: Autonomy & Reactance Theory & NEW: Appendix on Reactance & High \\
F14: Path Function & Journey Dynamics $S(t)$ & Appendix AW (STAGE) & Low \\
...
Appendix AAA (CORE-WHO: Welfare Levels L1--L4),
Appendix C (CORE-WHAT: FEPSDE Dimensions),
Appendix AW (CORE-STAGE: Behavioral Change Journey),
...
Appendix BBB (CORE-WHERE: Parameter Estimation --- Tier Hierarchy),
Appendix BA (FORMAL-SEGMENT: Segment Axioms),
Appendix BB (FORMAL-EQUILIBRIA: Intervention Equilibria),
Appendix IV (FORMAL-LET: Level Emergence Theorem),
Appendix AN (METHOD-LLMMC: LLM Monte Carlo Estimation),
Appendix III (METHOD-LLMMC-CAL: Calibration Pipeline for R-Score Decisions),
Appendix XVIII (LIT-FEHR-METHOD: Exclusion Principle),
Appendix G (REF-GLOSSARY: Central Glossary),
Appendix R (METHOD-EVAL: Evaluation Protocol).
