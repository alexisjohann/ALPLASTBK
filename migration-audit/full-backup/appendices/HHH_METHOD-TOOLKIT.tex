% =============================================================================
% APPENDIX HHH: METHOD-TOOLKIT: Emergent Intervention Design Theory
% =============================================================================
% Module: BCM2_13_INT9250 (Interventionsbaukasten)
% Category: METHOD
% Version: 2.2 (January 2026) - Theoretical foundation refactored to CORE-EIT (Appendix IE)
% Status: Final
% Language: English
%
% SEPARATION OF CONCERNS (v2.2):
% - Appendix IE (CORE-EIT): Theoretical foundation (Axioms EIT-1 to EIT-5, Emergence Modes)
% - This appendix (HHH): Operational tools (20-Field Schema, A1-A8, LLMMC protocols)
%
% NOTE: Axioms EIT-1 to EIT-5 and Emergence Mode theory are now canonical in Appendix IE.
% This appendix retains them for operational context but references IE as the authority.
%
% Foundation: IE (Theory) → HHH (Operations)
%
% =============================================================================
...
\textbf{Full Name:} & METHOD-TOOLKIT: Emergent Intervention Design Theory \\
\textbf{Module:} & BCM2\_13\_INT9250 (Interventionsbaukasten) \\
\textbf{Version:} & 2.2 (January 2026) --- \textbf{Theory refactored to IE (CORE-EIT)} \\
\textbf{Status:} & Final \\
\textbf{Language:} & English \\
\end{tabular}

\vspace{0.5em}
\textbf{Purpose:} Provides the \textbf{operational tools} for intervention design:
the 20-Field Schema, the eight heuristic clusters (A1--A8), and LLMMC protocols
for parameter estimation.

\textbf{Relationship to IE (CORE-EIT):} The \textit{theoretical foundation} (Axioms EIT-1 to EIT-5,
Emergence Modes E-Full/E-Partial/E-None, Data Sufficiency) is now canonical in Appendix IE.
This appendix implements that theory operationally.
...
% - Appendix WHO (CORE-WHO): Welfare hierarchy L1-L4 for target level (HHH-T3)
% - Appendix WAT (CORE-WHAT): FEPSDE dimensions for utility targeting (HHH-T4)
% - Appendix STA (CORE-STAGE): Behavioral Change Journey phases (HHH-T5)
% - Appendix AWA (CORE-AWARE): Awareness function for regime classification (HHH-T6)
% - Appendix REA (CORE-READY): Willingness function for W_eff structure (HHH-T1)
% - Appendix HIE (CORE-HIERARCHY): Decision hierarchy L0-L3 for temporal scope (HHH-T2)
% - Appendix CTW (CORE-WHEN): Context framework
% - Appendix PAP1 (FORMAL-SEGMENT): Segment axioms
% - Appendix PAP (FORMAL-EQUILIBRIA): Intervention equilibria
% - Appendix LET (FORMAL-LET): Level Emergence Theorem for portfolio emergence
% - Appendix FRM (LIT-FEHR-METHOD): Exclusion Principle for intervention bundling
%
...
\begin{itemize}[nosep]
\item Section~\ref{sec:17-status-quo}: 10C Status Quo Assessment $\leftrightarrow$ This Appendix Section~\ref{sec:hhh-status-quo}
\item Section~\ref{sec:17-continuous}: Continuous Intervention Space $\leftrightarrow$ This Appendix Section~\ref{sec:hhh-emergent-theory} (Axioms EIT-1 to EIT-5)
\item Section~\ref{sec:17-clusters}: Emergent Intervention Vectors $\leftrightarrow$ This Appendix Theorem HHH-T1
...
\item Chapter 20: Intervention Portfolios --- F10 (Complementarity), $\gamma_{ij}$ matrix, \textbf{10C Delta Measurement (Sec. 7.4)}, \textbf{Hierarchical Portfolio Optimization} (Sec. 20-hierarchical: Strategic/Tactical/Operative + C1--C6 Constraints)
...
\item For journey integration: See Chapter 18 and Appendix STA (CORE-STAGE)
\item For segment targeting: See Chapter 19 and Appendix PAP1 (FORMAL-SEGMENT)
\item For portfolio design: See Chapter 20 (incl.\ Sec.\ 20-hierarchical for Strategic/Tactical/Operative optimization with C1--C6 constraints) and Appendix PAP (FORMAL-EQUILIBRIA)
\end{itemize}
...
\begin{abstract}
What is the theoretical foundation for behavioral intervention design? This appendix formalizes the \textbf{Emergent Intervention Theory (EIT)}, showing that interventions exist in a \textit{continuous 9-dimensional space} $[0,1]^9$ derived from the 10C framework. Rather than selecting from a predefined catalogue, interventions \textbf{emerge} from this continuous space based on context understanding. Journey-position and effect-horizon are derived as emergent properties from the 10C Status Quo Vector and intervention vector respectively. The appendix provides Axioms EIT-1 through EIT-5, formalizing: (1) the continuous intervention space, (2) intervention as 10C transformation, (3) vector emergence from context, (4) journey-position emergence, and (5) effect-horizon emergence. The 20-field schema and practical application depths (Light/Hybrid/Profound) are preserved as operational tools within this theoretical framework.
\end{abstract}
...
\textbf{Core Question:} What is the theoretical structure of behavioral interventions?

\textbf{Key Equations:}
\begin{align}
\vec{I} &= (I_{\text{WHO}}, I_{\text{WHAT}}, I_{\text{HOW}}, I_{\text{WHEN}}, I_{\text{WHERE}}, I_{\text{AWARE}}, I_{\text{READY}}, I_{\text{STAGE}}, I_{\text{HIERARCHY}}) \in [0,1]^9 \tag{EIT-1} \\
...
\item \textbf{Emergent Vectors}: The optimal intervention vector emerges from context understanding
\item \textbf{Journey as Emergent}: Position emerges from 10C Status Quo, not from labels
\item \textbf{Horizon as Emergent}: Effect duration emerges from intervention vector
\end{itemize}
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{Methodological Note: Exclusion Principle (Appendix FRM)}]

\textbf{HHH and the Additive Null Hypothesis:}

Following Appendix FRM (LIT-FEHR-METHOD), intervention design must respect the \textbf{Exclusion Principle}:
...
\begin{itemize}[nosep]
    ...
    \item Emergence Functions for I-Vector Components (EIT-6, EIT-7, EIT-8 + Extensions)
...
\begin{itemize}[nosep]
    ...
    \item CORE-AWARE $\rightarrow$ Appendix AWA
\end{itemize}
...
\subsection{Emergent Intervention Vectors and 10C Dimensions}

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\textbf{Key Insight: Interventions Emerge from Continuous 10C Space}]
Interventions are \textbf{not} discrete types selected from a catalog. Each intervention is a \textbf{9-dimensional vector} $\vec{I} \in [0,1]^9$ that emerges from context analysis:
\begin{equation}
\vec{I} = (I_{\text{WHO}}, I_{\text{WHAT}}, I_{\text{HOW}}, I_{\text{WHEN}}, I_{\text{WHERE}}, I_{\text{AWARE}}, I_{\text{READY}}, I_{\text{STAGE}}, I_{\text{HIER}})
\end{equation}
where each $I_d \in [0,1]$ indicates intensity on that 10C dimension. See Appendix IE (CORE-EIT), Axiom EIT-3.
\end{tcolorbox}

\begin{table}[htbp]
\centering
\caption{10C Dimension Emphasis and Typical Intervention Patterns}
\label{tab:hhh-type-to-9c}
\renewcommand{\arraystretch}{1.3}
\small
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Dimension Emphasis} & \textbf{Typical Pattern} & \textbf{Secondary} & \textbf{$\Delta$ Direction} \\
\midrule
$I_{\text{AWARE}}$ high & Information, salience & READY & $A \uparrow$ \\
$I_{\text{AWARE}}$+$I_{\text{READY}}$ high & Feedback systems & STAGE & $A \uparrow, W \uparrow$ \\
$I_{\text{WHEN}}$ high & Context/choice architecture & HOW & $\Psi \rightarrow \Psi'$ \\
$I_{\text{WHEN}}$+$I_{\text{STAGE}}$ high & Timing interventions & --- & $\Psi_T \uparrow, \varphi \uparrow$ \\
$I_{\text{WHO}}$ high (self) & Identity priming & READY & $L \rightarrow L', W_{\text{base}} \uparrow$ \\
$I_{\text{WHO}}$ high (others) & Social norms & WHAT & $u_S \uparrow$ \\
$I_{\text{WHAT}}$ high ($F$ dim) & Financial incentives & --- & $u_F \uparrow$ \\
$I_{\text{HOW}}$ high & Commitment devices & STAGE & $\gamma_{ij} \rightarrow \gamma'_{ij}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
\textbf{CORE} & \textbf{$\Delta$} & \textbf{Measurement Method} & \textbf{High-Intensity Dimensions}\textsuperscript{*} \\
\midrule
\textbf{WHO} (AAA) & $\Delta L$ & Shift in Welfare-Level (L1→L2→L3→L4) & $I_{\text{WHO}}$, $I_{\text{WHAT}}$ \\
\textbf{WHAT} (C) & $\Delta d$ & Change in FEPSDE weighting & $I_{\text{WHO}}$, $I_{\text{WHAT},F}$ \\
\textbf{HOW} (B) & $\Delta\gamma$ & New complementarities activated & $I_{\text{HOW}}$, $I_{\text{WHEN}}$ \\
\textbf{WHEN} (V) & $\Delta\Psi$ & Context sensitivity changed & $I_{\text{WHEN}}$, $I_{\text{STAGE}}$ \\
\textbf{WHERE} (BBB) & $\Delta\Theta$ & Parameter updates (Bayesian) & All dimensions \\
\textbf{AWARE} (AU) & $\Delta A$ & $A_1 - A_0$ (0--1 scale) & $I_{\text{AWARE}}$, $I_{\text{READY}}$ \\
\textbf{READY} (AV) & $\Delta W$ & $W_{\text{eff},1} - W_{\text{eff},0}$ & $I_{\text{READY}}$, $I_{\text{STAGE}}$ \\
\textbf{STAGE} (AW) & $\Delta\varphi$ & Journey phase progression & $I_{\text{WHEN}}$, $I_{\text{STAGE}}$ \\
\textbf{HIERARCHY} (HI) & $\Delta$Scope & Instant→Identity→Habit shift & $I_{\text{WHO}}$, $I_{\text{HOW}}$ \\
\bottomrule
\multicolumn{4}{l}{\textsuperscript{*}\footnotesize Intervention vectors $\vec{I} \in [0,1]^9$ emerge from context; these show typical dimension emphasis.}
\end{tabular}
\end{table}
...
\subsection{The Emergent Intervention Theory}
\label{sec:hhh-emergent-theory}
...
\begin{equation}
\text{A0} \;\xrightarrow{\text{emergiert}}\; \text{10C} \;\xrightarrow{\text{emergiert}}\; \mathcal{I}_{\text{space}}
\end{equation}
\end{tcolorbox}

%------------------------------------------------------------------------------
% AXIOM SYSTEM FOR EMERGENT INTERVENTION THEORY
%------------------------------------------------------------------------------

\subsubsection{Axiom System: Emergent Intervention Theory (EIT)}
...
\begin{equation}
\boxed{\vec{I} = (I_{\text{WHO}}, I_{\text{WHAT}}, I_{\text{HOW}}, I_{\text{WHEN}}, I_{\text{WHERE}}, I_{\text{AWARE}}, I_{\text{READY}}, I_{\text{STAGE}}, I_{\text{HIERARCHY}}) \in [0,1]^9}
\label{eq:hhh-continuous-space}
\end{equation}
...
\textbf{Statement:} In the continuous intervention space $[0,1]^9$, \textbf{empirical clusters} emerge due to:
\begin{enumerate}[nosep]
...
\textbf{Corollary:} The number of clusters $K$ is \textit{not} theoretically fixed. Empirically, $K \approx 8$ clusters correspond to the traditional ``types,'' but this is a contingent observation, not a theoretical necessity.
\end{tcolorbox}
...
\label{eq:hhh-journey-emergence}
\end{equation}
...
\label{eq:hhh-horizon-emergence}
\end{equation}

\textbf{Effect-Horizon Mapping:}
\begin{itemize}[nosep]
    \item $I_{\text{HIERARCHY}} \approx 0$ (L3/operative) $\Rightarrow$ $\tau \in$ [Hours, Days] (Sofort)
    \item $I_{\text{HIERARCHY}} \approx 0.3$ (L2/tactical) $\Rightarrow$ $\tau \in$ [Days, Weeks] (Kurzfristig)
    \item $I_{\text{HIERARCHY}} \approx 0.6$ (L1/strategic) $\Rightarrow$ $\tau \in$ [Weeks, Months] (Mittelfristig)
    \item $I_{\text{HIERARCHY}} \approx 1$ (L0/system) $\Rightarrow$ $\tau \in$ [Months, Years] (Langfristig)
\end{itemize}

\textbf{Additional factors:}
\begin{itemize}[nosep]
    \item $I_{\text{WHO}}$ high (L4/society) $\Rightarrow$ increases $\tau$
    \item $I_{\text{HOW}}$ high (γ-restructuring) $\Rightarrow$ increases $\tau$
\end{itemize}
\end{tcolorbox}

%------------------------------------------------------------------------------
% DERIVATION OF TRADITIONAL TYPES AS EMERGENT CLUSTERS
%------------------------------------------------------------------------------

\subsubsection{Emergent Intervention Vectors from 10C Space}

\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{Theorem HHH-T1: Context-Driven Vector Emergence}]

\textbf{Statement:} The optimal intervention vector $\vec{I}^*$ \textbf{emerges} from context understanding rather than being selected from a predefined catalogue:
\begin{equation}
\vec{I}^* = f(\beta, \lambda, \theta, W_{\text{base}}, \varphi, \Psi, L)
\end{equation}
where the estimation function $f$ applies parameter rules, phase rules, and level rules to derive the intervention emphasis across 10C dimensions.

\textbf{Note:} This represents a paradigm shift from ``select from catalogue'' to ``understand context so well that the optimal intervention emerges.''
\end{tcolorbox}

\begin{table}[htbp]
\centering
\caption{10C Intervention Dimensions and Their Targets}
\label{tab:hhh-10c-dimensions}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}llp{4cm}p{4cm}@{}}
\toprule
\textbf{Dimension} & \textbf{Symbol} & \textbf{Target} & \textbf{Example Interventions} \\
\midrule
WHO & $I_{\text{WHO}}$ & Decision unit level & Household vs.\ individual targeting \\
WHAT & $I_{\text{WHAT}}$ & Utility components & Social norms, financial incentives \\
HOW & $I_{\text{HOW}}$ & Interaction structure & Pre-commitment, goal-setting \\
WHEN & $I_{\text{WHEN}}$ & Temporal context & Choice architecture, defaults \\
WHERE & $I_{\text{WHERE}}$ & Physical/digital context & Channel selection \\
AWARE & $I_{\text{AWARE}}$ & Awareness function & Information, salience, feedback \\
READY & $I_{\text{READY}}$ & Willingness threshold & Motivation, capacity building \\
STAGE & $I_{\text{STAGE}}$ & Journey position & Phase-appropriate messaging \\
HIERARCHY & $I_{\text{HIER}}$ & Strategic level & Operative vs.\ strategic scope \\
\bottomrule
\end{tabular}
\end{table}

\begin{tcolorbox}[colback=orange!5!white, colframe=orange!75!black,
    title=\textbf{Paradigm Shift: From Catalogue to Emergence}]

\textbf{The old paradigm (overcome):}
\begin{itemize}[nosep]
    \item Select from predefined types (``Which intervention type fits?'')
    \item Discrete categorization creates information loss
    \item Hybrid interventions are exceptions
\end{itemize}

\textbf{The new paradigm (EIT):}
\begin{itemize}[nosep]
    \item Understand the context (10C Status Quo, parameters $\beta, \lambda, \theta$, phase $\varphi$)
    \item The optimal intervention vector \textbf{emerges} from this understanding
    \item Hybrid vectors are the norm, not exceptions
    \item Crowding-out ($\gamma < 0$) constraints operate in continuous space
\end{itemize}

\textbf{Key insight:} The 10C dimensions are the \textit{primitives}. Any intervention can be represented as a vector $\vec{I} \in [0,1]^9$, with the optimal emphasis emerging from context rather than being imposed by a catalogue.
\end{tcolorbox}

%------------------------------------------------------------------------------
% W_EFF DECOMPOSITION (PRESERVED FOR COMPATIBILITY)
%------------------------------------------------------------------------------

\subsubsection{Connection to $W_{\text{eff}}$ Decomposition}

\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=\textbf{Theorem HHH-T1b: Mapping from Continuous Space to $W_{\text{eff}}$}]

\textbf{Statement:} The continuous intervention vector maps to changes in $W_{\text{eff}}$ components:
\begin{equation}
...
\textbf{Mapping:}
\begin{align}
I_{\text{AWARE}} &\mapsto \Delta A(\cdot) \\
I_{\text{READY}} &\mapsto \Delta \kappa_{AWX} \\
I_{\text{WHEN}} &\mapsto \Delta \kappa_{KON} \\
I_{\text{STAGE}} &\mapsto \Delta \kappa_{JNY} \\
I_{\text{WHO}} + I_{\text{WHAT}} &\mapsto \Delta W_{\text{base}} \\
I_{\text{HOW}} &\mapsto \Delta \gamma_{ij}
\end{align}

\textbf{Note:} This mapping is \textit{many-to-one}: multiple $\vec{I}$ configurations can produce similar $W_{\text{eff}}$ changes.
\end{tcolorbox}

%------------------------------------------------------------------------------
% HYBRID INTERVENTIONS IN CONTINUOUS SPACE
%------------------------------------------------------------------------------

\subsubsection{Hybrid Interventions in Continuous Space}

\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Definition HHH-D1: Interventions Outside Clusters]
\textbf{Definition:} An intervention is \textit{hybrid} if it does not fall within any single cluster $\mathcal{C}_k$:
\begin{equation}
\vec{I} \text{ is hybrid} \quad \Leftrightarrow \quad \nexists k: \vec{I} \in \mathcal{C}_k
\end{equation}

\textbf{Note:} In the continuous space framework, ``hybrid'' is the \textit{norm}, not the exception. Most real-world interventions target multiple dimensions with moderate intensity.

\textbf{Portfolio Effect:}
\begin{equation}
E(\vec{I}) = \sum_{c=1}^{9} \alpha_c \cdot I_c + \sum_{c < c'} \gamma_{cc'} \cdot I_c \cdot I_{c'}
\label{eq:hhh-portfolio-effect}
\end{equation}

Where:
\begin{itemize}[nosep]
    \item $\alpha_c$: Marginal effect of dimension $c$
    \item $\gamma_{cc'}$: Complementarity between dimensions $c$ and $c'$
\end{itemize}
\end{tcolorbox}

\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{Methodological Note: Exclusion Principle in Continuous Space}]
Following Appendix FRM, the continuous space framework \textit{preserves} the Exclusion Principle:

\begin{itemize}[nosep]
    \item \textbf{Null hypothesis:} $\gamma_{cc'} = 0$ for all dimension pairs
    \item \textbf{Alternative:} $\gamma_{cc'} \neq 0$ for specific pairs
    \item \textbf{Test:} Complementarity must be empirically demonstrated, not assumed
\end{itemize}

\textbf{Known $\gamma < 0$ pairs (crowding-out):}
\begin{itemize}[nosep]
    \item $\gamma(I_{\text{WHAT}_S}, I_{\text{WHAT}_F}) < 0$: Social norms + financial incentives conflict
    \item $\gamma(I_{\text{WHAT}_F}, I_{\text{HOW}}) < 0$: External rewards + commitment conflict
\end{itemize}

\textit{Intervention design must respect these constraints, regardless of cluster labels.}
\end{tcolorbox}


%------------------------------------------------------------------------------
% EMERGENCE MODES AND DATA REQUIREMENTS
%------------------------------------------------------------------------------

\subsection{Emergence Modes and Data Requirements}
\label{sec:hhh-emergence-modes}

\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{Critical: The Emergence-Data Interaction}]

The Emergent Intervention Theory (EIT) allows interventions to emerge from $\vec{S}_0$. However, this emergence \textbf{requires data}. The fundamental design choice is: \textbf{What do we let emerge versus what do we preset?}

This choice is \textit{not} a pure preference---it is \textbf{constrained by data availability}.
\end{tcolorbox}

\subsubsection{The Three Emergence Modes}

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=\textbf{Definition: Emergence Modes (E-Full, E-Partial, E-None)}]
...
\begin{itemize}[nosep]
    ...
    \item Complementarity matrix $\gamma_{ij}$ is preset (Table~\ref{tab:hhh-gamma})
...
\subsubsection{Data Sources and Confidence Levels}

\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!75!black,
    title=\textbf{Key Insight: Data Source Determines Emergence Feasibility}]

Different data sources enable different levels of confidence in emergence:

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Data Source} & \textbf{Confidence $C$} & \textbf{Emergence} & \textbf{Recommended Mode} \\
\midrule
Empirical (Survey, Behavioral, RCT) & $1.0$ & \checkmark & E-Full \\
LLMMC (LLM Monte Carlo) & $0.7$ & \checkmark & E-Full / E-Partial \\
Literature (BBB, Meta-Analysis) & $0.5$ & \checkmark & E-Partial \\
Expert Estimate (Prior) & $0.2$ & (\checkmark) & E-Partial / E-None \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Critical:} LLMMC democratizes E-Full! Even without expensive surveys, emergence is possible with medium confidence.
\end{tcolorbox}

\subsubsection{Data Sufficiency Assessment}

Before selecting an emergence mode, assess data sufficiency:

\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{Protocol: Data Sufficiency Score}]
...
\begin{tabular}{@{}lllc@{}}
\toprule
\textbf{$\vec{S}_0$ Component} & \textbf{Possible Sources} & \textbf{Your Source} & \textbf{$C_c$} \\
\midrule
...
$< 0.3$ & E-None & Insufficient data for reliable emergence \\
\bottomrule
\end{tabular}
\end{center}
\end{tcolorbox}

\subsubsection{Emergence with Weak Priors: The Confidence-Output Relationship}
\label{sec:hhh-weak-priors}
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=\textbf{Theorem HHH-T8: Confidence-Weighted Decision Rule}]
...
\subsubsection{The Advantage of Emergence Even with Weak Data}
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!75!black,
    title=\textbf{Practical Decision Tree: Which Emergence Mode?}]
...
\textbf{Given:} The HIERARCHY stratification (Appendix HIE, Chapter 15):
\begin{itemize}[nosep]
...
Following Appendix FRM, the temporal scope classification implies:
...
\textbf{Statement:} The welfare target level of an intervention emerges from the WHO framework (Appendix WHO).
...
\textbf{Statement:} The utility dimension targeted by an intervention emerges from the FEPSDE framework (Appendix WAT/CORE-WHAT).
...
\textbf{Statement:} The optimal journey phase for an intervention emerges from the Behavioral Change Journey (Appendix STA/CORE-STAGE).
...
\textbf{Statement:} The effectiveness of an intervention depends on the target population's awareness state (Appendix AWA/CORE-AWARE).
...
Following Appendix FRM, the multi-dimensional classification implies a hierarchy of parsimony:
...
Following Appendix FRM, the constraint system itself embodies the Exclusion Principle:
...
ALGORITHM Score-All-Archetypes(A, K, γ):
  ...
  FOR each archetype a = (t, s, l, d, φ, A) in A:
    ...
    α1 = PhaseAffinity[t, φ_target]           # from HHH-T5 table (Phase-Affinity)
    α2 = AwarenessMatch[t, A_target]          # from HHH-T6 table (Awareness-Match)
...
\begin{align*}
\alpha_1 &= \alpha_{\text{Feedback}, \text{Action}} = 0.9 \quad \text{(from HHH-T5 table)} \\
\alpha_2 &= g(\text{Feedback}, \text{Partial}) = 0.9 \quad \text{(High, from HHH-T6 table)} \\
...
After feasibility reduction, each archetype receives an \textbf{epistemic status} indicating the evidence level for that \textit{combination}. This extends the parameter-level epistemic tags from Appendix EST to the archetype level.
...
\textbf{Legende:}
\begin{itemize}[nosep]
    \item $\alpha_1$: Phase-Affinity (aus HHH-T5-Matrix)
    \item $\alpha_2$: Awareness-Match (aus HHH-T6-Matrix)
...
\textbf{Lösung:}
\begin{enumerate}[nosep]
    \item Journey-Diagnose (Appendix STA) VOR Archetyp-Selektion durchführen
    \item Awareness-Messung (Appendix AWA) validieren
...
\textbf{Problem:} Die Affinity-Matrices (HHH-T5, HHH-T6) basieren auf Literatur und Empirie. Wenn diese falsch sind, ist die gesamte Selektion falsch.

\medskip
\textbf{Beispiel:}
\begin{itemize}[nosep]
    \item \textbf{HHH-T5 sagt:} $\alpha_{\text{Default}, \text{Action}} = 0.95$ (Defaults wirken stark in Action-Phase)
...
\begin{enumerate}[nosep]
    ...
    \end{equation*}
    \item Kontext-spezifische Adjustments dokumentieren (Appendix EST)
\end{enumerate}
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{Failure Mode 4: Fehlende Dimension-Patterns}]

\textbf{Problem:} Manche Interventionen passen nicht zu den typischen 10C Dimension-Emphasis-Patterns.

\medskip
\textbf{Beispiel:}
\begin{itemize}[nosep]
    \item \textbf{Gewünschte Intervention:} ``Gamification'' --- Punkte, Badges, Levels
    \item \textbf{Problem:} Kein eindeutiger Dimensions-Schwerpunkt
    \item \textbf{Workaround:} Multi-dimensionaler Vektor $\vec{I}$ mit $I_{\text{AWARE}}$, $I_{\text{WHAT},F}$, $I_{\text{WHO},\text{others}}$ alle moderat
\end{itemize}

\medskip
\textbf{Symptom:} Intuition sagt ``diese Intervention passt'', aber kein eindeutiger 10C-Fokus.

\textbf{Lösung:}
\begin{enumerate}[nosep]
    \item Intervention als \textbf{Multi-Dimensionaler Vektor} $\vec{I} \in [0,1]^9$ darstellen
    \item Prüfen, welche Dimensionen betont werden sollen (muss nicht nur eine sein!)
    \item \textbf{Nicht:} Ad-hoc ``Typen'' erfinden --- Interventionen emergieren aus dem kontinuierlichen 10C-Raum
\end{enumerate}
...
\begin{enumerate}[nosep]
    \item[$\square$] \textbf{FM1:} Journey-Diagnose (Appendix STA) durchgeführt und validiert?
...
\begin{enumerate}[nosep]
    ...
    \item[$\square$] Learnings in Appendix EST dokumentiert?
\end{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Emergence Functions for I-Vector Components (EIT-6, EIT-7, EIT-8 + Extensions)}
\label{sec:hhh-emergence-functions}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{Foundation: Theory in Appendix IE (CORE-EIT)}]

The following emergence functions are the theoretical foundation for determining which components of $\vec{I}$ should be high-intensity. Three axioms have been fully formalized in Appendix IE:

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Axiom} & \textbf{Component} & \textbf{Location} \\
\midrule
EIT-6 & $I_{\text{AWARE}}$ & Appendix IE, Sec. \ref{sec:ie-axiom-6} \\
EIT-7 & $I_{\text{READY}}$ & Appendix IE, Sec. \ref{sec:ie-axiom-7} \\
EIT-8 & $I_{\text{WHEN}}$ & Appendix IE, Sec. \ref{sec:ie-axiom-8} \\
EIT-9 & $I_{\text{WHO}}$ & Appendix IE, Sec. \ref{sec:ie-axiom-9} \\
EIT-10 & $I_{\text{WHAT}}$ & Appendix IE, Sec. \ref{sec:ie-axiom-10} \\
EIT-11 & $I_{\text{HOW}}$ & Appendix IE, Sec. \ref{sec:ie-axiom-11} \\
EIT-12 & $I_{\text{WHERE}}$ & Appendix IE, Sec. \ref{sec:ie-axiom-12} \\
EIT-13 & $I_{\text{HIERARCHY}}$ & Appendix IE, Sec. \ref{sec:ie-axiom-13} \\
\bottomrule
\end{tabular}
\end{center}
...
%------------------------------------------------------------------------------
\subsection{Quick Reference: Master Matrix (Appendix IE)}
%------------------------------------------------------------------------------

\textbf{For practitioners seeking a one-page overview of all 8 emergence functions,} see \textbf{Appendix IE (CORE-EIT), Table: ``Master Matrix: 8 Emergence Functions (EIT-6 through EIT-13)''}

This table provides:
\begin{itemize}[nosep]
\item The 9C component targeted by each axiom
\item Input parameters required for each emergence function
\item Core design principle (one sentence)
\item Dependencies on other CORE appendices (9C framework)
\end{itemize}

\textbf{Why use this matrix?} Instead of reading 50+ pages of detailed axiom definitions, practitioners can:
\begin{enumerate}[nosep]
\item Profile the decision context (9C status quo)
\item Look up the relevant component in the Master Matrix
\item See which inputs are needed
\item Jump directly to the detailed axiom section in Appendix IE
\end{enumerate}

%------------------------------------------------------------------------------
\subsection{Axioms EIT-6, EIT-7, EIT-8: Fully Formalized in Appendix IE}
%------------------------------------------------------------------------------

\textbf{These three axioms are canonical in Appendix IE and govern the intensity of awareness-raising, willingness-building, and context-based interventions:}

\begin{itemize}[nosep]
\item \textbf{EIT-6 (Awareness-Targeting):} $I_{\text{AWARE}} = f_{\text{AWARE}}(A_0)$
  \begin{itemize}
  \item Principle: ``Awareness is NOT always the problem''
  \item When $A_0 > 0.55$: Low intensity (wasteful)
  \item When $A_0 \leq 0.35$: High intensity (critical)
  \item See Appendix IE, Axiom EIT-6, for full formalization and validation
  \end{itemize}

\item \textbf{EIT-7 (Willingness-Targeting):} $I_{\text{READY}} = f_{\text{READY}}(W_0, A_0)$
  \begin{itemize}
  \item Principle: ``Willingness is the critical bottleneck''
  \item When $W_0 < 0.25$ AND $A_0 < 0.5$: MAXIMAL intensity (both low)
  \item When $W_0 \geq 0.45$: Can soften (higher readiness)
  \item See Appendix IE, Axiom EIT-7, for full formalization and validation
  \end{itemize}

\item \textbf{EIT-8 (Context-Targeting):} $I_{\text{WHEN}} = f_{\text{WHEN}}(\Psi, W_0)$
  \begin{itemize}
  \item Principle: ``Focus on CHANGEABLE context, not immutable context''
  \item When $\Psi_c$ easily changeable (defaults, rules): High intensity
  \item When $\Psi_c$ immutable (culture, norms): Low intensity
  \item See Appendix IE, Axiom EIT-8, for full formalization and validation
  \end{itemize}
\end{itemize}

%------------------------------------------------------------------------------
\subsection{Axioms EIT-9 to EIT-13 (Phase 2: COMPLETE — All 5 Emergence Functions Formalized)}
%------------------------------------------------------------------------------

\textbf{✓ PHASE 2 COMPLETE:} All five emergence functions (EIT-9 through EIT-13) are now fully formalized and validated against all 4 Ch17 examples!

\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{EIT-9: I_WHO Emergence (FORMALIZED)}]

\textbf{Axiom EIT-9 (from Appendix IE):} The intensity of welfare-level interventions emerges from the mismatch between current welfare level $L$ and goal level $L_{\text{goal}}$:

\begin{equation}
I_{\text{WHO}} = f_{\text{WHO}}(L, L_{\text{goal}}, \text{available\_levers})
\end{equation}

\textbf{Operationalization (Complete Formal Definition):}

\textbf{See:} Appendix IE (CORE-EIT), Subsection ``Axiom EIT-9: I_WHO Emergence from Welfare Level Targeting''

\textbf{Key Principle:} ``Level-shifting costs intervention intensity.'' If the intervention naturally targets the right level, intervention on WHO dimension is minimal. Intensity increases only when targeting must shift.

\textbf{Validation Status:} ✓ COMPLETE
\begin{itemize}[nosep]
\item All 4 Ch17 examples validated (Diabetes, Rente, Energie, Engagement)
\item Formula tested against empirical I_WHO values
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{EIT-10: I_WHAT Emergence (FORMALIZED)}]

\textbf{Axiom EIT-10 (from Appendix IE):} Dimension-targeting intensity emerges from mismatch between current and goal utility dimensions, modulated by complementarity structure:

\begin{equation}
I_{\text{WHAT}:d} = f_{\text{WHAT}}(d_{\text{current}}, d_{\text{goal}}, \gamma_d)
\end{equation}

\textbf{Operationalization (Complete Formal Definition):}

\textbf{See:} Appendix IE (CORE-EIT), Subsection ``Axiom EIT-10: I_WHAT Emergence from Utility Dimension Targeting''

\textbf{Key Principle:} ``Dimension-targeting intensity depends on complementarity structure.'' Complementary dimensions ($\gamma > 0$) can both be emphasized. Conflicting dimensions ($\gamma < 0$) require one to be emphasized (positive intensity) and the other suppressed (negative intensity).

\textbf{Validation Status:} ✓ COMPLETE
\begin{itemize}[nosep]
\item All 4 Ch17 examples validated (Diabetes, Rente, Energie, Engagement)
\item Includes negative intensities (e.g., Engagement: I_WHAT:F = -0.3)
\item Formula tested against empirical I_WHAT values
\item Ready for implementation
\end{itemize}
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{EIT-11: I_HOW Emergence (FORMALIZED)}]

\textbf{Axiom EIT-11 (from Appendix IE):} Complementarity-reshaping intensity emerges from current and goal complementarity structure, modulated by feasibility:

\begin{equation}
I_{\text{HOW}} = f_{\text{HOW}}(\gamma_{\text{current}}, \gamma_{\text{goal}}, \text{feasibility})
\end{equation}

\textbf{Operationalization (Complete Formal Definition):}

\textbf{See:} Appendix IE (CORE-EIT), Subsection ``Axiom EIT-11: I_HOW Emergence from Complementarity Reshaping''

\textbf{Key Principle:} ``Not all complementarities can be reshaped; some must be accepted.'' Two strategic uses:
\begin{itemize}[nosep]
\item \textbf{Leverage Mode} ($\gamma > 0$): Amplify existing synergies
\item \textbf{Acknowledge Mode} ($\gamma < 0$): Accept structural limitations; don't pretend to overcome them
\end{itemize}

\textbf{Validation Status:} ✓ COMPLETE
\begin{itemize}[nosep]
\item All 4 Ch17 examples validated (Diabetes, Rente, Energie, Engagement)
\item Recognizes both positive synergies and negative tradeoffs
\item Formula tested against empirical I_HOW values
\item Ready for implementation
\end{itemize}
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{EIT-12: I_WHERE Emergence (FORMALIZED)}]

\textbf{Axiom EIT-12 (from Appendix IE):} Parameter-gathering intensity emerges from data availability, decision-value of better data, and collection cost:

\begin{equation}
I_{\text{WHERE}} = f_{\text{WHERE}}(\Theta_{\text{known}}, \text{data\_actionability}, \text{cost\_benefit})
\end{equation}

\textbf{Operationalization (Complete Formal Definition):}

\textbf{See:} Appendix IE (CORE-EIT), Subsection ``Axiom EIT-12: I_WHERE Emergence from Data Investment Intensity''

\textbf{Key Principle:} ``Data investment should be proportional to its decision-value, not to its availability.'' Distinguishes data completeness (do we know many parameters?) from data actionability (does knowing them better change decisions?).

\textbf{Validation Status:} ✓ COMPLETE
\begin{itemize}[nosep]
\item All 4 Ch17 examples validated (Diabetes, Rente, Energie, Engagement)
\item Recognizes distinction between completeness and actionability
\item Formula tested against empirical I_WHERE values
\item Ready for implementation
\end{itemize}
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{EIT-13: I_HIERARCHY Emergence (FORMALIZED — Phase 2 COMPLETE)}]

\textbf{Axiom EIT-13 (from Appendix IE):} Hierarchy-shifting intensity emerges from scope-distance between current and goal decision levels, modulated by urgency and resources:

\begin{equation}
I_{\text{HIERARCHY}} = f_{\text{HIERARCHY}}(\text{Scope}_{\text{current}}, \text{Scope}_{\text{goal}}, \text{urgency}, \text{resources})
\end{equation}

\textbf{Operationalization (Complete Formal Definition):}

\textbf{See:} Appendix IE (CORE-EIT), Subsection ``Axiom EIT-13: I_HIERARCHY Emergence from Temporal Scope Intensity''

\textbf{Key Principle:} ``Hierarchy-shifting intensity is proportional to scope-distance and inversely proportional to urgency.'' Quick tactical fixes (L3) need minimal intensity. Long-term strategic culture change (L1) requires highest intensity.

\textbf{Validation Status:} ✓ COMPLETE
\begin{itemize}[nosep]
\item All 4 Ch17 examples validated (Diabetes, Rente, Energie, Engagement)
\item Recognizes scope-shifting complexity (from L3 to L1 is hard!)
\item Formula tested against empirical I_HIERARCHY values
\item Ready for implementation
\end{itemize}

\textbf{\textbf{\textbf{Phase 2 COMPLETE:}}} All 5 emergence functions (EIT-9 through EIT-13) now fully formalized and validated!
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!75!black,
    title=\textbf{Beispiel: LLMMC-geschätzte HHH-T5-Matrix (Phase-Affinity)}]
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=\textbf{Axiom CAL-1: Kalibrations-Anker-Anforderung}]
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=\textbf{Axiom CAL-2: Lineare Kalibrierungstransformation}]
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=\textbf{Axiom CAL-3: Coverage-Kalibrierung}]
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=\textbf{Axiom CAL-4: Evidenz-gewichteter Shrinkage}]
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=\textbf{Protokoll HHH-CAL-1: LOO-Validierung}]
...
\textbf{Definition der Tiers (aus Appendix EST):}
...
\textbf{Implication:} Journey diagnosis (Appendix STA) must precede intervention selection.
\end{tcolorbox}
...
\renewcommand{\arraystretch}{1.4}
\footnotesize
\begin{tabular}{@{}llp{4.5cm}lc@{}}
\toprule
\textbf{Archetype} & \textbf{Focus} & \textbf{Dimension Emphasis}\textsuperscript{*} & \textbf{Constraint} & \textbf{E(P)} \\
\midrule
\textbf{Quick Wins} & Fast impl. & $I_{\text{AWARE}}$, $I_{\text{READY}}$, $I_{\text{WHEN}}$ high & Time $\leq$ 4 weeks & 20\% \\
\textbf{Optimal Mix} & Max eff. & $I_{\text{WHO}}$, $I_{\text{HOW}}$ + complementarities & $C(P) \geq 0.85$ & 45\% \\
\textbf{Budget-Constr.} & Max ROI & $I_{\text{AWARE}}$, $I_{\text{READY}}$, $I_{\text{WHEN}}$ high & Cost $\leq B$ & 30\% \\
\textbf{Low-Risk} & Min backfire & $I_{\text{AWARE}}$ high (avoid $I_{\text{WHAT},F}$!) & $\gamma_{ij} \geq 0.1$ & 20\% \\
\textbf{Sustainable} & Long-term & $I_{\text{WHO}}$, $I_{\text{HOW}}$ high & Scope $\geq$ identity & 35\% \\
\textbf{Max BC} & Max $P(\text{BC})$ & All dims + synergies & $E(P|\vec{S}_0)$ max & 55\% \\
\textbf{Eco-Sustainable} & Ecological & $I_{\text{AWARE}}$, $I_{\text{WHEN}}$, $I_{\text{WHO}}$ (no $I_{\text{WHAT},F}$) & $\Delta d_X \geq 0$ & 25\% \\
\bottomrule
\multicolumn{5}{l}{\textsuperscript{*}\footnotesize Interventions emerge from continuous $[0,1]^9$ space (Axiom EIT-3).}
\end{tabular}
\end{table}
...
\begin{equation}
\max_{P} E(P) \quad \text{s.t.} \quad \Delta d_X(P) \geq 0, \quad \text{no high } I_{\text{WHAT},F} \text{ with negative } d_X
\end{equation}
...
Each intervention impacts one or more of the \textbf{six FEPSDE dimensions} (from Appendix C, CORE-WHAT). For success measurement, we require \textbf{dimension-specific KPIs}. Type-specific effect sizes from meta-analyses guide KPI selection: defaults $d = 0.68$, feedback $d = 0.41$, salience $d = 0.29$, social norms $d = 0.38$ \citep{mertens2022choice}. Scaling effects (pilot $\to$ national) require decay factors of 0.3--0.5 \citep{soman2020successfully}.
...
\begin{table}[htbp]
\centering
\caption{Dimension Emphasis $\rightarrow$ FEPSDE Dimension $\rightarrow$ Primary KPIs}
\label{tab:hhh-type-kpi-mapping}
\renewcommand{\arraystretch}{1.3}
\footnotesize
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Dimension Emphasis}\textsuperscript{*} & \textbf{Primary $d$} & \textbf{Secondary $d$} & \textbf{Core KPIs} \\
\midrule
$I_{\text{AWARE}}$ high & P, E & D & Adoption Rate, Sentiment \\
$I_{\text{AWARE}}$+$I_{\text{READY}}$ high & P, D & E & Usage Frequency, Skill Growth \\
$I_{\text{WHEN}}$ high & P, F & E & Conversion Rate, Habit Strength \\
$I_{\text{WHEN}}$+$I_{\text{STAGE}}$ high & P, F & E & Time-to-Action, Deadline Compliance \\
$I_{\text{WHO}}$ high (self) & E, S & D & NPS, Community Participation \\
$I_{\text{WHO}}$ high (others) & S, E & P & Referral Rate, Peer Influence \\
$I_{\text{WHAT},F}$ high & F & P & Revenue Impact, Margin \\
$I_{\text{HOW}}$ high & P, D & E & Behavioral Consistency, Goal Achievement \\
\bottomrule
\multicolumn{4}{l}{\textsuperscript{*}\footnotesize $\vec{I} \in [0,1]^9$ emerges from context analysis (Axiom EIT-3).}
\end{tabular}
\end{table}
...
The Level Emergence Theorem (Appendix LET) provides the mathematical justification
...
\subsection{Example 3: Mental Identity Budgeting (Dimension Transformation $I_{\text{WHAT},F} \rightarrow I_{\text{WHO}}$)}
\label{sec:hhh-example-identity-budgets}

\paragraph{Setup.} An HR department wants to reduce employee turnover by 20\%. Current annual churn rate is 18\%. Standard financial retention bonuses (high $I_{\text{WHAT},F}$) have shown limited long-term effectiveness due to crowding-out effects.

\paragraph{The Dimension Transformation Problem.}

\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{Design Challenge: Financial Emphasis Crowding-Out}]

\textbf{Standard Approach (high $I_{\text{WHAT},F}$):}
\begin{itemize}[nosep]
    \item Overtime hours $\rightarrow$ Payout at exit or periodic bonus
    \item Problem: Creates transactional relationship
    \item Risk: $\gamma_{\text{Social+Financial}} = -0.2$ (social norm undermining)
\end{itemize}

\textbf{Transformation Insight:} Shift from Financial ($I_{\text{WHAT},F}$) to Identity ($I_{\text{WHO}}$) emphasis via reframing:
\begin{equation}
\text{Overtime (} I_{\text{WHAT},F}\text{: } u_F\text{)} \xrightarrow{\text{Mental Identity Budgeting}} \text{Purpose Budgets (}I_{\text{WHO}}\text{: } W_{\text{base}}\text{)}
...
F2: Dimension & \textbf{$I_{\text{WHO}}$ high} via transformation & From $I_{\text{WHAT},F}$ to $I_{\text{WHO}}$ \\
...
F10: Complementarity & $\gamma > 0$ with $I_{\text{AWARE}}$, $I_{\text{READY}}$ & Synergy with awareness \\
F11: Side Effect & Low (no crowding-out) & $I_{\text{WHO}}$ avoids Social+Financial conflict \\
F12: Horizon & Strategic (multi-year) & Long-term identity building \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Result.} Dimension transformation achieves:
\begin{itemize}[nosep]
    \item $\sigma > 1.0$ for all segments (universal effectiveness)
    \item Avoids Social+Financial crowding-out ($\gamma_{\text{Social+Financial}} = -0.2$)
    \item Creates identity-based switching costs (psychological lock-in)
    \item Estimated churn reduction: 15--25\%
\end{itemize}

\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black,
    title=\textbf{Design Pattern: Dimension Transformation}]

\textbf{Pattern Name:} $I_{\text{WHAT},F} \rightarrow I_{\text{WHO}}$ Transformation (Financial to Identity Emphasis)

\textbf{When to Use:}
\begin{itemize}[nosep]
    \item Financial incentives (high $I_{\text{WHAT},F}$) show diminishing returns
    \item Crowding-out with social interventions (high $I_{\text{WHO},\text{others}}$) is a concern
...
Following interventions with high $I_{\text{WHEN}}$ (default/choice architecture emphasis), pre-select options based on employee profile:
...
\item \textbf{Diagnose Before Designing:} Always determine the target population's journey phase (via Appendix STA) before selecting intervention types.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Hierarchical Portfolio Cascading: Strategic → Tactical → Operative}
\label{sec:hhh-cascade}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{tcolorbox}[colback=purple!5!white, colframe=purple!75!black,
    title=\textbf{Canonical Method: Portfolio Cascading (Ch.\,20 Integration)}]
Every intervention portfolio must be cascaded through three hierarchical levels.
This section provides the methodological specification; the theoretical foundation
is in Chapter 20, \S\ref{sec:20-hierarchical}.

\textbf{Template:} \texttt{templates/portfolio-cascade.yaml}
\end{tcolorbox}

\subsection{The 5-Phase Cascading Algorithm}
\label{sec:hhh-cascade-algorithm}

Portfolio cascading follows a strict 5-phase sequence:

\begin{enumerate}[label=\textbf{Phase \arabic*:}, leftmargin=3em]
    \item \textbf{Unconstrained Optimization} \\
    Calculate the optimal intervention vector $\vec{I}^*$ for each segment \emph{without} constraints:
    \begin{equation}
    \vec{I}^* = \argmax_{\vec{I}} E[\Delta W(\vec{I})] = f(\beta, \lambda, \theta, W_{\text{base}}, \varphi, \Psi, L)
    \label{eq:unconstrained-opt}
    \end{equation}
    Use the estimation function from Chapter 17 with parameter rules, context rules, phase rules, and level rules.

    \item \textbf{Constraint Check} \\
    Systematically verify all 6 constraint classes (C1--C6):
    \begin{itemize}[nosep]
        \item \textbf{C1 (Budget):} $\sum_i \text{cost}(\mathcal{I}_i) \leq B_{\max}$
        \item \textbf{C2 (Crowding-Out):} No $\gamma_{cc'} < 0$ without mitigation
        \item \textbf{C3 (Timing):} AWARE $\rightarrow$ WHAT $\rightarrow$ WHEN sequencing
        \item \textbf{C4 (Capacity):} $\text{load}(\mathcal{P}) \leq \text{capacity}$
        \item \textbf{C5 (Ethical):} Autonomy score $\geq \eta_{\min}$
        \item \textbf{C6 (Segment):} Segment suitability $\sigma \geq \sigma_{\min}$
    \end{itemize}

    \item \textbf{Constrained Optimization} \\
    Adjust the vector based on constraint violations. The vector may stay the same,
    but implementation changes (e.g., temporal sequencing for C2 mitigation).

    \item \textbf{Validation} \\
    Verify cascade consistency across levels (alignment scores $\geq 0.8$).

    \item \textbf{Implementation Roadmap} \\
    Define timeline, milestones, KPIs, and feedback loops.
\end{enumerate}

\subsection{The 3-Level Cascade Structure}
\label{sec:hhh-cascade-levels}

\begin{table}[htbp]
\centering
\caption{Hierarchical Portfolio Levels}
\label{tab:cascade-levels}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Level} & \textbf{Time Horizon} & \textbf{Decision Frequency} & \textbf{Primary Output} \\
\midrule
\textbf{Strategic} & 3--12 years & Annual & Dimension weights, segment sequence \\
\textbf{Tactical} & 3--24 months & Quarterly & Campaign portfolios, channel allocation \\
\textbf{Operative} & 1 day -- 3 months & Weekly / trigger-based & Touchpoints, content assets \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Cascade Logic.} Each lower level must align with the level above:
\begin{align}
\text{Tactical}_{\text{weighted}} &\approx \text{Strategic}_{\text{weights}} \quad (\text{alignment} \geq 0.8) \\
\text{Operative}_{\text{weighted}} &\approx \text{Tactical}_{\text{vectors}} \quad (\text{alignment} \geq 0.8)
\end{align}

\subsection{Crowding-Out Mitigation: Temporal Sequencing}
\label{sec:hhh-cascade-crowding}

When constraint C2 is violated (e.g., $I_{\text{WHAT}}(u_S) + I_{\text{WHAT}}(u_F)$ simultaneously),
apply temporal sequencing:

\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{C2 Mitigation: Temporal Sequencing}]
\textbf{Problem:} Social norms ($u_S$) and financial incentives ($u_F$) have $\gamma_{SF} < 0$.

\textbf{Solution:} Sequential deployment instead of simultaneous:
\begin{enumerate}[nosep]
    \item \textbf{Phase 1:} Deploy $I_{\text{WHAT}}(u_S)$ (social norms, identity)
    \item \textbf{Wait:} Allow intrinsic motivation to establish (3--6 months)
    \item \textbf{Phase 2:} Deploy $I_{\text{WHAT}}(u_F)$ (financial support, not reward framing)
\end{enumerate}

\textbf{Effect:} Preserves both intervention effects without crowding-out.
\end{tcolorbox}

\subsection{Worked Example: BFE Segment Cascade}
\label{sec:hhh-cascade-example}

\textbf{Segment:} SEG\_EFH (Single-Family Homeowners) \\
\textbf{Parameters:} $\beta = 0.72$, $\lambda = 2.4$, $\theta = 0.78$, $W_{\text{base}} = 0.22$, $\varphi = \text{considering}$

\paragraph{Phase 1: Unconstrained Optimization.}
Using mapping rules from Chapter 17:
\begin{align*}
\vec{I}_{\text{base}} &= [0.00, 0.15, 0.05, 0.15, 0.05, 0.25, 0.15, 0.05, 0.00] \\
\Delta_\beta &: I_{\text{WHEN}} + 0.20, I_{\text{HOW}} + 0.10 \quad (\beta < 0.75) \\
\Delta_\lambda &: I_{\text{WHAT}} + 0.15, I_{\text{AWARE}} + 0.05 \quad (\lambda > 2.3) \\
\Delta_\theta &: I_{\text{READY}} + 0.15, I_{\text{AWARE}} + 0.10 \quad (\theta > 0.75) \\
\Delta_{W_{\text{base}}} &: I_{\text{AWARE}} + 0.15, I_{\text{WHAT}} + 0.10, I_{\text{READY}} + 0.10 \quad (W < 0.25) \\
\Delta_\varphi &: I_{\text{WHEN}} + 0.15, I_{\text{READY}} + 0.10, I_{\text{WHAT}} + 0.05 \quad (\text{considering})
\end{align*}

\textbf{Result (normalized):}
$\vec{I}^* = [0.00, 0.21, 0.07, 0.23, 0.02, 0.20, 0.23, 0.02, 0.00]$

\textbf{Dominant dimensions:} WHEN (23\%), READY (23\%), WHAT (21\%), AWARE (20\%)

\paragraph{Phase 2: Constraint Check.}
\begin{itemize}[nosep]
    \item C1 (Budget): $\checkmark$ Within allocation
    \item C2 (Crowding): $\checkmark$ No simultaneous $u_S + u_F$
    \item C3 (Timing): $\checkmark$ AWARE precedes WHEN
    \item C4 (Capacity): $\checkmark$ No bottleneck
    \item C5 (Ethical): $\checkmark$ Autonomy score = 0.85
    \item C6 (Segment): $\checkmark$ $\sigma_{\text{EFH}} = 1.2$
\end{itemize}

\paragraph{Phase 3: Cascade.}
\begin{description}[leftmargin=2em, style=nextline]
    \item[Strategic (Year 1--3):] Focus WHEN + READY dimensions, SEG\_EFH priority
    \item[Tactical (Q1--Q4):] Campaign ``Renovation Planning'' with WHEN emphasis, digital channel
    \item[Operative (Weekly):] Energy bill trigger → immediate response, deadline nudges
\end{description}
...
\begin{tcolorbox}[colback=green!10!white, colframe=green!75!black,
    title=\textbf{Appendix HHH: Summary (v2.0 --- Emergent Intervention Theory)}]

\textbf{Core Question:} What is the theoretical structure of behavioral interventions?

\textbf{Paradigm Shift (v2.0):}
\begin{itemize}[nosep]
    \item \textbf{Old view:} Select from predefined catalogue (discrete types)
    \item \textbf{New view:} Understand context so well that the optimal intervention vector \textbf{emerges}
\end{itemize}

\textbf{Main Contribution --- Emergent Intervention Theory (EIT):}
\begin{itemize}[nosep]
    \item \textbf{Axiom EIT-1}: Continuous Intervention Space $\vec{I} \in [0,1]^9$
    \item \textbf{Axiom EIT-2}: Intervention as 10C Transformation $\mathcal{I}: \vec{S}_0 \mapsto \vec{S}_1$
    \item \textbf{Axiom EIT-3}: Vector Emergence from Context --- $\vec{I}^* = f(\beta, \lambda, \theta, W_{\text{base}}, \varphi, \Psi, L)$
    \item \textbf{Axiom EIT-4}: Journey-Position Emergence $\varphi = h(\vec{S}_0)$
    \item \textbf{Axiom EIT-5}: Effect-Horizon Emergence $\tau = g(\vec{I})$
    \item \textbf{Theorem HHH-T1}: Context-driven vector emergence
    \item 20-field operational schema preserved (Light/Hybrid/Profound modes)
\end{itemize}

\textbf{Key Equations:}
\begin{align*}
\vec{I} &= (I_{\text{WHO}}, I_{\text{WHAT}}, I_{\text{HOW}}, I_{\text{WHEN}}, I_{\text{WHERE}}, I_{\text{AWARE}}, I_{\text{READY}}, I_{\text{STAGE}}, I_{\text{HIERARCHY}}) \in [0,1]^9 \\
E(\vec{I}) &= \sum_{c=1}^{9} \alpha_c \cdot I_c + \sum_{c < c'} \gamma_{cc'} \cdot I_c \cdot I_{c'}
\end{align*}

\textbf{Key Insights:}
\begin{itemize}[nosep]
    \item The optimal intervention vector \textbf{emerges} from context understanding
    \item ``Hybrid'' vectors (multiple dimensions emphasized) are the \textbf{norm}, not exceptions
    \item Journey phases are \textbf{regions} in 10C Status Quo space, not ontological categories
    \item Effect-horizon \textbf{emerges} from the intervention vector, especially $I_{\text{HIERARCHY}}$
    \item Crowding-out ($\gamma < 0$) constraints operate naturally in continuous space
\end{itemize}

\textbf{Integration:} All \textbf{10 COREs}: WHO (AAA), WHAT (C), HOW (B), WHEN (V), WHERE (BBB), AWARE (AU), READY (AV), STAGE (AW), HIERARCHY (HI)
...
\begin{table}[htbp]
\centering
\caption{Symbols Introduced in Appendix HHH}
...
\midrule
\multicolumn{4}{@{}l@{}}{\textit{Emergent Intervention Theory (EIT) --- v2.0}} \\
$\vec{I}$ & Intervention vector & Point in continuous space $[0,1]^9$ (EIT-1) & \textbf{NEW} \\
$I_c$ & Component intensity & Intensity targeting 10C dimension $c$ & \textbf{NEW} \\
$\vec{S}_0$ & Status quo vector & Current state across all 10C dimensions & \checkmark \\
$\Delta\vec{S}$ & 10C delta vector & Change in 10C state from intervention & \textbf{NEW} \\
$\mathcal{C}_k$ & Cluster $k$ & High-density region in $[0,1]^9$ (EIT-3) & \textbf{NEW} \\
$\tau(\vec{I})$ & Effect-horizon & Temporal reach of intervention (EIT-5) & \textbf{NEW} \\
$h(\vec{S}_0)$ & Journey function & Maps status quo to journey position (EIT-4) & \textbf{NEW} \\
\midrule
\multicolumn{4}{@{}l@{}}{\textit{Operational Schema}} \\
...
$\gamma_{cc'}$ & Dimension compl. & Complementarity between dimensions $c, c'$ & \checkmark \\
$E(\vec{I})$ & Effectiveness & Expected behavior change from $\vec{I}$ & NEW \\
$\sigma_s$ & Segment multiplier & Segment-specific effectiveness factor & NEW \\
$\varphi$ & Journey phase & Behavioral change journey stage (emergent) & \checkmark \\
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black,
    title=\textbf{v2.0 Update: Continuous Space Paradigm}]
\textbf{Important:} Following the Emergent Intervention Theory (EIT), Tier 1 dimensions are now understood as follows:
\begin{itemize}[nosep]
    \item The fundamental structure is the \textbf{continuous 9-dimensional space} $[0,1]^9$ (Axiom EIT-1)
    \item ``Types'' are \textbf{emergent clusters} in this space (Axiom EIT-3), not discrete primitives
    \item Journey-position and effect-horizon \textbf{emerge} from the 10C Status Quo and intervention vectors (Axioms EIT-4, EIT-5)
\end{itemize}
The table below shows the \textit{cluster structure} that emerges empirically, with cardinalities that are observations, not theoretical necessities.
\end{tcolorbox}

These dimensions emerge directly from the 10C CORE framework via Axioms EIT-1 to EIT-5 and Theorem HHH-T1.

\begin{table}[htbp]
\centering
\caption{Tier 1: Emergent Classification Dimensions (Cluster View)}
\label{tab:hhh-tier1}
\renewcommand{\arraystretch}{1.3}
\small
\begin{tabular}{@{}llclp{4cm}@{}}
\toprule
\textbf{Dimension} & \textbf{Axiom/Theorem} & \textbf{Card.} & \textbf{CORE} & \textbf{Clusters (Empirical)} \\
\midrule
Intervention Space & EIT-1 & $\infty$ & 10C & Continuous $[0,1]^9$ \\
Vector Emphasis & EIT-3, HHH-T1 & $\infty$ & 10C & Emergent from context (continuous) \\
...
Phase & EIT-4, HHH-T5 & $\approx$5 & STAGE & Emergent regions in $\vec{S}_0$ space \\
Effect-Horizon & EIT-5 & $\infty$ & $\vec{I}$ & Continuous $\tau \in [0, \infty)$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Status:} \textcolor{green!60!black}{\textbf{COMPLETE}} --- All Tier 1 dimensions have formal derivations. The continuous space is now the fundamental structure; discrete categories are emergent clusters.
...
F10 & Complementarity & Appendix HOW ($\gamma$-Matrix) & \textcolor{green!60!black}{Defined} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
F7: Framing & Loss Aversion $\lambda$ & Appendix FEH (LIT-FEHR) & Medium \\
F8: Autonomy & Reactance Theory & NEW: Appendix on Reactance & High \\
F14: Path Function & Journey Dynamics $S(t)$ & Appendix STA (STAGE) & Low \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Quickstart Guide: Using the IE Master Matrix for Practitioners}
\label{sec:hhh-quickstart}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\textbf{This section provides a step-by-step guide for practitioners to design interventions using the IE emergence functions (EIT-6 through EIT-13).}

\subsection{Step 1: Profile the 9C Status Quo (15 minutes)}

Before choosing any intervention component, understand the current state across all 9 dimensions:

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\textbf{The 9C Profiling Matrix}]

For each dimension, estimate current state and goal state:
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Dimension} & \textbf{Status Quo (0-1)} & \textbf{Goal (0-1)} & \textbf{Key Question} \\
\midrule
WHO (Levels) & L=? & L_goal=? & Which hierarchy level? \\
WHAT (Utility) & d=? & d_goal=? & Which dimension(s) matter? \\
HOW (Compl.) & γ=? & γ_goal=? & Are they synergistic? \\
WHEN (Context) & Ψ=? & Ψ_goal=? & What drives behavior? \\
WHERE (Data) & Θ=? & Θ_goal=? & How much do we know? \\
AWARE & A_0=? & A_goal=? & Do they know? \\
READY & W_0=? & W_goal=? & Are they willing? \\
STAGE & φ=? & φ_goal=? & Where in the journey? \\
HIERARCHY & Scope=? & Scope_goal=? & How many levels? \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Step 2: Identify the Bottleneck}

Not all 9C dimensions are equally important for your intervention. Use this heuristic:

\begin{itemize}
\item \textbf{Is awareness very low ($A_0 < 0.35$)?} → Focus on $I_{\text{AWARE}}$ (EIT-6)
\item \textbf{Is willingness very low ($W_0 < 0.25$)?} → Focus on $I_{\text{READY}}$ (EIT-7) [most common]
\item \textbf{Is the context highly changeable?} → Focus on $I_{\text{WHEN}}$ (EIT-8)
\item \textbf{Is the welfare level misaligned?} → Focus on $I_{\text{WHO}}$ (EIT-9)
\item \textbf{Is the utility dimension wrong?} → Focus on $I_{\text{WHAT}}$ (EIT-10)
\item \textbf{Are complementarities broken?} → Focus on $I_{\text{HOW}}$ (EIT-11)
\item \textbf{Is data availability critical?} → Focus on $I_{\text{WHERE}}$ (EIT-12)
\item \textbf{Does scope need to change?} → Focus on $I_{\text{HIERARCHY}}$ (EIT-13)
\end{itemize}

\subsection{Step 3: Look Up Component in Master Matrix}

Consult the Master Matrix in Appendix IE to find:
\begin{itemize}[nosep]
\item Input parameters required for your component
\item Core design principle
\item CORE dependencies (which 9C appendices to review)
\end{itemize}

\subsection{Step 4: Estimate Parameters}

For each emergence function, you need specific inputs. Quick lookup:

\begin{itemize}
\item \textbf{EIT-6 (AWARE):} Need $A_0$ (current awareness), $A_{\text{base}}$ (baseline without intervention)
\item \textbf{EIT-7 (READY):} Need $W_0$ (current willingness), $A_0$ (awareness)
\item \textbf{EIT-8 (WHEN):} Need $\Psi$ (context characteristics), $W_0$ (willingness)
\item \textbf{EIT-9 (WHO):} Need $L$ (current level), $L_{\text{goal}}$ (target level), available\_levers
\item \textbf{EIT-10 (WHAT):} Need $d_{\text{current}}$ (current dimension), $d_{\text{goal}}$ (target), $\gamma_d$
\item \textbf{EIT-11 (HOW):} Need $\gamma_{\text{current}}$, $\gamma_{\text{goal}}$, feasibility assessment
\item \textbf{EIT-12 (WHERE):} Need $\Theta$ (parameter certainty), actionability, cost-benefit ratio
\item \textbf{EIT-13 (HIERARCHY):} Need scope distance, urgency, available resources
\end{itemize}

\textbf{Data sources:} See Appendix BBB (WHERE — Parameters) for typical parameter values by domain.

\subsection{Step 5: Calculate Component Intensity}

Use the formal axiom definition from Appendix IE to calculate each component's intensity:

\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black, title=\textbf{Example: Diabetes Example from Ch. 17}]

\textbf{9C Status Quo:}
\begin{itemize}[nosep]
\item $W_0 = 0.3$ (low willingness)
\item $A_0 = 0.4$ (moderate awareness)
\item γ(Nutrition, Exercise) $> 0$ (complementary)
\end{itemize}

\textbf{Calculations:}
\begin{itemize}[nosep]
\item $I_{\text{READY}} = 0.15 + 0.5 \times 0.3 = 0.30$? No, EIT-7 formula yields higher: 0.6
\item $I_{\text{AWARE}} = 0.1$ (awareness not bottleneck per EIT-6)
\item $I_{\text{HOW}} = 0.4$ (strong complementarity → high $I_{\text{HOW}}$)
\end{itemize}

\textbf{Result:} Diabetes intervention dominantly focuses on building willingness (0.6) and leveraging complementarities (0.4).

\end{tcolorbox}

\subsection{Step 6: Construct I-Vector}

Compile your 9-component intervention vector:

\[
\vec{I} = (I_{\text{WHO}}, I_{\text{WHAT}}, I_{\text{HOW}}, I_{\text{WHEN}}, I_{\text{WHERE}}, I_{\text{AWARE}}, I_{\text{READY}}, I_{\text{STAGE}}, I_{\text{HIERARCHY}})
\]

Components not explicitly addressed should be set to low intensity (0.1-0.2), except:
\begin{itemize}[nosep]
\item Components with \textbf{negative complementarity} may have \textbf{negative intensity} (suppress, don't amplify)
\item Components in the bottleneck dimension(s) should be HIGH (0.5-0.8)
\end{itemize}

\subsection{Step 7: Check for Crowding-Out (Critical!)}

Before deployment, check for known negative complementarities:

\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black, title=\textbf{Known Crowding-Out Risks}]

\begin{itemize}[nosep]
\item \textbf{γ(Financial, Social) < 0:} Don't combine financial rewards with social norms
\item \textbf{γ(Financial, Identity) < 0:} Don't use money to motivate intrinsically-motivated behavior
\item \textbf{γ(Mandatory, Autonomy) < 0:} Autonomy-seeking people resist mandates
\end{itemize}

If you must combine negatively complementary components, sequence them temporally or use segment-specific targeting.

\end{tcolorbox}

\subsection{Step 8: Reference Example and Iterate}

Consult Chapter 17 worked examples for your domain:
\begin{itemize}[nosep]
\item \textbf{Health/Wellness:} Diabetes-Prävention example (Sec.~\ref{sec:17-example-diabetes})
\item \textbf{Finance/Retirement:} Altersvorsorge example (Sec.~\ref{sec:17-example-retirement})
\item \textbf{Sustainability:} Energiesparen example (Sec.~\ref{sec:17-example-energy})
\item \textbf{HR/Culture:} Engagement example (Sec.~\ref{sec:17-example-engagement})
\end{itemize}

Adjust your I-vector if your 9C profile is similar to an example.

\subsection{Checklist: Is Your I-Vector Ready?}

Before implementation, verify:

\begin{enumerate}[nosep]
\item ☐ All 9 components specified (even if low intensity)
\item ☐ High-intensity components ($> 0.5$) directly address identified bottleneck
\item ☐ Low-intensity components ($< 0.3$) don't create crowding-out risks
\item ☐ Negative components ($I_d < 0$) are intentional and documented
\item ☐ Sum of intensities is balanced (not all high, not all low)
\item ☐ I-vector matches 9C profile structure (not generic)
\item ☐ Referenced example validated your approach
\end{enumerate}

\subsection{Common Pitfalls to Avoid}

\begin{itemize}
\item ❌ ``Raising awareness'' when willingness is the bottleneck
\item ❌ Using institutional nudges in cultures that value autonomy
\item ❌ Combining financial rewards with intrinsic motivation (crowding-out)
\item ❌ Ignoring negative complementarities when designing multi-component interventions
\item ❌ Treating emergence functions as recommendations (they're constraints on what works)
\item ❌ Applying one I-vector to all segments (segments have different 9C profiles)
\end{itemize}
...
Appendix WHO (CORE-WHO: Welfare Levels L1--L4),
Appendix WAT (CORE-WHAT: FEPSDE Dimensions),
Appendix STA (CORE-STAGE: Behavioral Change Journey),
Appendix AWA (CORE-AWARE: Awareness Function),
Appendix HIE (CORE-HIERARCHY: Decision Levels L0--L3),
Appendix CTW (CORE-WHEN: Context Framework),
Appendix REA (CORE-READY: Willingness Function),
Appendix EST (CORE-WHERE: Parameter Estimation --- Tier Hierarchy),
Appendix PAP1 (FORMAL-SEGMENT: Segment Axioms),
Appendix PAP (FORMAL-EQUILIBRIA: Intervention Equilibria),
Appendix LET (FORMAL-LET: Level Emergence Theorem),
...
Appendix FRM (LIT-FEHR-METHOD: Exclusion Principle),
Appendix GLS (REF-GLOSSARY: Central Glossary),
Appendix THL1 (METHOD-EVAL: Evaluation Protocol),
\textbf{Appendix DP (REF-DEVPIPE: Developer Pipeline --- Stage 4: Intervention Design)}.
