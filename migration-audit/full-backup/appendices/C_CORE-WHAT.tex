...
\begin{itemize}[nosep]
\item Appendix AAA (CORE-WHO): Provides bearers whose utility dimensions WHAT specifies
\item Appendix B (CORE-HOW): Uses WHAT dimensions for complementarity matrix
\item Appendix V (CORE-WHEN): Context affects dimension weights
\end{itemize}
...
\textbf{Out-of-Scope:}
\begin{itemize}[nosep]
    \item Whose utility $\rightarrow$ Appendix AAA (CORE-WHO)
    \item Cross-dimension interactions $\rightarrow$ Appendix B (CORE-HOW)
    \item Context-dependent weighting $\rightarrow$ Appendix V (CORE-WHEN)
    \item Empirical parameter values $\rightarrow$ Appendix BBB (CORE-WHERE)
\end{itemize}
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!75!black,
    title=Appendix C Summary: CORE-WHAT]
...
\textbf{Next Steps:}
See Appendix B (CORE-HOW) for cross-dimension interactions ($\gamma_{dd'}$).
See Appendix V (CORE-WHEN) for context-dependent weights ($\alpha_d(\Psi)$).
...
\begin{table}[htbp]
\centering
\caption{Symbols Introduced in Appendix C}
