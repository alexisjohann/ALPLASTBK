...
The optimization problem defined in Appendix MP maximizes $P_{eff}$ over interventions $I$. But not all interventions are feasible -- context and state impose constraints that define the \textbf{solution space}.
...
The EMERGE algorithm (Appendix IE) can be understood as a \textbf{solution space generator}: given context $\Psi$ and state $S$, it computes $\mathcal{S}(\Psi, S)$.
...
Appendix MP defines the optimization:
\begin{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Appendix MP (Master Problem)}: Objective function and state evolution
    \item \textbf{Appendix IE (EMERGE)}: Algorithm for computing interventions
    \item \textbf{Appendix V (WHEN)}: Context dimensions that determine boundaries
    \item \textbf{Appendix B (HOW)}: Complementarity structure creating coupling
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{EMERGE Integration}: The EMERGE algorithm from Appendix IE is reinterpreted as a solution space generator that computes $\mathcal{S}(\Psi, S)$ from context and state.

    \item \textbf{MP Connection}: This appendix provides the constraint set for the optimization problem in Appendix MP, completing the formal optimization framework with: $\max P_{eff}$ subject to $I_t \in \mathcal{S}(\Psi_t, S_t)$.
\end{enumerate}
...
\begin{itemize}
    \item Appendix MP: Master optimization problem formulation
    \item Appendix V: Context dimensions determining solution space
    \item Appendix IE: EMERGE algorithm for intervention generation
    \item Appendix B: Complementarity creating cross-dimensional constraints
    \item Appendix AAA: WHO dimension scope constraints
    \item Appendix C: WHAT dimension utility constraints
    \item Appendix BBB: WHERE parameter constraints
\end{itemize}
