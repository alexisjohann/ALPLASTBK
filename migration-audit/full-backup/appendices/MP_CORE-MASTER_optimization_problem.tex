...
\textbf{Out-of-Scope:}
\begin{itemize}[nosep]
    \item \textbf{Lösungsalgorithmen} $\rightarrow$ Appendix AN (METHOD-LLMMC), Appendix EEE (METHOD-DESIGN)
    \item \textbf{Empirische Kalibrierung} $\rightarrow$ Appendix BBB (CORE-WHERE), METHOD appendices
    \item \textbf{Domain-spezifische Anwendungen} $\rightarrow$ DOMAIN appendices (AA--AG, AJ, AK, W--Z)
    \item \textbf{Interventions-Design Workflow} $\rightarrow$ Appendix HHH (METHOD-TOOLKIT), /design-intervention
...
\begin{enumerate}[nosep]
    \item \textbf{Master-Problem (MP-1):} Vollständige Optimierungsgleichung mit Objective, Constraints, Dynamics
...
\textbf{Reference:} Appendix FRM, Section \ref{sec:frm-exclusion-principle}
...
Interventions are vectors in continuous 9-dimensional space (\textbf{Chapter 17, Appendix IE}):
...
\textit{Reference:} Appendix FRM (Axiom EXC-3), \texttt{data/formula-registry.yaml} (ID: K*-Ch20)
...
\textit{See Appendix SS (CORE-SOLSPACE) for complete formalization.}
...
From \textbf{Chapter 17, Appendix IE}:
...
\begin{itemize}
\item Creates utility interdependence (Chapter 5, Appendix B)
\item Generates non-concave landscapes (Chapter 7)
\item Enables social preferences and spillovers (Chapter 10, Appendix AAA)
\end{itemize}
...
(CORE-WHERE, Appendix BBB) derives from 1,900+ experimental papers, field studies,
...
\item \textbf{Parameter source:} $\Theta$ in equation (\ref{eq:mp-objective}) comes from
      1,900+ papers, not assumption. See Appendix BBB (CORE-WHERE).
...
\item \textbf{163 parameters to estimate:} The solution space $\mathcal{S}(\Psi, S)$ depends
      on calibrated $\Gamma$, $\Lambda$, and $\delta$ values from Appendix BBB.
...
\item \textbf{Calibration requirement:} Parameters $\alpha, \beta, \gamma$ in $\Phi$ must be calibrated from experimental data (Appendix BBB).
...
\textit{Proof: See Chapter 7, Proposition 7.1, and Appendix NC (FORMAL-NC).}
...
\item \textbf{99+ axioms:} Complete specification in Appendix AV (CORE-READY).
\end{enumerate}
...
\item \textbf{85 axioms, 152 boundary conditions:} Complete specification in Appendix AW (CORE-STAGE).
\end{enumerate}
...
\item \textbf{13 axioms (SEG-1 to SEG-13):} Complete specification in Appendix BA (FORMAL-SEGMENT).
\end{enumerate}
...
\item \textbf{12 axioms, 15 theorems:} Complete specification in Appendix HI (CORE-HIERARCHY).
\end{enumerate}
...
The coupled ODE system from Appendix UN:
...
\item \textbf{10 axioms (P1-P10):} Complete specification in Appendix PBB (FORMAL-PROBABILITY).
\end{enumerate}
...
\item \textbf{20-field schema:} Complete specification in Appendix HHH (METHOD-TOOLKIT).
\end{enumerate}
...
\item \textbf{EIT-4, EIT-5:} Complete specification in Appendix HHH (METHOD-TOOLKIT).
\end{enumerate}
...
\end{equation}
Segmentation: $\beta > 0.9$ (Time-consistent), $\beta \in [0.7, 0.9]$ (Mild PB), $\beta < 0.7$ (Strong PB) \citep{PAP-odonoghue1999doingdoing, PAP-GruberKoszegi2001}.
\end{tcolorbox}

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black,
    title=\textbf{Axiom PAP1-3: Autonomy-Reactance Heterogeneity \citep{deci1999meta, brehm1966}}]
\begin{equation}
\boxed{
...
population-level behavior in a domain \citep{halpern2015, service2014think}, subject to coherence constraints and cross-domain
spillover effects.
...
optimization logic applies at all organizational levels \citep{ostrom1990governing, acemoglu2015state, PAP-north1990institutionsinstitutions}.
...
\item \textbf{Context-enabled reachability}: $S^* \notin \mathcal{R}(S_0, T)$ but $\exists$ context change sequence enabling access (Valley Problem, see Appendix SS)
...
\textbf{MP-A32 (163 Parameters):} The complete complementarity structure requires $\Gamma$ (15 FEPSDE interactions) + $\Lambda$ (28 context interactions) + $\delta$ (120 modulation tensor) = 163 parameters calibrated from experimental evidence (Appendix BBB).
...
\textbf{Reference:} Appendix IE (CORE-EIT), core-framework-definition.yaml (stage\_6)
...
$\Theta$ & \textbf{Parameter Set}---all calibrated parameters from 1,900+ empirical studies (Appendix BBB). \\[6pt]
...
Complementarity ($\gamma > 0$) generates non-concavity (Chapter 7, Appendix NC):
\begin{itemize}
...
\item \textbf{Behavioral Economics}: \citet{PAP-kahneman1979prospect} on prospect theory;
      \citet{PAP-thaler2008} on nudging; \citet{fehr1999} on social preferences;
      \citet{fehrschmidt1999} on inequality aversion

\item \textbf{Intertemporal Choice}: \citet{laibson1997} on $\beta$-$\delta$ discounting;
      \citet{PAP-odonoghue1999doing} on sophistication vs. naïveté;
...
\item \textbf{Solution Space Formalization}: Appendix SS (CORE-SOLSPACE)
...
\item Ch5-7 (Complementarity): See Appendix B, NC, MIL (Milgrom-Roberts)
\item Ch9-15 (10C CORE): See individual CORE appendices (AAA, B, C, V, BBB, AU, AV, AW, HI)
\item Ch16 ($P_{eff}$): See Appendix PBB (FORMAL-PROBABILITY), UN (UNTCM)
\item Ch17-21 (Interventions): See Appendix IE, HHH, KKK
\item Ch22-24 (Scaling): See Appendix LLL, NNN, RRR, OOO, PPP
\end{itemize}
