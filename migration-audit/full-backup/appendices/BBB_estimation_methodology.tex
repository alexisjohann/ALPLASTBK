...
\title{Appendix BBB: Methodological Foundations of Parameter Estimation\\
...
\textbf{Dependencies:} & Appendix B (\ref{app:complementarity-how}) (Complementarity Theory) \\
 & Appendix AN (\ref{app:llm-monte-carlo}) (LLM Monte Carlo) \\
 & Appendix E (\ref{app:operationalization}) (Empirical Operationalization) \\
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black, 
    title=\textbf{Appendix BBB: The Epistemics of Numbers}]
...
\midrule
Utility-Utility Complementarity & $\Gamma = [\gamma_{dd'}]$ & 15 & Appendix B (\ref{app:complementarity-how}), Teil II \\
Context-Context Complementarity & $\Lambda = [\lambda_{kl}]$ & 28 & Appendix B (\ref{app:complementarity-how}), Teil IV \\
Context Modulation & $\delta_{dd'k}$ & 120 & Appendix B (\ref{app:complementarity-how}), Teil III \\
Base Utility Weights & $w_d$ & 6 & Appendix C (\ref{app:fepsde-matrix}) \\
Reference Points & $r_d$ & 6 & Appendix C (\ref{app:fepsde-matrix}) \\
Discount Factor & $\delta$ & 1 & Chapter 3 \\
Tipping Thresholds & $\Psi^*_k$ & 8 & Appendix B (\ref{app:complementarity-how}), Teil VI \\
Coherence Threshold & $K^*$ & 1 & Appendix B (\ref{app:complementarity-how}), Teil I \\
\midrule
...
\textbf{Reference:} Appendix E (\ref{app:operationalization}) (Operationalization), Chapter 4
\end{tcolorbox}
...
\textbf{Reference:} Appendix AN (\ref{app:llm-monte-carlo}) (LLM Monte Carlo Methodology)
\end{tcolorbox}
...
\textbf{The LLM-MC Protocol} (detailed in Appendix AN (\ref{app:llm-monte-carlo})):
\begin{enumerate}
...
\textbf{Reference:} Appendix AU (\ref{app:axiom-formalization}) (Axiom Formalization)
\end{tcolorbox}
...
\bottomrule
\end{tabular}
\caption{Axioms defined in Appendix BBB}
...
\hrule
\vspace{0.5em}
\noindent\textit{Cross-references: Appendix B (\ref{app:complementarity-how}) (Complementarity Theory), 
Appendix AN (\ref{app:llm-monte-carlo}) (LLM Monte Carlo), Appendix E (\ref{app:operationalization}) (Operationalization), 
Appendix AU (\ref{app:axiom-formalization}) (Axiom Formalization), Chapter 4 (Calibration)}
...
\bottomrule
\caption{Complete notation for Appendix BBB}
...
\begin{equation}
I^2 = \frac{Q - (K-1)}{Q} < 0.5 \quad \text{(Low heterogeneity)}
\end{equation}
...
\bottomrule
\caption{Complete axiom index for Appendix BBB}
...
Appendix BBB serves as the \textbf{methodological hub} of the EBF framework, 
...
\midrule
\endfirsthead
Appendix B (\ref{app:complementarity-how}) & Complementarity theory, $\gamma$ definitions & Defines \textit{what} parameters mean \\
Appendix V (\ref{app:psi-dimensions}) & 8$\Psi$-dimension definitions & Context vector for $\delta$-modulation \\
Appendix C (\ref{app:fepsde-matrix}) & FEPSDE utility structure & Defines dimensions $d \in \mathcal{D}$ \\
Appendix AU (\ref{app:axiom-formalization}) & Axiom formalization & Tier 3 axiomatic constraints \\
Appendix AN (\ref{app:llm-monte-carlo}) & LLM-MC protocol details & Method B implementation \\
Appendix E (\ref{app:operationalization}) & Empirical data sources & Tier 1 calibration inputs \\
Chapter 4 & Application cases & Validation benchmarks \\
\bottomrule
\caption{Upstream dependencies for Appendix BBB}
\end{longtable}
...
\midrule
\endfirsthead
Appendix B (\ref{app:complementarity-how}) & Calibrated $\Gamma$, $\Lambda$, $\delta$ & Quantified complementarity for analysis \\
Appendix AAA (\ref{app:aggregation-levels}) & Level-specific parameters & Parameters by aggregation level \\
Appendix C (\ref{app:fepsde-matrix}) & Calibrated weights $w_d$ & FEPSDE utility specification \\
...
\bottomrule
\caption{Downstream outputs from Appendix BBB}
\end{longtable}
...
\begin{enumerate}
    \item \textbf{Context Assessment:} Measure $\boldsymbol{\Psi}$ using Appendix V (\ref{app:psi-dimensions}) instruments
    ...
    \item \textbf{Complementarity Analysis:} Apply Appendix B (\ref{app:complementarity-how}) theory to identify leverage points
...
\midrule
Theory of complementarity & Appendix B (\ref{app:complementarity-how}) & Teil I-II \\
...
$\Psi$-dimension definitions & Appendix V (\ref{app:psi-dimensions}) & All \\
FEPSDE definitions & Appendix C (\ref{app:fepsde-matrix}) & Teil I \\
Estimation method details & \textbf{BBB} & Five Methods \\
LLM-MC protocol & Appendix AN (\ref{app:llm-monte-carlo}) & Protocol \\
...
Social, Digital, Ecological. Defined in Appendix C (\ref{app:fepsde-matrix}).
...
source tiers, and epistemic status. Central reference in Appendix BBB.
...
Market Scope ($\Psi_M$), Factor Flexibility ($\Psi_F$). Defined in Appendix V (\ref{app:psi-dimensions}).
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, 
    title=\textbf{Appendix BBB Summary: The Epistemics of Numbers}]
...
The following elements from Appendix BBB are \textbf{foundational} for the 
