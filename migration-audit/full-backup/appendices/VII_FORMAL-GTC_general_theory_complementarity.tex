...
%
% Cross-Reference Map:
%   - Appendix B (CORE-HOW): Basic γ definition
%   - Appendix IV (LET): Level emergence from γ
%   - Appendix V (MEP): Portfolio optimization with Γ matrix
%   - Appendix VI (MEQ): Equilibria dependent on γ structure
%   - Appendix III (EIH): Efficiency with ∏(1+γ)
%   - Appendix C (CORE-WHAT): Utility dimension complementarities
...
\begin{itemize}[nosep]
    \item \textbf{Appendix B (CORE-HOW):} Original $\gamma$ definition
    \item \textbf{Appendix C (CORE-WHAT):} Utility dimension complementarities
    \item \textbf{Appendix III (FORMAL-EIH):} Uses $\prod(1+\gamma)$
    \item \textbf{Appendix IV (FORMAL-LET):} Emergence premium from $\bar{\gamma}$
    \item \textbf{Appendix V (FORMAL-MEP):} Portfolio optimization with $\Gamma$
    \item \textbf{Appendix VI (FORMAL-MEQ):} Equilibrium structure from $\Gamma$
    \item \textbf{Appendix HHH (METHOD-TOOLKIT):} Intervention complementarities
    \item \textbf{Appendix BA (FORMAL-SEGMENT):} Segment-specific $\gamma^S$
\end{itemize}
