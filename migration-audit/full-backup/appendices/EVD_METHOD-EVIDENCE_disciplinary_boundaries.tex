...
The false precision problem emerges from \textbf{implicit} rather than \textbf{explicit} evidence hierarchies. When researchers do not articulate their assumptions about evidence quality, systematic errors become invisible and uncorrectable \citep{PAP-ioannidis2005}. This insight builds on decades of work in philosophy of science showing that much scientific knowledge is ``tacit''---we can know more than we can tell \citep{PAP-polanyi1966tacit}.
...
\textbf{5. Replication Illusion} \citep{simmons2011}
\begin{quote}
...
\textbf{Within disciplines, implicit hierarchies work} \citep{kuhn1962, collins1985}:
\begin{itemize}[nosep]
    \item Economists know which journals require causal identification
    \item Psychologists know which findings replicate
    \item Shared tacit knowledge enables quality judgments \citep{PAP-polanyi1966tacit}
\end{itemize}

\textbf{Across disciplines, implicit hierarchies fail:}
\begin{itemize}[nosep]
    \item Economists may not know psychology's replication crisis \citep{osc2015, camerer2016}
    \item Psychologists may not distinguish IV from OLS
    \item Practitioners (consultants, policymakers) lack disciplinary context
    \item LLMs have no tacit knowledge at all \citep{PAP-collins2010tacit}
\end{itemize}
...
The solution follows the path pioneered by evidence-based medicine with the GRADE system \citep{PAP-guyatt2008grade} and the open science movement's push for transparency \citep{nosek2015, munafò2017}:
...
\item \textbf{LLMMC Parameter Estimation (Appendix AN):}
\begin{itemize}[nosep]
