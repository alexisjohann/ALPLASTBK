...
\begin{itemize}[nosep]
\item Appendix AN (METHOD-LLMMC): Parameter estimation via LLM simulation
\item Appendix R (METHOD-EVAL): Validation framework
\item Appendix AZ (METHOD-CONSTRUCT): Model construction microfoundations
\item Appendix BBB (CORE-WHERE): Where parameters originate (literature vs. field vs. synthetic)
\end{itemize}
...
This appendix integrates evidence from behavioral economics literature on parameter estimation, validated through cross-validation audit of Chapters 17-19. See Appendix AN (METHOD-LLMMC) for parameter estimation methodology and Appendix R (METHOD-EVAL) for validation protocols.
