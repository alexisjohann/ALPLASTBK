...
class TestHistoricalValidation(unittest.TestCase):
    ...
    def test_duration_estimates_reasonable(self):
        ...
        # Average error should be reasonable (within 2 rounds for 12 conclaves spanning 147 years)
        self.assertLess(validation['average_duration_error'], 2.0,
                       msg="Average duration error should be < 2 rounds")
...
class TestConclaveDurationEstimate(unittest.TestCase):
    ...
    def test_duration_high_network_high_predecessor(self):
        ...
        # Λ=0.95, Π=0.92 → 10/1.87 ≈ 5.3 → 5 rounds
        candidate = CandidateParameters(
        ...
        # Should predict relatively short conclave (≤6 rounds)
        self.assertLessEqual(duration, 6,
...
class TestGammaParameters(unittest.TestCase):
    """Test GammaParameters dataclass"""

    def test_default_values(self):
        """Verify default gamma values match model-definition.yaml"""
        g = GammaParameters()
        self.assertEqual(g.gamma_lambda_pi, 0.80)
        self.assertEqual(g.gamma_iota_pi, 0.50)
        self.assertEqual(g.gamma_lambda_iota, 0.40)
        self.assertEqual(g.gamma_nu_alpha, 0.30)

    def test_zeros(self):
        """Verify zeros() creates additive (no interaction) model"""
        g = GammaParameters.zeros()
        self.assertTrue(np.allclose(g.to_vector(), 0.0))

    def test_vector_roundtrip(self):
        """Verify to_vector() and from_vector() are inverse"""
        g = GammaParameters()
        g2 = GammaParameters.from_vector(g.to_vector())
        np.testing.assert_array_almost_equal(g.to_vector(), g2.to_vector())

    def test_labels_count(self):
        """Verify 8 labels for 8 gamma parameters"""
        self.assertEqual(len(GammaParameters.LABELS), 8)


class TestInteractionModel(unittest.TestCase):
    """Test interaction (complementarity) model extensions"""

    def setUp(self):
        self.model_additive = PapalSuccessionFramework(
            "model-definition.yaml", use_interactions=False)
        self.model_interaction = PapalSuccessionFramework(
            "model-definition.yaml", use_interactions=True)

    def test_gamma_loaded_from_config(self):
        """Verify gamma parameters loaded from model-definition.yaml"""
        g = self.model_interaction.gamma_params
        self.assertAlmostEqual(g.gamma_lambda_pi, 0.80)
        self.assertAlmostEqual(g.gamma_iota_pi, 0.50)
        self.assertAlmostEqual(g.gamma_lambda_iota, 0.40)

    def test_interaction_increases_probability(self):
        """With positive gammas and high scores, interactions should boost probability"""
        candidate = CandidateParameters(
            name="Strong Candidate",
            lambda_=0.85, iota=0.90, pi=0.85, nu=0.80, alpha=0.90
        )

        prob_add = self.model_additive.calculate_individual_probability(candidate)
        prob_int = self.model_interaction.calculate_individual_probability(candidate)

        self.assertGreater(prob_int, prob_add,
                          "Interaction model should increase probability for strong candidates")

    def test_interaction_effect_scales_with_scores(self):
        """Interaction contribution should be larger for higher dimension scores"""
        low = CandidateParameters(
            name="Low", lambda_=0.3, iota=0.3, pi=0.3, nu=0.3, alpha=0.3)
        high = CandidateParameters(
            name="High", lambda_=0.9, iota=0.9, pi=0.9, nu=0.9, alpha=0.9)

        int_low = self.model_interaction._calculate_interaction_contribution(low)
        int_high = self.model_interaction._calculate_interaction_contribution(high)

        self.assertGreater(int_high, int_low,
                          "Interaction contribution should be larger for higher scores")

    def test_additive_model_unchanged(self):
        """Additive model should be unaffected by gamma parameters"""
        candidate = CandidateParameters(
            name="Test", lambda_=0.7, iota=0.7, pi=0.7, nu=0.7, alpha=0.7)

        prob_add = self.model_additive.calculate_individual_probability(candidate)
        prob_explicit = self.model_interaction.calculate_individual_probability(
            candidate, use_interactions=False)

        self.assertAlmostEqual(prob_add, prob_explicit, places=10,
                              msg="Explicit use_interactions=False should match additive model")

    def test_use_interactions_override(self):
        """use_interactions parameter should override instance setting"""
        candidate = CandidateParameters(
            name="Test", lambda_=0.8, iota=0.8, pi=0.8, nu=0.8, alpha=0.8)

        # Additive model with interactions forced on
        prob_forced = self.model_additive.calculate_individual_probability(
            candidate, use_interactions=True)
        # Interaction model normal
        prob_normal = self.model_interaction.calculate_individual_probability(candidate)

        self.assertAlmostEqual(prob_forced, prob_normal, places=10)

    def test_interaction_breakdown_sums_to_total(self):
        """Individual interaction terms should sum to total"""
        candidate = CandidateParameters(
            name="Test", lambda_=0.8, iota=0.7, pi=0.9, nu=0.6, alpha=0.85)

        total = self.model_interaction._calculate_interaction_contribution(candidate)
        breakdown = self.model_interaction._get_interaction_breakdown(candidate)

        self.assertAlmostEqual(sum(breakdown.values()), total, places=10)

    def test_historical_accuracy_with_interactions(self):
        """Interaction model should maintain 100% historical accuracy"""
        validation = self.model_interaction.validate_against_historical_data()
        self.assertGreaterEqual(validation['accuracy'], 0.87,
                               msg="Interaction model should maintain high accuracy")

    def test_contributions_include_interactions(self):
        """get_dimension_contributions should include interaction_total when enabled"""
        candidate = CandidateParameters(
            name="Test", lambda_=0.8, iota=0.7, pi=0.6, nu=0.5, alpha=0.4)

        contrib_add = self.model_additive.get_dimension_contributions(candidate)
        contrib_int = self.model_interaction.get_dimension_contributions(candidate)

        self.assertNotIn('interaction_total', contrib_add)
        self.assertIn('interaction_total', contrib_int)
        self.assertGreater(contrib_int['interaction_total'], 0)


class TestGammaEstimation(unittest.TestCase):
    """Test gamma parameter estimation pipeline"""

    def setUp(self):
        self.model = PapalSuccessionFramework(
            "model-definition.yaml", use_interactions=True)

    def test_historical_dataset_generated(self):
        """Verify synthetic dataset has correct shape"""
        X, y, labels = self.model._build_historical_dataset()

        # 12 conclaves × (1 winner + 3 runners) = 48
        self.assertEqual(X.shape[0], 48)
        self.assertEqual(X.shape[1], 5)
        self.assertEqual(len(y), 48)
        self.assertEqual(int(y.sum()), 12)  # 12 winners

    def test_interaction_matrix_shape(self):
        """Verify interaction matrix has 8 columns"""
        X, y, _ = self.model._build_historical_dataset()
        X_int = self.model._compute_interaction_matrix(X)

        self.assertEqual(X_int.shape[0], 48)
        self.assertEqual(X_int.shape[1], 8)

    def test_fit_gamma_runs(self):
        """Verify gamma fitting runs without errors"""
        result = self.model.fit_gamma_parameters(n_bootstrap=50)

        self.assertIn('fitted_gammas', result)
        self.assertIn('gamma_details', result)
        self.assertIn('model_comparison', result)
        self.assertIn('dataset_info', result)

    def test_fit_gamma_returns_valid_values(self):
        """Verify fitted gammas are finite and reasonable"""
        result = self.model.fit_gamma_parameters(n_bootstrap=50)
        gammas = result['fitted_gammas'].to_vector()

        self.assertTrue(np.all(np.isfinite(gammas)),
                       "All fitted gamma values should be finite")
        # Gammas should be in reasonable range
        self.assertTrue(np.all(np.abs(gammas) < 10),
                       "Fitted gammas should be in reasonable range")

    def test_model_comparison_structure(self):
        """Verify model comparison contains AIC/BIC"""
        result = self.model.fit_gamma_parameters(n_bootstrap=50)
        comp = result['model_comparison']

        self.assertIn('additive', comp)
        self.assertIn('interaction', comp)
        self.assertIn('delta_aic', comp)
        self.assertIn('interaction_preferred_aic', comp)

    def test_compare_models_returns_all_conclaves(self):
        """Verify compare_models covers all 12 conclaves"""
        result = self.model.compare_models()
        self.assertEqual(len(result['conclaves']), 12)


def run_test_suite():
    """Run complete test suite and report results"""
    print("\n" + "=" * 80)
    print("PSF 2.0 TEST SUITE (v1.2 — with Interaction Model)")
    print("=" * 80 + "\n")
    ...
    suite.addTests(loader.loadTestsFromTestCase(TestGammaParameters))
    suite.addTests(loader.loadTestsFromTestCase(TestInteractionModel))
    suite.addTests(loader.loadTestsFromTestCase(TestGammaEstimation))
